...
# PACKAGE_DIR is repo-relative; the packet wrapper sets it. compose.yaml is read only for the pinned digest.
...
printf '%s preflight (profile: %s)\n\n' "${PACKET_NAME:-container packet}" "${profile}"
...
[[ "${ram}" -ge $(( ${MODEL_GIB:-11} + 1 )) ]] && ok "${ram} GiB system RAM" || note "${ram} GiB system RAM; the container is limited to 12g and may need swap"
model_dir=${MODEL_DIR:-}
if [[ -n "${model_dir}" && -d "${model_dir}" ]]; then
  ...
  if [[ -n "${avail}" ]]; then
    [[ "${avail}" -ge $(( ${MODEL_GIB:-11} + 1 )) ]] && ok "${avail} GiB free at ${target} (model needs ~'"${MODEL_GIB:-11}"' GiB)" || bad "${avail} GiB free at ${target}; the model needs about '"${MODEL_GIB:-11}"' GiB"
  else
    note "could not measure free space; the model needs about '"${MODEL_GIB:-11}"' GiB"
  fi
fi

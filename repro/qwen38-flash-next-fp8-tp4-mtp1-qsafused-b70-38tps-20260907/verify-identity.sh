...
die() { printf 'fused-QSA MTP1 identity: %s\n' "$*" >&2; exit 2; }
...
REPRO_VLLM_TREE="$vllm_tree" "$repo/patches/qwen38-flash-next-fp8-b70/vllm-qsafused-mtp1-6d872457/verify-series.sh" >/dev/null
[[ "$(git -C "$vllm_tree" rev-parse HEAD)" == 6d8724577dabbee5fa0bbc70c4d927c6174c8d8a ]] || die "vLLM tree is not checked out at 6d872457 (git checkout q38-qsafused-mtp1-6d872457)"
...
(cd "$repo/experiments/qwen38-flash-next-fp8-b70/tools" && sha256sum --quiet -c "$script_dir/frozen-a306-packet.sha256") || die "frozen A189 packet drifted"
...
PY
echo "fused-QSA MTP1 identity verified: vLLM 6d872457 over 62219122 over 005dc578 over 1b2a17c1 over 76cfe1cd, stage 2f829747, oneCCL 4ceafd1 public, model bcd9f01d"

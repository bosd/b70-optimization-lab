...
attempt="${REPRO_ATTEMPT:?set REPRO_ATTEMPT to an unused attempt number above 306}"
...
die() { printf 'fused-QSA MTP1 record gate: %s\n' "$*" >&2; exit 2; }
[[ "$attempt" =~ ^[1-9][0-9]*$ && "$attempt" -gt 306 ]] || die "REPRO_ATTEMPT must be a number above 306"

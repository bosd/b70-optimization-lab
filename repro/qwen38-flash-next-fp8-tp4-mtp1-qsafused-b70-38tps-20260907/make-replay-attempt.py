#!/usr/bin/env python3
"""Derive a replay packet a<N> from the frozen A306 packet (MTP1 with both reference kernels restored, 37.825654 tok/s).

The four A306 scripts are pinned by frozen-a306-packet.sha256. The derived packet is
...
SRC = {
    "launcher": "launch-tp4-mtp1-4352-ple-only-a306-fullgraphdet-w13n32.sh",
    "client": "run-tp4-mtp1-4352-ple-only-a306-fullgraphdet-w13n32-client.sh",
    "supervisor": "supervise-tp4-mtp1-4352-ple-only-a306-fullgraphdet-w13n32.sh",
    "host": "run-q38-a306-host-controlled.sh",
}
SRC_PORT = "19975"
...
def pinned() -> dict[str, str]:
    pins = {}
    for line in (HERE / "frozen-a306-packet.sha256").read_text().splitlines():
        sha, name = line.split()
        pins[name] = sha
    return pins
...
def source(name: str, pins: dict[str, str]) -> str:
    ...
    if digest(data) != pins[name]:
        sys.exit(f"frozen A306 packet drifted: {name}")
    return data.decode()

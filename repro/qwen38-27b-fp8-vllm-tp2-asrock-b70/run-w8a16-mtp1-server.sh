...
decoder_boundary_trace_file=${VLLM_XPU_DECODER_BOUNDARY_TRACE_FILE:-}
...
[[ "${draft_lm_head_int4_apply_rows}" =~ ^[0-9]+$ ]] || { printf 'VLLM_XPU_DRAFT_LM_HEAD_INT4_APPLY_ROWS must be a non-negative integer\n' >&2; exit 1; }
...
if [[ -n "${gdn_state_input_trace_file}" ]]; then
  case "${gdn_state_input_trace_file}" in
    /root/.cache/vllm/*) ;;
    *)
...
[[ "${lm_head_chunk_rows}" =~ ^[0-9]+$ && "${lm_head_chunk_rows}" -le 512 ]] || {
  printf 'VLLM_XPU_LM_HEAD_CHUNK_ROWS must be an integer in 0..512\n' >&2
  exit 1
}
for value_name in gemma_rmsnorm_triton rmsnorm_triton gdn_split_mixed; do
  value=${!value_name}
...
exec docker run --rm --name "${container}" \
  ...
  --env VLLM_XPU_GDN_SPLIT_MIXED="${gdn_split_mixed}" \
  ...
  --env VLLM_XPU_DECODER_BOUNDARY_TRACE_FILE="${decoder_boundary_trace_file}" \

...
die() { printf 'Triton-HC MTP1 identity: %s\n' "$*" >&2; exit 2; }
...
# 1. overlay source: the placement series (nine patches over the lossless MTP1 head) and the Triton-HC series (one patch over the placement head), bundles, tags, trees
REPRO_VLLM_TREE="$vllm_tree" "$repo/patches/qwen38-flash-next-fp8-b70/vllm-placement-mtp1-005dc578/verify-series.sh" >/dev/null
REPRO_VLLM_TREE="$vllm_tree" "$repo/patches/qwen38-flash-next-fp8-b70/vllm-hctriton-mtp1-62219122/verify-series.sh" >/dev/null
[[ "$(git -C "$vllm_tree" rev-parse HEAD)" == 622191221475b53cc6f7f4d847860939f4c300ab ]] || die "vLLM tree is not checked out at 62219122 (git checkout q38-hctriton-mtp1-62219122)"
...
# 5. tuned MoE map, exactness verifier, frozen packet, expert placement file
[[ "$(sha256sum "$repo/experiments/qwen38-flash-next-fp8-b70/data/20260906-q38-expert-host-placement-3p5gib-per-rank.json" | cut -d' ' -f1)" == f3f4812cfae2f4f704e75cc9b47a4031a476f846b353e3def23141b9d9c05f96 ]] || die "expert placement file drifted"
...
pin_sha=$(sed -n 's/^sha256=//p' "$script_dir/verifier-pin.txt")
[[ "$(sha256sum "$script_dir/verify-moe-selection-frozen.py" | cut -d' ' -f1)" == "$pin_sha" ]] || die "the packaged exactness verifier differs from verifier-pin.txt"
(cd "$repo/experiments/qwen38-flash-next-fp8-b70/tools" && sha256sum --quiet -c "$script_dir/frozen-a272-packet.sha256") || die "frozen A189 packet drifted"
...
PY
echo "Triton-HC MTP1 identity verified: vLLM 62219122 over 005dc578 over 1b2a17c1 over 76cfe1cd, stage 2f829747, oneCCL 4ceafd1 public, model bcd9f01d"

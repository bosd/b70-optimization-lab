...
def successor(text: str, attempt: int, port: str) -> str:
    def rename(seg: str) -> str:
        ...
        seg = seg.replace("ATTEMPT=226", f"ATTEMPT={attempt}").replace("a272", f"a{attempt}").replace("A272", f"A{attempt}")
        return seg
    parts, last = [], 0
    ...
    assert SRC_PORT not in out and "attempt272" not in out and "ATTEMPT=226" not in out and "a272-" not in out
    return out

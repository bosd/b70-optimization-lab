...
SRC = {
    "launcher": "launch-tp4-mtp1-4352-ple-only-a272-fullgraphdet-w13n32.sh",
    "client": "run-tp4-mtp1-4352-ple-only-a272-fullgraphdet-w13n32-client.sh",
    "supervisor": "supervise-tp4-mtp1-4352-ple-only-a272-fullgraphdet-w13n32.sh",
    "host": "run-q38-a272-host-controlled.sh",
}
SRC_PORT = "19942"
...
def pinned() -> dict[str, str]:
    pins = {}
    for line in (HERE / "frozen-a272-packet.sha256").read_text().splitlines():
        sha, name = line.split()
        pins[name] = sha
    return pins
...
def source(name: str, pins: dict[str, str]) -> str:
    ...
    if digest(data) != pins[name]:
        sys.exit(f"frozen A272 packet drifted: {name}")
    return data.decode()
...
def successor(text: str, attempt: int, port: str) -> str:
    def rename(seg: str) -> str:
        ...
        seg = seg.replace("attempt272", f"attempt{attempt}").replace(SRC_PORT, port)
        seg = seg.replace("ATTEMPT=272", f"ATTEMPT={attempt}").replace("a272", f"a{attempt}").replace("A272", f"A{attempt}")
        return seg
    parts, last = [], 0
    ...
    assert SRC_PORT not in out and "attempt272" not in out and "ATTEMPT=272" not in out and "a272-" not in out
    return out

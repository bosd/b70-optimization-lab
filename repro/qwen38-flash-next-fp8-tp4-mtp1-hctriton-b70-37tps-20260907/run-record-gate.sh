...
attempt="${REPRO_ATTEMPT:?set REPRO_ATTEMPT to an unused attempt number above 272}"
...
die() { printf 'Triton-HC MTP1 record gate: %s\n' "$*" >&2; exit 2; }
[[ "$attempt" =~ ^[1-9][0-9]*$ && "$attempt" -gt 272 ]] || die "REPRO_ATTEMPT must be a number above 272"

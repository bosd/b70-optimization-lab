#!/usr/bin/env bash
# The record's server line (A272 server-command.shell.txt; Q38_EXPERT_HOST_PLACEMENT names the never-routed expert placement file) with the model path
...
export Q38_EXPERT_HOST_PLACEMENT=/opt/expert-host-placement-3p5gib-per-rank.json
exec vllm serve "$model" --host 0.0.0.0 --port "$port" --served-model-name qwen38-flash-next-fp8-tp4 \
  ...
  --offload-backend uva --cpu-offload-gb 12.25 --cpu-offload-params ple_embedding.ngram_embedding.weight embed_tokens.weight \

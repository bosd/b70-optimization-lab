#!/usr/bin/env python3
"""Derive a replay packet a<N> from the frozen A189 packet (lossless MTP1, 27.048 tok/s).

The four A189 scripts are pinned by frozen-a189-packet.sha256. The derived packet is
...
SRC = {
    "launcher": "launch-tp4-mtp1-4352-ple-only-a189-fullgraphdet-w13n32.sh",
    "client": "run-tp4-mtp1-4352-ple-only-a189-fullgraphdet-w13n32-client.sh",
    "supervisor": "supervise-tp4-mtp1-4352-ple-only-a189-fullgraphdet-w13n32.sh",
    "host": "run-q38-a189-host-controlled.sh",
}
SRC_PORT = "19860"
...
def pinned() -> dict[str, str]:
    pins = {}
    for line in (HERE / "frozen-a189-packet.sha256").read_text().splitlines():
        sha, name = line.split()
        pins[name] = sha
    return pins
...
def source(name: str, pins: dict[str, str]) -> str:
    ...
    if digest(data) != pins[name]:
        sys.exit(f"frozen A189 packet drifted: {name}")
    return data.decode()
...
def successor(text: str, attempt: int, port: str) -> str:
    def rename(seg: str) -> str:
        seg = seg.replace("tp4-mtp1-4352-ple-only-a189", f"tp4-mtp1-4352-ple-only-a{attempt}")
        seg = seg.replace("attempt189", f"attempt{attempt}").replace(SRC_PORT, port)
        seg = seg.replace("ATTEMPT=189", f"ATTEMPT={attempt}").replace("a189", f"a{attempt}").replace("A189", f"A{attempt}")
        return seg
    parts, last = [], 0
...
def main() -> int:
    ...
    assert attempt > 189 and port.isdigit() and port != SRC_PORT
    pins = pinned()

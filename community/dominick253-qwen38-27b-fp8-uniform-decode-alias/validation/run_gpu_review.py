        ...
        for arm in args.arms:
            ...
            try:
                ...
                if record_path.exists():
                    ...
                    record.update(status="failed", final_error=str(exc), campaign_stage_complete=False)
                    record_path.write_text(json.dumps(record, indent=2) + "\n")
                raise
            else:
                record = json.loads(record_path.read_text())
                record.update(status="passed", campaign_stage_complete=True,
...
def run_arm(args, arm):
    ...
    probe_only = arm == "candidate-no-compile"
    if probe_only:

...
| --- | --- | ---: | --- |
| **[Qwen3.5 4B W4A16 with its own MTP head on one or two Intel Arc Pro B70](models/qwen35-4b-w4a16-b70.html)**<br>1&times; B70 · W4A16 (compressed-tensors INT4 weights, FP16 activations) · vLLM XPU | `candidate` · clean-host replay pending | **`177.287 tok/s`**<br>MTP depth 3 with the draft-only INT4 lm_head, one card, strict completions suite (center of two fresh servers, 177.406 / 177.168) | [reproduction guide](repro/qwen35-4b-w4a16-b70/README.md) |
...
| **[Qwen3.5 9B W4A16 with its own MTP head on one or two Intel Arc Pro B70](models/qwen35-9b-w4a16-b70.html)**<br>1&times; B70 · W4A16 (compressed-tensors INT4 weights, FP16 activations) · vLLM XPU | `candidate` · clean-host replay pending | **`113.265 tok/s`**<br>MTP depth 3 with the draft-only INT4 lm_head, one card, strict completions suite (center of two fresh servers, 113.627 / 112.904) | [reproduction guide](repro/qwen35-9b-w4a16-b70/README.md) |
...
| **[Qwen3.8 Flash-Next FP8 with no speculation, never-routed experts host-placed, both reference Triton kernels restored and the W13 MoE tile at the base width, on four Intel Arc Pro B70 cards](models/qwen38-flash-next-fp8-tp4-mtp0-w13n64-b70-34tps-20260908.html)**<br>4&times; B70 · FP8 block-128 weights / BF16 KV · vLLM XPU | `candidate` · clean-host replay pending | **`37.825654 tok/s`**<br>class-balanced decode median | [reproduction guide](repro/qwen38-flash-next-fp8-tp4-mtp0-w13n64-b70-34tps-20260908/README.md) |
| **[Qwen3.8 Flash-Next FP8 with lossless MTP1, never-routed experts host-placed and both reference Triton kernels restored, on four Intel Arc Pro B70 cards](models/qwen38-flash-next-fp8-tp4-mtp1-qsafused-b70-38tps-20260907.html)**<br>4&times; B70 · FP8 block-128 weights / BF16 KV · vLLM XPU | `candidate` · clean-host replay pending | **`37.825654 tok/s`**<br>class-balanced decode median | [reproduction guide](repro/qwen38-flash-next-fp8-tp4-mtp1-qsafused-b70-38tps-20260907/README.md) |
...
| **[Qwen3.8 Flash-Next FP8 with lossless MTP1, never-routed experts host-placed and the Triton hyper-connection glue on four Intel Arc Pro B70 cards](models/qwen38-flash-next-fp8-tp4-mtp1-hctriton-b70-37tps-20260907.html)**<br>4&times; B70 · FP8 block-128 weights / BF16 KV · vLLM XPU | `candidate` · clean-host replay pending | **`37.045844 tok/s`**<br>class-balanced decode median | [reproduction guide](repro/qwen38-flash-next-fp8-tp4-mtp1-hctriton-b70-37tps-20260907/README.md) |
...
| **[Qwen3.8 Flash-Next FP8 with lossless MTP1 and never-routed experts host-placed on four Intel Arc Pro B70 cards](models/qwen38-flash-next-fp8-tp4-mtp1-placement-b70-32tps-20260906.html)**<br>4&times; B70 · FP8 block-128 weights / BF16 KV · vLLM XPU | `candidate` · clean-host replay pending | **`31.929484 tok/s`**<br>class-balanced decode median | [reproduction guide](repro/qwen38-flash-next-fp8-tp4-mtp1-placement-b70-32tps-20260906/README.md) |
...
| **[Qwen3.8 Flash-Next FP8 with lossless MTP1 on four Intel Arc Pro B70 cards](models/qwen38-flash-next-fp8-tp4-mtp1-lossless-b70-27tps-20260905.html)**<br>4&times; B70 · FP8 block-128 weights / BF16 KV · vLLM XPU | `candidate` · clean-host replay pending | **`27.048435 tok/s`**<br>class-balanced decode median | [reproduction guide](repro/qwen38-flash-next-fp8-tp4-mtp1-lossless-b70-27tps-20260905/README.md) |
...
| **Qwen3.8 27B GGUF on 2x ASRock B70** | Q4_K_M + Q4_0 MTP2 has a replicated strict **`64.237301 tok/s`** one-user headline (two fresh servers, 24/24 complete candidate arrays exact to MTP0). Q4_K_M target-only remains `49.717503 tok/s` for one user and reaches **`192.341954 tok/s aggregate at c96`** with the exact F16 `ffn_down+ffn_gate` cache: two fresh candidates, 96/96 same-shape batch-oracle identity each, cache zero, effective 49,152-token pool, and near-capacity ~30.5 GiB/card use. The separate sequential comparison was 50/96, so this is not a batch-invariant-text claim. Q8_0 target-only is `36.726447 tok/s`. Speculative, target-only, aggregate, and single-stream rates remain distinct. | [Q4 MTP2 repro](repro/qwen38-27b-q4km-q4mtp-mtp2-tp2-b70/README.md), [Q4 target-only repro](repro/qwen38-27b-q4km-tp2-asrock-b70/README.md), [c96 result](experiments/qwen38-27b-b70/notes/2026-08-30-qwen38-q4km-tp2-exact-f16-cache-c96-result.md), [Q8 primary repro](repro/qwen38-27b-q8-tp2-asrock-b70/README.md) |
| **Qwen3.8 27B official FP8/W8A16 on 2x B70** | **Strict R187: MTP1 `54.935 tok/s`, MTP depth-2 `70.142 tok/s`, MTP depth-3 `79.183 tok/s`, MTP depth-4 `82.396 tok/s`, MTP depth-5 `86.182 tok/s`, MTP0 `33.097 tok/s`** (2026-09-03/04 clean boots; every pair 12/12; repeat-exact at 224-300-token prompts; MTP0 output-identical through c64 at 927.9 tok/s aggregate, MTP1 through c16 at 477.7, depth 3 through c16 at 557.0, depth 4 through c16 at 529.4, depth 5 through c16 at 493.2, depth 2 through c4 at 210.8). R187 = R156 served with one whole-graph torch.compile (`splitting_ops=[]`; on the piecewise compile MTP depth 2 emitted a phantom first token on one request in 64, an unfixed upstream defect that also occurs on the unmodified image; no whole-graph pass has shown it). Prebuilt image on GHCR (`ghcr.io/steveseguin/vllm-openai-xpu-qwen38-fp8`, digest `173660ec…`). LocalMaxxing approved R187 MTP1 `cmtmfa23g01tvl601eyqliy9y`, depth-2 `cmtmfa2i701tyl601yrkxjn46`, depth-3 `cmtmfa2t101u1l601gth9lyx6`, depth-4 `cmtmk61qo01x8l601vtqbitg5`, depth-5 `cmtn23c500005mm010ierd4g3`. R156 = the row-invariant R139 W8A16 image (release `qwen38-fp8-tp2-r139-20260902`, LocalMaxxing `cmtkvle7a0428p701k0ttabyy`) plus a Python mixed-step GDN split. Independent-host replay passes all gates; its slower CPU/fabric lowers c1 decode 1.8x graph-off, 94% recovered graph-on. | [fresh-cache audit](experiments/qwen38-27b-b70/notes/2026-09-01-qwen38-fp8-public-reproduction-audit.md), [R54 result](experiments/qwen38-27b-b70/data/2026-09-01-qwen38-fp8-explicit-deterministic-matrix-r54-result.json), [package](packages/qwen38-27b-fp8-tp2-b70/README.md), [reproduction guide](repro/qwen38-27b-fp8-vllm-tp2-asrock-b70/README.md) |
| **Qwen3.8 27B AutoRound INT4 on 1-2x B70, fixed-K batch-invariant profile** | Refreshed 2026-09-05 on the FP8 lane's whole-graph stack with a rebuilt kernel library (two-tier fixed-K oneDNN W4A16), FP16 linears in 32-row pieces, single-split attention and size-independent Inductor reductions. TP2 with XPU graph capture and the draft-only INT4 head: **depth 4 `112.36 / 112.33 tok/s`** (R257); graphs alone: MTP0 `49.83 / 49.89`, depth 1 `76.72 / 76.63`, depth 4 `91.00 / 91.01`, depth 5 `88.84 / 89.12`, depth 6 `84.06 / 83.98` (R247/R250/R253); all lossless vs the eager MTP0 oracle; eager TP2: MTP0 `34.21 / 35.64`, depth 1 `51.10 / 50.09`, depth 2 `61.14 / 61.54`, depth 3 `67.61 / 67.83`, depth 4 `68.55 / 67.79`; TP1: MTP0 `32.96 / 32.95`, depth 3 `58.47 / 58.47`; every pair 12/12 vs each other and vs the MTP0 oracle (lossless). MTP0 output byte-identical to a single request through 64 concurrent users in 6 of 8 ladders (998.6 tok/s aggregate at c64 on TP2); speculative depths exact through c16 (TP2) / c8 (TP1). The older `101.170` MTP5 rows used vLLM's ARK routing and an output-changing margin and are withdrawn. | [recipe](repro/qwen38-27b-autoround-int4-b70/README.md), [package](packages/qwen38-27b-int4-fixed-k-tp2-b70/README.md), [note](experiments/qwen38-27b-b70/notes/2026-09-05-qwen38-int4-concurrency-identity-r222-r226.md) |
| **Qwen3.8 Flash-Next 125B-A6B official FP8 on 4x B70** | Ongoing research preview, **not deployment-qualified**. The promoted deterministic full-decode-graph MTP0 line now runs at **`25.617613 tok/s`** on the fixed cold realistic suite (class-balanced median; LocalMaxxing-approved `cmtp3g14502cun701y5ey93rh`, 2026-09-05; the prior approved line was 14.433684, `cmtn32b2w000tmm01t7j2wlpn`) and `25.4 tok/s` at exact 2K and 4K, with every output pin unchanged: the old step was the xe driver paging expert weights with the card full to the byte, and 1.6 GiB of expert weights host-offloaded through the existing UVA path ends it. Placing the never-routed experts in host memory instead (hot experts resident, per-expert offset table in the MoE kernel) lifts the no-speculation line to **`27.640875 tok/s`** (`cmtq4elns03ivn701hgvpo053`, 2026-09-06; exact-2K/4K 27.4, every output pin unchanged). The same placement on the lossless MTP1 line is the fastest quality-preserving row at **`31.929484 tok/s`** (`cmtq59cy503jvn701kgvg62zt`, 2026-09-06; exact-2K 33.2, exact-4K 32.5, every output pin equal to the MTP0 line). The earlier lossless MTP1 row with the headroom placement was approved at **`27.048435 tok/s`** (`cmtp5u0ip02eln701lntsl2ns`; every output pin equal to the MTP0 line, exact-2K 28.3-28.7 and exact-4K 27.3-30.3). The older MTP3/MTP4 screens (15.50 / 20.73 tok/s) came from a jittery native server class and stay listed as research. | [public family page](models/qwen-flash-next.html), [dated repro snapshot](repro/qwen38-flash-next-fp8-tp4-mtp3-b70/EXPERIMENTAL-SNAPSHOT-20260831.md), [full result packet](results/qwen38-flash-next-fp8-b70/README.md) |
...
Last audited **2026-08-30**. Qwen3.8 retains Qwen3.6's exact 64-layer,
...
| --- | --- | ---: | --- |
| **GGUF Q4_K_M + external Q4_0 draft, MTP2** | 2x ASRock B70, TP2 target; draft on first card | **`64.237301 tok/s` strict paired headline** | Median of two fresh candidate servers (`64.180644`/`64.293959`) versus matched fresh MTP0 `49.787366` (+29.02%); full 12-prompt/six-class cache-zero suites, objective canaries passed, and 24/24 complete candidate arrays target-exact. Short-context one-user result only; no target-only 32K or concurrency value transfers. [standalone repro](repro/qwen38-27b-q4km-q4mtp-mtp2-tp2-b70/README.md), [promotion attestation](experiments/qwen38-27b-b70/data/2026-08-30-qwen38-q4km-q4mtp-tp2-mtp2-promotion-attestation.json) |
| GGUF Q4_K_M, lab TP2 stack + device-local dense-FFN fusion | 2x ASRock B70, TP2 | **one user `50.219700` (`49.717503` conventional); c96 aggregate `192.341954`** | No speculation; the one-user suite remains 12/12 complete-output exact. The separate two-family exact-cache c96 profile used two fresh candidate servers, prompt cache off, and 96/96 same-shape batch-oracle identity in each. It used an effective 49,152-token pool and about 30.5 GiB/device; isolated sequential identity remained 50/96 and one-user MMVQ decode is unchanged. [standalone repro](repro/qwen38-27b-q4km-tp2-asrock-b70/README.md), [c96 result](experiments/qwen38-27b-b70/notes/2026-08-30-qwen38-q4km-tp2-exact-f16-cache-c96-result.md), [source increment](patches/qwen38-27b-q4km-tp2-asrock-b70/README.md) |
...
| **Official block-scaled FP8 + block-W8A16** | 2x ASRock B70, TP2 | **strict MTP1 `51.808087 tok/s`; target-only `1,112.570323 tok/s` aggregate c128** | The R53/R54 matrix used two fresh MTP1 candidates and two same-image MTP0 controls. Every complete natural-512 workload/canary gate passed and all four target/candidate comparisons were 12/12 exact. MTP0 centers at `33.733520 tok/s`; dynamic MTP stays withheld. The historical exact-depth profile separately measured `46.636241 tok/s` at 32K; [audit](experiments/qwen38-27b-b70/notes/2026-09-01-qwen38-fp8-public-reproduction-audit.md), [R54 result](experiments/qwen38-27b-b70/data/2026-09-01-qwen38-fp8-explicit-deterministic-matrix-r54-result.json), [32K result](experiments/qwen38-27b-b70/notes/2026-08-28-qwen38-fp8-w8a16-mtp1-exact-depth-r33-result.md), [package](packages/qwen38-27b-fp8-tp2-b70/README.md), [standalone repro](repro/qwen38-27b-fp8-vllm-tp2-asrock-b70/README.md) |
...
| **AutoRound INT4 W4A16, fixed-K batch-invariant, vLLM/XPU MTP depth 4, XPU graph capture + draft-only INT4 head** | 2x B70, TP2 | **`112.36 / 112.33 tok/s`** strict pair (graphs alone `91.00 / 91.01`; MTP0 with graphs `49.83 / 49.89`; eager depth 4 `68.55 / 67.79`, MTP0 `34.21 / 35.64`) | R247/R257 2026-09-05/06: graph capture of the verify step removes the all-reduce host waits (+33%), the draft-only INT4 head makes draft passes a quarter of the bytes and lifts acceptance to 3.51 (+23%), 12/12 vs the eager MTP0 oracle throughout. R239 matrix: every depth 0-4 pair 12/12 vs each other and vs the MTP0 oracle on TP2 and TP1; MTP0 identity ladders exact c1-c64 in 6 of 8 runs; speculative depths exact through c16 on TP2. Research-status, not submitted. Supersedes the withdrawn `101.170` margin-on ARK rows. [recipe](repro/qwen38-27b-autoround-int4-b70/README.md), [data](experiments/qwen38-27b-b70/data/2026-09-05-qwen38-int4-r239-matrix-result.json) |

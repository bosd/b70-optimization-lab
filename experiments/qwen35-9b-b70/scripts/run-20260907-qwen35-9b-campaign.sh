...
# STAGES (space list of: strict ladders), LADDER_CONCURRENCY, LADDER_REPEATS, LADDER_MML/MNS/MBT, XPU_DEVICE_MASK.
set -uo pipefail
...
if [[ "$GRAPH" == 1 ]]; then
  ...
  comp='{"cudagraph_mode":"NONE","inductor_compile_config":{"combo_kernels":false,"benchmark_combo_kernel":false,"deterministic":true,"split_reductions":false,"triton.autotune_pointwise":false,"benchmark_epilogue_fusion":false}}'
  eager=1; xgraph=0
fi
...
launch() {
  ...
  local name=qwen35-9b-${RUN}-${label} served=qwen35-9b-fp8-${label} launcher=run-w8a16-mtp0-strict-server.sh spec='{}'

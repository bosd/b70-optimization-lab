...
ap.add_argument('--tp', type=int, default=1); ap.add_argument('--out-attestation'); ap.add_argument('--out-queue'); a = ap.parse_args()
...
att = {
 ...
 'model': {'hf_id': 'RedHatAI/Qwen3.5-9B-FP8-dynamic', 'revision': '790f0576d2d77dd5322aa0603a470bd9e3a3d1f6', 'served_as': 'compressed-tensors FP8 per-channel weights, dynamic activations, publisher MTP head as draft'},
...
payload = {'label': a.label, 'payload': {
 'hfId': 'RedHatAI/Qwen3.5-9B-FP8-dynamic', 'modelRevision': '790f0576d2d77dd5322aa0603a470bd9e3a3d1f6', 'engineName': 'vllm',
 'engineVersion': 'vllm 0.27.2rc1.dev77+gac7509e2b (XPU); vllm-xpu-kernels 1e90ffa672 + lab GDN patches; _xpu_C 271db0d4 (R221 kernel library); R276 image (grouped sync-free GDN spec rows); XPU graph capture FULL_DECODE_ONLY sizes 1-64; qwen3_5_mtp depth %d' % d,
 'backend': 'xpu', 'quantization': 'FP8-dynamic compressed-tensors (per-channel weights, dynamic act.)',
 ...
 'engineFlags': {'apiMode': 'completions', 'apiKvCacheDtype': 'fp16', 'attentionBackend': 'vLLM XPU FlashAttention v2 + lab GDN kernels', 'benchmarkJson': rel, 'promotionAttestation': rel, 'promotionAttestationSha256': sha256(oa),
   ...
   'githubResultPacket': 'https://github.com/steveseguin/b70-optimization-lab/tree/main/packages/qwen35-9b-fp8-b70', 'headlineUse': 'fresh-realistic-suite', 'historyAccelerated': False, 'responseReuse': False, 'prefixCaching': False, 'flashAttn': True, 'gpuLayers': -1, 'concurrency': 1,
   ...
   'localmaxxingSubmissionAllowedUnderCurrentPolicy': True, 'metricWindowGeneratedTokens': 100, 'metricWindowIntervals': 99, 'modelPath': '/path/to/Qwen3.5-9B-FP8-dynamic',
   ...
   'temperature': 0, 'tokenTimingSource': 'openai_stream_token_ids_chunk_timestamp', 'primaryMetricName': 'class_balanced_tok_s_1_100_intervals_after_ttft', 'primaryMetricAccounting': 'generated tokens 1-100 after TTFT, cumulative token-id timing', 'primaryMetricAggregation': 'median-of-prompt-class-medians',
   'commandSnippet': 'MODEL_DIR=/path/to/Qwen3.5-9B-FP8-dynamic VLLM_CACHE_DIR=/path/to/new-cache MTP_DEPTH=%d MAX_MODEL_LEN=1024 MAX_NUM_SEQS=1 repro/qwen35-9b-fp8-b70/scripts/run-qwen35-9b-fp8-server.sh  (ghcr.io/steveseguin/vllm-openai-xpu-b70@sha256:521eb277…)' % d,

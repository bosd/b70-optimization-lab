...
## 05:23–06:02 09-07 — in-server GEMM events on the eager lineage: the 16-warp map is slower in the server's kernel path (map lever closed)

- A285 (control map) and A286 (`moe-m1-w13-n32-w16`), both eager packets derived from A230 on the skips head with the Triton HC glue live and `Q38_MOE_GEMM_EVENT_TIMING=1` / `Q38_LAYER_TIMING_LOG=3` (per-rank medians over the two exact-2K requests, `summarize-q38-subop-timing.py`, `data/20260907-tp4-mtp0-a28{5,6}-eager-hctriton-*-subop-timing-2k.json`): per step the 48 W13 launches sum to **10.90 ms (control) vs 11.41 ms (16 warps)**, the W2 launches 8.59 vs 8.72, the MoE experts kernel 104.3 vs 104.8, the forward 350.4 vs 354.9 — every other sub-op within ±0.4 ms. The lineage hash `86b5b6c7…` held on both.
- So the isolated graph-replay harness (0.211 → 0.151 ms per block) does not represent the server's launches: the server runs the modular Triton experts path with the phase-config override, and under that path 16 warps costs 4.7% more per W13 launch, which is what A282 showed at the step level. The 16-warp map is closed; `configs/moe-m1-w13-n32-w16` stays as the record of the screen. Kernel and tuned-map knobs for this line are screened in the server only (eager event pair for attribution, then a graph packet for the rate), and the offline harness is not used again until its launch path is shown to match the server's.
- A287 (eager events, M=1 base entry at BLOCK_N 32, i.e. the W2 launch at N32 with W13 unchanged, 8 warps): W2 launches **8.59 → 8.94 ms per step** (+4%, on every rank), W13 10.90 → 11.07/10.85/11.01/10.86, forward 350.4 → 354.0; hash held. Negative; A288 (graph rate) cancelled as unnecessary. With 16 warps and BLOCK_N 32 both slower in the server, the certified M=1 entry (BLOCK_N 64, 8 warps, W13 at N32) is confirmed as the server optimum of the tile knobs; the remaining MoE GEMM lever is kernel work on the modular experts launch path itself. Filed: `data/20260907-tp4-mtp0-a287-eager-hctriton-bn32-map-subop-timing-2k.json`.

## 06:30–06:35 09-07 — two more config-free leads checked: the grouped HC-up lever is eager-only; the QSA pre-indexer is unfused on XPU

- A289 (`VLLM_XPU_QWEN4_EXP_HC_GROUPED_UP=1` with the Triton HC glue on the diagnostics head): the workers refuse at construction — `Qwen4Exp grouped HC-up requires fully eager execution` (`amd/low_latency_gemm.py`, `_validate_grouped_up_config`). The lever packs the 97 HC-up 320→10240 projections into one weight bank with a scheduler-bounded grouped GEMM and was built for the eager line; the graph line already captures those launches, so using it here means making the packed-bank method capture-safe (code), not a flag.
- Model-code survey for XPU fallbacks of the HC-glue kind: the XPU QSA indexer (`amd/indexer_qsa.py`) runs the unfused sequence — projection, torch RMSNorm, Triton rope, group compress, key normalization, two cache-row stores, then selection with a stable `argsort` top-k (chosen for determinism over the atomic XPU top-k) — while the NVIDIA path fuses norm, rope, compress and the stores into one Triton kernel (`nvidia/ops/qsa_pre_indexer.py`, gate `_supports_fused_pre_indexer`: neox rope, head_dim 128, rotary_dim 64, one kv head, power-of-two compress ratio — all true for this model). Porting it behind an opt-in flag is the next HC-style lever for the 5.1 ms QSA module; it fuses bf16 intermediates, so it is a new-authority candidate like the glue and would need the same screen (quality profile, fresh pairs, lossless MTP1). Other XPU branches (PLE layer from the NVIDIA tree, stable top-k, split-K stages) are deliberate exactness or memory choices, not fallbacks.

## 10:15–10:25 09-07 — the swapped MoE accumulator layout is bit-identical and has never run on XPU

- `enable_swap_ab()` (`fused_moe/utils.py`) returns true only on `is_device_capability(90)`: the swapped layout exists because Hopper's wgmma forbids `BLOCK_SIZE_M < 64`, and no other platform has ever taken it. On this line every MoE GEMM block carries two or three valid routed rows inside `BLOCK_SIZE_M=16`, which is the shape the swapped layout is for, and the branch already composes with the placement offset table (it reads the same `b_base`) and with split-K.
- Branch `q38-moe-swapab` = 16e2d998 (one commit over the skips head 7751df34): `VLLM_XPU_MOE_SWAP_AB=1` opts in on XPU when the tile shape qualifies. `tools/screen-moe-swap-ab-equality-offline.py` runs the decode-shaped block-FP8 `fused_experts` call with the gate off and on in one process: **bit-identical in all three regimes** (M=1 ep-like 5/10 local hits, M=1 all-local, M=2 ep-like 8/20; zero differing elements). So if it is faster it is an exact lever for both the torch-fallback rows and the Triton-HC rows. A290 screens it in the server (port 19959, HC glue live).
- Queued behind it: A291, the W13 phase tile at `BLOCK_N=16` (four times as many programs over the same K order, so exact by construction; the 16-warp and BLOCK_N-32 screens were the only tile knobs tried in the server so far), and A292, `Q38_DIAG_SKIP=moe,qsa_attn,gdn_attn,hc_mix`, which measures directly what the 6.1 ms unattributed part of the step is. The eager module table (A285) already shows the four modules cover 98.8% of the forward, so that remainder is norms, residual adds, the PLE path, the head and the per-kernel graph launch floor rather than a hidden module.
- Feasibility check for the QSA lever: the fused pre-indexer (`nvidia/ops/qsa_pre_indexer.py`, 508 lines) is plain Triton (`tl.load`/`tl.store`, a 1-D grid over k-work and q-work tiles, no tensor descriptors, no tcgen05, no inline asm), and `k_work_metadata` lives in the shared `QSAForwardMetadata`, so the XPU path already carries the plumbing it needs. The port is an implementation job, not a research question; it fuses the norm and rope into registers, so it lands in the new-authority class and needs the certification the glue got.
- A290 (`VLLM_XPU_MOE_SWAP_AB=1`, head 16e2d998, HC glue live): exact-2K **31.69 / 31.72** against the 31.82 / 31.84 control, and the output moved to `457143c1…`. Both counts close it: no gain, and not exact. The packet differs from the A275 control only by that export (verified by diff), and the branch commit touches only the gate (13 lines, one file), so the change is the layout itself.
- The offline probe had said bit-identical, and it was wrong because it was too small: three calls, 7,680 output elements, zero differences. Re-run over 300 fresh random decode calls, **22 calls differ, 0.20% of elements, by one bf16 step** (max 0.00195). The swapped operand order gives the MMA a different internal reduction order that only rarely changes the last bit, so a handful of calls cannot see it while a 2,048-token generation always will. `tools/screen-moe-swap-ab-equality-offline.py` now loops over `Q38_ITERS` (default 300) and reports the differing-call rate; the server exact-2K hash stays the test of record. The tile-shape screens that did hold their hash over a full generation (A282 warps, A287 BLOCK_N) are therefore the ones with real evidence behind them.

## 10:52–11:07 09-07 — the step floor is 3.9 ms: the unattributed remainder was mostly under-measurement, not launch overhead

- A291 never reached the GPUs: the overlay's own phase-config validator admits `BLOCK_SIZE_N` in {32, 64, 128, 256}, so the 16-column W13 tile was refused at engine construction (`W1_CONFIG.BLOCK_SIZE_N must be one of [32, 64, 128, 256]`). The kernel supports 16; branch `q38-moe-bn16` = 7929f43d admits it for the screen and A294 retries.
- A292 (`Q38_DIAG_SKIP=moe,qsa_attn,gdn_attn,hc_mix`, HC glue live): **255.68 / 254.53 tok/s, a 3.91 ms step**, against the 31.4 ms control. So the four modules jointly account for **27.5 ms**, while the individual skips sum to 25.3 (MoE 15.1, QSA 5.1, HC 2.7, GDN 2.4). The 6.1 ms I could not attribute is therefore **3.9 ms of genuine non-module floor** (embeddings, the PLE gather over UVA, the per-layer norms and residual adds, the head, sampling, and every remaining launch) plus **2.2 ms that the single-module skips under-report**, because removing one module also removes shared per-layer overhead that the others still pay.
- A292 returned the same output hash as A281 (`6e84bd08…`), which is correct rather than suspicious: skipping `hc_mix` already cuts every block's output out of the residual streams, so the attention and MoE modules cannot influence the tokens once it is off. Their skips change the timing, not the text, which is why A281's 2.7 ms is a timing measure and its hash carries no exactness meaning.
- Consequence for the ladder: the floor is 12.5% of the step, so it is not the thing to attack, and reducing kernel count is not the headline lever after all. Even a free MoE, QSA, GDN and HC would leave 3.9 ms (256 tok/s). The modules stay the target, and MoE at 15.1 ms remains more than half of the addressable 27.5. Filed: `data/20260907-tp4-mtp0-a292-hctriton-all-modules-off-floor-exact-depth-2k-r{1,2}.json`.

## 11:12–11:30 09-07 — the QSA indexer's cost is compression and projection, not the argsort

- A293 (eager pass on `q38-qsa-subop` = 93013c52, the indexer's single timer split into three; hash `86b5b6c7…` held): per eager step on TP0, `qsa_idx_compress` **9.97 ms**, `qsa_idx_proj` **8.07 ms**, `qsa_idx_select` **4.74 ms**, against the undifferentiated `qsa_index` 22.15 in A285 (23.90 with the extra timers). The stable `argsort` top-k that the XPU path uses for determinism is therefore only a fifth of the indexer, and replacing it is worth far less than it looked.
- That redirects the QSA lever onto exactly what the NVIDIA fused pre-indexer covers: it collapses the projection with its norm and rope, the group compression, and the cache stores into one kernel, which is the 18.0 ms of the 23.9 that the split just attributed. The `select` stays separate on both paths. So the port is the right target, and larger than assumed, but it fuses bf16 intermediates and so remains a new-authority change needing the certification the glue got.
- Two queue faults cost launches but no GPU time and no data. A290 and A293 were refused because a diagnostics branch created with `git worktree add -b` stays checked out in that worktree, so the chain cannot check it out in the main clone; all scratchpad worktrees are now removed and the trap is in memory. A294 then fired early because its sequencer keyed on a "seq293 done" line that A293's *failed* attempt had left behind, and the launcher's own guard refused it ("a vLLM engine is still running"). The replacement sequencer waits on the running attempt's `.rc` file and a confirmed idle engine instead of a log line. Filed: `data/20260907-tp4-mtp0-a293-eager-qsa-subop-split-2k.json`.

## 11:31–11:48 09-07 — the 16-column W13 tile is exact and slower, which says the MoE GEMM is not program-starved
...
## 12:07–12:15 09-07 — the MoE GEMM is neither bandwidth- nor FLOP-bound: it is a tile GEMM doing a batch-of-one gather, and exact levers are exhausted

- Weight-read bandwidth probe on card 0 (`tools/probe-moe-weight-read-bandwidth-offline.py`, 300 MiB, identical bytes and grid, only the address mapping differing): the kernel's own strided tile pattern reads at **566 GB/s**, two K-blocks in flight at 416, and a pre-tiled contiguous copy at 416. So the segmented 128-byte reads are not a penalty at all, a load-time repack would buy nothing, and the 472 MB a step moves would take 0.83 ms at the achievable rate against the 7.7 ms the K loop actually spends. The layout hypothesis from A294 is dead. Log: `data/20260907-moe-weight-read-bandwidth-probe-card0.log`.
- Grid probe: at M=1 with top_k 10, the block aligner emits **one 16-row block per routed slot's expert, all ten of them**, so the launch carries 200 programs for W13 and 400 for W2 in the flat config (400 and 400 in the server's phase config). Under EP4 only two to five of those experts are local; the rest enter the kernel and return through the zero-writing path. Of the blocks that do work, one or two rows of sixteen are real, because `tl.dot` cannot go below a 16-row tile.
- So the cost is neither memory nor arithmetic. It is a tile GEMM asked to do a batch-of-one gather: sixteen-fold row oversubscription, most blocks empty, and twenty dependent K iterations per program whose latency the machine cannot hide at this occupancy. That reconciles every result of the last hours: more programs hurt (A294), fewer and larger hurt (A295), more warps hurt (A282), prefetching hurt, and split-K, which is the only change that shortens the dependent chain, is the only one that ever helped (+2.4%) and it is not exact at 4K.
- **The MoE GEMM is therefore at a local optimum for exact changes**, and the remaining moves are all inexact: split-K, or a GEMV-shaped kernel for M=1 that drops the row oversubscription entirely and would reduce the arithmetic sixteenfold. Both change the K reduction order, so both are new-authority work rather than continuations of the certified line. Nothing further should be spent tuning this kernel's tiles.

## 12:13–13:50 09-07 — the reference fused QSA pre-indexer: +21.9% over the certified line, and the 2K "lossless" match was a coincidence

- The XPU indexer runs the qk projection, the q norm and rope, the group compression, the compressed-key norm and rope and the cache stores as separate kernels; the model's reference path fuses them (`nvidia/ops/qsa_pre_indexer.py`). A293 attributed 18.0 of the indexer's 23.9 ms per eager step to exactly those phases, so the kernel was ported to the XPU path behind `VLLM_XPU_QSA_FUSED_INDEXER=1` (branches `q38-qsa-fused` 2a372e86 on the certified HC MTP0 head, `q38-qsa-fused-mtp1` 6d872457 on the MTP1 one).
- A297 and A299 (two fresh servers, glue and fused indexer both on): exact-2K **33.49 / 33.48** and **33.48 / 33.51**, with the ids and text of the **certified torch-fallback authority** (`afffd211…` / `e39e32c3…`). That looked like a lossless line 21.9% faster than the certified 27.48.
- The controls said otherwise. A298 (fused indexer alone, glue off) gives a **third** stream `2ac12a57…` at 28.04 / 28.01. So each change alone moves the output and only the pair lands back on the certified one, which is the signature of a single near-tie early in the generation whose two perturbations cancel, not of a numerically identical computation. Matching token ids never implied matching logits.
- A300 settled it. The frozen client, keeping the certified assertions unmodified, passed recovery, quality, the short suite and both exact-2K repeats, then **failed at exact-4K: `1d833e5f…` against `c6193cc6…`**, deterministically on both repeats, at 33.45 / 33.44. The 2K agreement is a coincidence at that depth; the line is **not lossless**.
- What it is instead is a fast line with an intact quality profile: all **7/7 exact cases byte-identical** to the certified battery, 16/16 repeats one hash, the long-context needle identical, and both depths deterministic. Against the published Triton-HC row (A269: 32.58 / 32.56 at 2K, 32.58 / 32.62 at 4K) it is **+2.7% at both depths**, and it carries the same justification: both the hyper-connection glue and this pre-indexer are the model's own reference kernels that the XPU port had replaced. Certifying it means a *second* authority change in one day, superseding a lineage published this morning, so the suite number (A301) comes first and the decision is stated rather than assumed.
- Two process faults, both caught by the guards: editing the verifier after generating the packets drifted its pin and the client refused it at the first gate (rule now in memory: freeze the verifier before a packet batch); and on the MTP1 head the fused branch had been placed ahead of the row-serial verify path that keeps MTP1 lossless, which would have silently replaced it — fixed before it ran. Filed: `data/20260907-tp4-mtp0-a29{7,8,9}-*`, `data/20260907-tp4-mtp0-a300-qsafused-hc-*`.

## 14:13–15:25 09-07 — the reference-restoration lineage certified and published

- A304 (MTP0 battery, second server, client pinned to this lineage's authorities): every gate passed, exact-2K 33.32 / 33.45 and exact-4K 33.47 / 33.41, quality 6/7 with the inherited miss, 16/16 repeats, exact needle. A305 (MTP1 battery): every gate passed, exact-2K **38.98 / 38.97**, exact-4K **39.30 / 39.30**, pins equal to the MTP0 line's, so **MTP1 stays lossless within the lineage**. Suites: MTP0 **33.797067** (A301), MTP1 **37.825654** (A306), both gates passed with cached_tokens 0.
- LocalMaxxing approved both rows: MTP0 `cmtrmp37v001bps01a7fi46nf`, MTP1 `cmtrmp3mj001fps01thcathd0`. Attestations carry all six gates true and state the authority change in the note; the row notes say plainly that the difference is at depth, not in quality.
- Published: overlay series `patches/qwen38-flash-next-fp8-b70/vllm-qsafused-mtp1-6d872457/` (two patches over the Triton-HC head, bundle, manifest, verifier that re-creates tree 3e8dfc59), replay guide `repro/qwen38-flash-next-fp8-tp4-mtp1-qsafused-b70-38tps-20260907/` (identity, pins, evidence manifests for A305 and A306, identity check that now verifies all three series), candidate package, guide and package catalogs, family manifest with the hero moved to the new MTP1 row, coverage registry, results packet section, recipes and repro index rows, landing page, model and family pages, share cards. All 81 tests pass; guide catalog valid at 37 guides.
- The 31.93 and 37.05 rows keep their guides and their approvals; nothing was lowered or overwritten. The published ladder is now: certified placement 27.64 / 31.93 (bit-identical to the original authorities), Triton-HC 32.90 / 37.05 (first restoration), reference-restoration 33.80 / 37.83 (both).

## 15:35–16:05 09-07 — a GEMV-shaped decode MoE kernel is not faster, which closes the MoE avenue

- The tile kernel's waste is real (`tl.dot` cannot go below a 16-row tile, and at decode one row of sixteen is valid), so a GEMV doing one sixteenth of the arithmetic was the largest remaining idea. `tools/probe-moe-gemv-decode-offline.py` writes one, checks it against a dequantized fp32 matmul, and times it under graph replay for comparability.
- Result: **95.0 µs per layer for one GEMM at prefetch 1, 95.1 at 2, 95.2 at 4**, against the tile kernel's 211 µs per layer for *both* GEMMs plus the entire surround. The GEMV is no faster, and the prefetch depth is completely inert. The kernel also emits a handful of non-finite values in most trials, an unfixed bug, but the timing settles the question without it.
- Both halves of the earlier diagnosis are now falsified. It is not systolic-array occupancy: a kernel using one sixteenth of the arithmetic on the vector ALUs costs the same. And it is not the dependent K chain: prefetching two or four blocks ahead changes nothing here, just as it did not in the tile kernel, and the register-pressure explanation for that failure is dead too. What remains is per-iteration behaviour in the memory path that neither more parallelism, fewer flops, nor deeper pipelining touches.
- **The MoE GEMM is closed.** Exact tile levers were exhausted in the server (A282, A287, A294, A295, all slower), split-K is fast but not exact at 4K, in-kernel prefetch is slower, the swapped accumulator layout is slower and not exact, a load-time repack is pointless at 566 GB/s of measured read bandwidth, and a GEMV is no faster. Moving this kernel further needs a different stack, not another Triton variant. Filed: `data/20260907-moe-gemv-decode-probe-card0.log`.

## 16:29–16:50 09-07 — letting the fused pre-indexer take multi-row verify steps changes nothing

- The reference path has no row-serial branch: it runs the fused pre-indexer on every step, and `k_work_metadata` schedules the compression work per group so rows sharing a group cannot race for a compressed slot. On XPU the fused branch was placed after `_forward_serial_rows` deliberately, which leaves the MTP1 verify step on the serial path, so `VLLM_XPU_QSA_FUSED_INDEXER_MULTIROW=1` (branch `q38-qsa-fused-multirow` = be43a908) was screened to see what that conservatism costs.
- A308: exact-2K **38.97** on the warm repeat against the certified A305's 38.98 / 38.97, hash `afffd211…` held. **No gain, and no change in output.** The serial path was not costing anything measurable, and the fused kernel reproduces its selections exactly on multi-row steps, which is a useful confirmation that the reference kernel resolves the compression-group race the same way. The gate stays off; the certified heads keep the conservative placement, which is now a documented choice rather than an untested caution.
- State of the ladder after today. The step at the fastest line is roughly 25 ms at MTP1, of which the MoE block is about 15 and closed to Triton variants, the floor is 3.9, and attention plus the hyper-connection glue account for most of the rest. The remaining named levers are small: MoE surround fusion (3.2 ms, and fusing it would change intermediate rounding), a capture-safe rebuild of the grouped hyper-connection GEMM (built for eager, refuses under the graph), and an MTP2 re-screen. A further step change would need a different MoE kernel stack rather than another Triton variant, which is a project rather than a screen. Filed: `data/20260907-tp4-mtp1-a308-fused-multirow-exact-depth-2k-r{1,2}.json`.

## 18:01–18:21 09-07 — MTP2 on the reference-restoration lineage: lossless within it, and still slower than MTP1 (coverage closed)

- A309 (head 6d872457, two speculative tokens, both reference kernels live): exact-2K r1 24.54 (cold), **r2 35.12 tok/s**, hash `afffd211…` held, so two tokens stay lossless within the lineage. Against one token's 38.97 (A305) it is 3.9 tok/s slower, the same verdict as on the placement identity (A228: 30.94 against 33.2) and on the Triton-HC head (A274: 33.03 against 36.4). The third token's verify step costs more than the extra accepted token returns, and neither restored kernel changes that balance.
- MTP0/1/2 coverage on the fastest lineage is now closed with measurements at every depth: **33.45 / 38.97 / 35.12** exact-2K, all carrying the same pins. One speculative token remains the right setting, which is what the published rows use. Filed: `data/20260907-tp4-mtp2-a309-qsafused-exact-depth-2k-r{1,2}.json`.
- Recovery note for the record: the evidence disk re-enumerated at 17:56:47 and killed the first two attempts at this screen. The launcher's mount guard refused to run with `/mnt/usb-models` unmounted, which would otherwise have read the model and written results to the root filesystem, and after the remount it refused to reuse the killed attempt's run directory. Both refusals were correct; the run directories were archived rather than deleted.

## 19:25 09-07 — the grouped hyper-connection GEMM is blocked by the pinned kernel stage (closed without a run)

- `enable_qwen4_exp_low_latency_gemm` packs the 97 HC-up 320→10240 projections into one weight bank behind a grouped GEMM, and A289 had shown it refuses under the decode graph. Reading the gate shows the eager check is only one clause of a contract that also demands the older PLE-only 12.0 GiB UVA identity (ours is PLE + embeddings at 12.25), the exact one-sequence scheduler contract, and `torch.ops._xpu_C.cutlass_grouped_gemm_interface`.
- That last one settles it: the pinned stage `2f829747` exposes **no grouped or gemm ops in `_xpu_C` at all**, so the method could not run even with every policy clause relaxed. Providing it means a different kernel-stage build, and the stage is pinned by 18 file hashes in every published guide and identity, so that is an identity change rather than a flag. Closed, at the cost of one import check rather than a 25-minute run.
- With this, all three remaining named levers are resolved: MoE surround fusion is inside a block that is closed to Triton variants and would change intermediate rounding; grouped HC-up needs a runtime op the pinned stage lacks; and two speculative tokens are measured and slower (A309). The cheap, exact ladder is exhausted, and the next step change is a different MoE kernel stack, which is a project to scope rather than a screen to queue.

## 19:27–19:47 09-07 — what losslessness under speculation costs: 30% (A310, a bound, not a candidate)

- The lossless-MTP1 line owes its exactness to three batch-invariance selectors (`VLLM_XPU_GDN_SERIAL_SPEC_DECODE`, `VLLM_XPU_ROWWISE_ALLREDUCE_MAX_ROWS`, `VLLM_XPU_ROWWISE_HC_NORM_MAX_ROWS`), which force row-serial computation during speculative steps so the multi-row math reproduces the single-row result. A231 had measured their cost on the placement identity (33.2 → 38.63) but nobody had measured it on the current line.
- A310 (head 6d872457, both reference kernels live, all three selectors off): exact-2K r1 30.43 (cold), **r2 50.62 tok/s**, output `347cd364…` — changed, by construction. Against the certified 38.97 that is **+29.9%, or 5.9 ms of a 25.7 ms step**.
- This is a bound, not a candidate, and it is filed as one. The selectors are the mechanism by which one speculative token is lossless; turning them off does not produce a faster version of the same answers, it produces different answers. What the number does is price the guarantee: on this hardware and this line, exact speculation costs about 30% of decode throughput. Anyone weighing an explicitly non-deterministic serving mode now has the figure, measured on the fastest certified identity rather than extrapolated. Filed: `data/20260907-tp4-mtp1-a310-selectors-off-bound-exact-depth-2k-r{1,2}.json`.

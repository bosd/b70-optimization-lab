...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp1-4352-ple-only-a120", "tp4-mtp1-4352-ple-only-a251")
        seg = seg.replace("attempt120", "attempt251").replace("19792", "19896")
        seg = seg.replace("ATTEMPT=120", "ATTEMPT=251").replace("a120", "a251").replace("A120", "A251")
        return seg
    parts=[]; last=0
...
def main():
    ...
    launcher = patch_a251_launcher(launcher)
    ...
    env = os.environ.copy(); env["Q38_A251_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a251-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a251" in derived
    ...
    client = patch_a251_client(client)
    ...
    supervisor = patch_a251_supervisor(supervisor)
    ...
    out_names = ("launch-tp4-mtp1-4352-ple-only-a251-fullgraphdet-w13n32.sh", "run-tp4-mtp1-4352-ple-only-a251-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp1-4352-ple-only-a251-fullgraphdet-w13n32.sh", "run-q38-a251-host-controlled.sh")

...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a280-fullgraphdet-w13n32.sh"
expected_wrapper=d0b6f89747cd0cf53954db166a5f2a2964e4d49d5bd2ab8306c58276bb485f38
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a280-fullgraphdet-w13n32-client.sh"
expected_client=45bff35875584e9c4bf49042c0f40319460e115e736ccd140b5e2cbaae4684a4
state=/tmp/q38-mtp0-ple-only-a280
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt280
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt280
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt280-compile
rpc_dir=/tmp/q38-ple2k-a280-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt280-supervisor
port=19949
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A280 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A280 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A280_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A280_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A280_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A280 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A280 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

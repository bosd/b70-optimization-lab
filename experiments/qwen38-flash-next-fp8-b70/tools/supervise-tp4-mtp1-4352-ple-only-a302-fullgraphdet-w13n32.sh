...
wrapper="${script_dir}/launch-tp4-mtp1-4352-ple-only-a302-fullgraphdet-w13n32.sh"
expected_wrapper=c3feb5e5102fc6bf64929e3d6a037f211028124a93bf5fac56cfc23e0959221d
client="${script_dir}/run-tp4-mtp1-4352-ple-only-a302-fullgraphdet-w13n32-client.sh"
expected_client=2d00e9b7f91a948e21a330996910380731fe7b395a33f147bbcd3fe5ec2dede4
state=/tmp/q38-mtp1-ple-only-a302
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt302
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt302
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt302-compile
rpc_dir=/tmp/q38-ple2k-a302-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt302-supervisor
port=19971
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A302 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A302 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A302_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A302_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A302_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A302 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A302 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

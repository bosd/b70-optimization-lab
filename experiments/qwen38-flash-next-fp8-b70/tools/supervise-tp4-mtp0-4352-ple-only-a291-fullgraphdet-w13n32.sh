...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a291-fullgraphdet-w13n32.sh"
expected_wrapper=87640b86ae0617f1ae77ab53d95b6dbc670c8ecb27b99ef7ae291866c7aa9f36
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a291-fullgraphdet-w13n32-client.sh"
expected_client=8e8efe8e07ed8823a431f775befe6843c4c123ad8e54a471d3b363fa518c502c
state=/tmp/q38-mtp0-ple-only-a291
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt291
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt291
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt291-compile
rpc_dir=/tmp/q38-ple2k-a291-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt291-supervisor
port=19960
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A291 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A291 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A291_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A291_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A291_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A291 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A291 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

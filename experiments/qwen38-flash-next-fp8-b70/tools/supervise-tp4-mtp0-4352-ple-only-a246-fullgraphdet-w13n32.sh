...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a246-fullgraphdet-w13n32.sh"
expected_wrapper=37423f33ab669ce18ceb8944e63ee8ce9582efae31c26489a654a324e83eb4ea
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a246-fullgraphdet-w13n32-client.sh"
expected_client=4231a6d2f464b9e5fe4fb49602861e4f51d21066efd61392dc15770c9e7b6d0d
state=/tmp/q38-mtp0-ple-only-a246
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt246
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt246
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt246-compile
rpc_dir=/tmp/q38-ple2k-a246-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt246-supervisor
port=19916
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A246 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A246 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A246_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A246_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A246_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A246 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A246 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

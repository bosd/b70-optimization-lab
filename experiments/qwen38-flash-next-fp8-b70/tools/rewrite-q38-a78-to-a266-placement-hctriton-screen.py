...
VALIDATE_ONLY = os.environ.get("Q38_A266_REWRITE_VALIDATE_ONLY") == "1"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp0-4352-ple-only-a78", "tp4-mtp0-4352-ple-only-a266")
        seg = seg.replace("attempt78", "attempt266").replace("19750", "19936")
        seg = seg.replace("ATTEMPT=78", "ATTEMPT=266").replace("a78", "a266").replace("A78", "A266")
        return seg
    parts=[]; last=0
...
def main():
    ...
    env = os.environ.copy(); env["Q38_A266_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a266-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a266" in derived
    ...
    out_names = ("launch-tp4-mtp0-4352-ple-only-a266-fullgraphdet-w13n32.sh", "run-tp4-mtp0-4352-ple-only-a266-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp0-4352-ple-only-a266-fullgraphdet-w13n32.sh", "run-q38-a266-host-controlled.sh")

...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a304-fullgraphdet-w13n32.sh"
expected_wrapper=3b8006ffa6ec7546c2f084039e53a79fbc37a1c35fdfb1365752eeafe090533a
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a304-fullgraphdet-w13n32-client.sh"
expected_client=54c1f2ecd12f8e6bcc0af7244225a0af4e2dfa4a808a4672dde03bec20e1cee1
state=/tmp/q38-mtp0-ple-only-a304
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt304
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt304
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt304-compile
rpc_dir=/tmp/q38-ple2k-a304-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt304-supervisor
port=19973
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A304 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A304 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A304_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A304_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A304_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A304 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A304 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

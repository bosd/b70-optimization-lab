...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a292-fullgraphdet-w13n32.sh"
expected_wrapper=420c7b010f3e0a91bbcd74a2558e7920b8b202fd61074287ce9aa3c5a8087ebb
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a292-fullgraphdet-w13n32-client.sh"
expected_client=d33c5366b522a17ee7726746851c6f5eb2f51c646d72ed189a228103771ad47a
state=/tmp/q38-mtp0-ple-only-a292
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt292
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt292
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt292-compile
rpc_dir=/tmp/q38-ple2k-a292-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt292-supervisor
port=19961
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A292 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A292 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A292_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A292_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A292_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A292 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A292 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a294-fullgraphdet-w13n32.sh"
expected_wrapper=7c98b34a2565de5cd8ac04fb01d9814eb841350a16735714d5d8ad51307d69cd
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a294-fullgraphdet-w13n32-client.sh"
expected_client=cea9fdd90fae7052a32c563ed7484a9eef53b284dfdb600d388088e7a85f84a2
state=/tmp/q38-mtp0-ple-only-a294
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt294
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt294
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt294-compile
rpc_dir=/tmp/q38-ple2k-a294-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt294-supervisor
port=19963
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A294 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A294 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A294_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A294_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A294_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A294 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A294 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

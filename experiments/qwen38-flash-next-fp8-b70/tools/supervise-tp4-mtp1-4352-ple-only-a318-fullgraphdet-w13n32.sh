...
wrapper="${script_dir}/launch-tp4-mtp1-4352-ple-only-a318-fullgraphdet-w13n32.sh"
expected_wrapper=2a8c05fd0679eee2fb837e466091dfcc19febc1475929350ff3d275174c121ec
client="${script_dir}/run-tp4-mtp1-4352-ple-only-a318-fullgraphdet-w13n32-client.sh"
expected_client=6b14ddf88ac1b7dd3dd3654d9dc570d270daca140031dc96d429cf854bc03674
state=/tmp/q38-mtp1-ple-only-a318
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt318
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt318
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt318-compile
rpc_dir=/tmp/q38-ple2k-a318-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt318-supervisor
port=19988
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A318 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A318 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A318_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A318_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A318_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A318 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A318 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

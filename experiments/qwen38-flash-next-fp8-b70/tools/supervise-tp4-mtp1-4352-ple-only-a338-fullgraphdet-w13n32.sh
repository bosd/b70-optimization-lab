...
wrapper="${script_dir}/launch-tp4-mtp1-4352-ple-only-a338-fullgraphdet-w13n32.sh"
expected_wrapper=29cf042e8fd431269efdef4e85a753b27301c319f704c2a1419750e9dca938b5
client="${script_dir}/run-tp4-mtp1-4352-ple-only-a338-fullgraphdet-w13n32-client.sh"
expected_client=1d75682819ec1e4072a412ea5cc72c1c75876abadb8c108d63c5d3ea83de805d
state=/tmp/q38-mtp1-ple-only-a338
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt338
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt338
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt338-compile
rpc_dir=/tmp/q38-ple2k-a338-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt338-supervisor
port=19951
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A338 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A338 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A338_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A338_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A338_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A338 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A338 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

#!/usr/bin/env python3
"""Create the A284 packet from frozen A78: graph MTP0 exact-2K timing screen with PLE + embeddings offloaded at budget 12.25 (hot experts resident) and 3.5 GiB/rank of never-hit experts host-placed at load time on q38-placement-v5 c4f921f5."""
...
VALIDATE_ONLY = os.environ.get("Q38_A284_REWRITE_VALIDATE_ONLY") == "1"
...
NEW_HEAD = "7751df34a3ca4865b13f27cee889f160cf62c21d"  # q38-placement-v5: table kernel + load-time never-hit expert placement
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp0-4352-ple-only-a78", "tp4-mtp0-4352-ple-only-a284")
        seg = seg.replace("attempt78", "attempt284").replace("19750", "19953")
        seg = seg.replace("ATTEMPT=78", "ATTEMPT=284").replace("a78", "a284").replace("A78", "A284")
        return seg
    parts=[]; last=0
...
def main():
    ...
    launcher = replace_once(launcher, "export KV_CACHE_MEMORY_BYTES=134217728\n", "export KV_CACHE_MEMORY_BYTES=134217728\nexport Q38_STEP_TIMING_LOG=10\nexport Q38_LAYER_TIMING_LOG=1\nexport Q38_MOE_GEMM_EVENT_TIMING=1\n")  # server-side GEMM event timing (wrapper-level Q38_* survive the env stripping)
    assert launcher.count("moe-m1-w13-n32") == 6 and launcher.count("a8f1f8982e3e1af80ff31b9e0a00afaacf1af1b3c401585109b4d60d3c8267be") == 3, (launcher.count("moe-m1-w13-n32"), launcher.count("a8f1f8982e3e1af80ff31b9e0a00afaacf1af1b3c401585109b4d60d3c8267be"))
    launcher = launcher.replace("moe-m1-w13-n32", "moe-m1-w13-n32-w16").replace("a8f1f8982e3e1af80ff31b9e0a00afaacf1af1b3c401585109b4d60d3c8267be", "39a1cce223f6911ca96d0769c1cabf6d15f5b82eb399b6748048d077917f450d")  # M=1 entry at 16 warps (graph-replay sweep: 0.151 vs 0.211 ms per MoE block)
    launcher = replace_once(launcher, '\'."1".num_warps\' "$tuned_config_map")" == 8 &&', '\'."1".num_warps\' "$tuned_config_map")" == 16 &&')  # the shape guard pins the warp count
    ...
    env = os.environ.copy(); env["Q38_A284_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a284-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a284" in derived
    _fm.check_derived(derived)
    assert f'expected_vllm_head="{NEW_HEAD}"' in derived and OLD_HEAD not in derived
    assert "export Q38_MEM_NOTE=1\n" in launcher and "export Q38_EXPERT_HOST_PLACEMENT=" in launcher and "Q38_DIAG_ROUTE" not in launcher
    ...
    out_names = ("launch-tp4-mtp0-4352-ple-only-a284-fullgraphdet-w13n32.sh", "run-tp4-mtp0-4352-ple-only-a284-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp0-4352-ple-only-a284-fullgraphdet-w13n32.sh", "run-q38-a284-host-controlled.sh")

...
supervisor="${repo}/experiments/qwen38-flash-next-fp8-b70/tools/supervise-tp4-mtp0-4352-ple-only-a327-fullgraphdet-w13n128.sh"
state=/tmp/q38-mtp0-ple-only-a327
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt327
...
torchinductor_cache=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt327-compile/torchinductor
...
[[ "$supervisor_command" == *"supervise-tp4-mtp0-4352-ple-only-a327-fullgraphdet-w13n128.sh"* ]] || {
...
grep -zFxq 'VLLM_TUNED_CONFIG_FOLDER=/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/configs/moe-m1-w13-n128' "/proc/${server_pid}/environ" || {
  printf 'FAIL: live server lacks the exact A327 tuned M1 map folder\n' >&2
  exit 1
}
...
[[ "$(sha256sum '/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/configs/moe-m1-w13-n128/E=128,N=640,device_name=Intel(R)_Arc(TM)_Pro_B70_Graphics,dtype=fp8_w8a8,block_shape=[128,128].json' | cut -d' ' -f1)" == 8b67485c1ad13ea3b1b0ebdb06a6a2d0e18bd4b9058f44a2378c4a47d0f41315 ]] || {
  printf 'FAIL: A327 tuned M1 map drifted before client work\n' >&2
  exit 1
}
[[ "$(sha256sum "${repo}/experiments/qwen38-flash-next-fp8-b70/tools/verify-moe-m1-w13-n128-selection.py" | cut -d' ' -f1)" == 95d01be9f4877784c3270580a6a93f4220d62e718cb3a970731d949194e63a02 ]] || {
  printf 'FAIL: W13-N128 selection verifier drifted\n' >&2
  exit 1
}
...
env PYTHONPATH=/mnt/usb-models/qwen38-build/runtime-core-moe-negidguard-b70:/home/steve/src/vllm-current-main \
  ...
  VLLM_TUNED_CONFIG_FOLDER=/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/configs/moe-m1-w13-n128 \
  "$python" "${repo}/experiments/qwen38-flash-next-fp8-b70/tools/verify-moe-m1-w13-n128-selection.py" \
  ...
  --candidate-config-file '/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/configs/moe-m1-w13-n128/E=128,N=640,device_name=Intel(R)_Arc(TM)_Pro_B70_Graphics,dtype=fp8_w8a8,block_shape=[128,128].json' \
  ...
  --output "${run_dir}/moe-m1-w13-n128-selection-receipt.json" || {
  printf 'FAIL: official W13-N128 resolver receipt failed\n' >&2
  exit 1
}
jq -e '.status == "pass" and .selected_batch_key == 1 and
  .m1.w13.BLOCK_SIZE_N == 128 and .m1.w13.num_warps == 8 and
  .m1.w2.BLOCK_SIZE_N == 64 and .m1.w2.num_warps == 8 and
  .config.candidate_sha256 == "8b67485c1ad13ea3b1b0ebdb06a6a2d0e18bd4b9058f44a2378c4a47d0f41315" and
  .preservation.all_integer_m_2_through_512_match_retained_map == true' \
  "${run_dir}/moe-m1-w13-n128-selection-receipt.json" >/dev/null || {
  printf 'FAIL: W13-N128 resolver receipt did not prove key 1 W13-N128 / W2-N64\n' >&2
  exit 1
}
...
[[ "$(git -C /home/steve/src/vllm-current-main rev-parse HEAD)" == 2a372e860e273273357cb7437ac7de1694304f9f ]] || {
...
[[ "$server_command" == *"vllm serve /mnt/usb-models/llm-models/Qwen3.8-Flash-Next-FP8"* && \
   "$server_command" == *"--port 19997"* && "$server_command" == *"--max-model-len 4352"* ]] || {
...
[[ "$server_command" != *"--enforce-eager"* && "$server_command" == *"--cudagraph-metrics"* && \
  ...
  printf 'FAIL: frozen A327 graph command identity mismatch\n' >&2
  exit 1
}
for receipt in \
  'vllm_head=2a372e860e273273357cb7437ac7de1694304f9f' \
  ...
  'tuned_config_folder=moe-m1-w13-n128' 'tuned_config_map_sha256=8b67485c1ad13ea3b1b0ebdb06a6a2d0e18bd4b9058f44a2378c4a47d0f41315' \
...
PY
journal_start=$(cat "/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt327-supervisor/journal-start-epoch.txt")
...
request = urllib.request.Request(
    ...
    headers={"Content-Type": "application/json", "X-Request-Id": "q38-ple-only-a327-recovery-canary"},
    method="POST")
...
timeout --signal=TERM --kill-after=10s 1200s "$python" "$quality" \
  ...
  --seed 20260609 --repeat-runs 16 --request-id-prefix q38-ple-only-a327 \
...
assert depth4k_hashes == ['1d833e5f463366223a669aa15495840d1337b173e675a9ea04f00a5ae339d5cc'] * 2
summary = {
    "schema_version": 1,
    "status": "passed",
    "identity": {
        ...
        "ccl_sycl_allreduce_ll": "twoshots",
        "tuned_config_folder": "moe-m1-w13-n128",
        "tuned_config_map_sha256": "8b67485c1ad13ea3b1b0ebdb06a6a2d0e18bd4b9058f44a2378c4a47d0f41315",

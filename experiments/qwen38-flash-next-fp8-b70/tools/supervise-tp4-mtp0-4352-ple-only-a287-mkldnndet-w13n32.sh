...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a287-mkldnndet-w13n32.sh"
expected_wrapper=7e6084c79a3aa943ef73ff880d5dfbd6695c9ebc387d7ddc724fc361bdfae413
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a287-mkldnndet-w13n32-client.sh"
expected_client=46b36c0285808f333734cc5a48577a4fb3f5b61667fbe80a90758cafffde9d3c
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-mkldnndet-mtp0-4352-ple-only-r1-attempt287
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-mkldnndet-mtp0-4352-ple-only-r1-attempt287-compile
rpc_dir=/tmp/q38-ple2k-a287-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-mkldnndet-mtp0-4352-ple-only-r1-attempt287-supervisor
port=19956
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A287 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A287 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A287_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A287_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A287_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A287 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A287 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

#!/usr/bin/env python3
"""Create the A288 packet from frozen A78: graph MTP0 exact-2K timing screen with PLE + embeddings offloaded at budget 12.25 (hot experts resident) and 3.5 GiB/rank of never-hit experts host-placed at load time on q38-placement-v5 c4f921f5."""
...
VALIDATE_ONLY = os.environ.get("Q38_A288_REWRITE_VALIDATE_ONLY") == "1"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp0-4352-ple-only-a78", "tp4-mtp0-4352-ple-only-a288")
        seg = seg.replace("attempt78", "attempt288").replace("19750", "19957")
        seg = seg.replace("ATTEMPT=78", "ATTEMPT=288").replace("a78", "a288").replace("A78", "A288")
        return seg
    parts=[]; last=0
...
def main():
    ...
    launcher = launcher.replace("moe-m1-w13-n32", "moe-m1-w13-n32-bn32").replace("a8f1f8982e3e1af80ff31b9e0a00afaacf1af1b3c401585109b4d60d3c8267be", "efb3b0fe65d25412eb6a13babf19306139d729818861ebe3f02b567b80681aca")  # M=1 entry at 16 warps (graph-replay sweep: 0.151 vs 0.211 ms per MoE block)
    ...
    env = os.environ.copy(); env["Q38_A288_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a288-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a288" in derived
    ...
    out_names = ("launch-tp4-mtp0-4352-ple-only-a288-fullgraphdet-w13n32.sh", "run-tp4-mtp0-4352-ple-only-a288-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp0-4352-ple-only-a288-fullgraphdet-w13n32.sh", "run-q38-a288-host-controlled.sh")

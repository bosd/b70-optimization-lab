...
derived=/tmp/q38-ple2k-a278-base.sh
expected_base=d5ccc4d52220f7ef46f19202436edf56e0c40f125b1b807c84125df18093b5c1
expected_derived=90b3a22e166cca110956056241227b88ce4fda39cca195282c52e0a3acf44281
...
[[ "$(sha256sum "$tuned_config_map" | cut -d' ' -f1)" == a8f1f8982e3e1af80ff31b9e0a00afaacf1af1b3c401585109b4d60d3c8267be ]] || { printf 'FAIL: A278 tuned M1 map drifted\n' >&2; exit 1; }
[[ "$(jq -r '."1".num_warps' "$tuned_config_map")" == 8 && "$(jq -r '."1".W1_CONFIG.BLOCK_SIZE_N' "$tuned_config_map")" == 32 && "$(jq -r '."1" | has("W2_CONFIG")' "$tuned_config_map")" == false ]] || { printf 'FAIL: A278 tuned M1 map entry is not the qualified W13-N32 shape\n' >&2; exit 1; }
...
$0 == "rpc_dir=\"/tmp/${campaign}-attempt${attempt}-rpc\"" {
  print "rpc_dir=/tmp/q38-ple2k-a278-rpc"
  next
}
...
' "$base" >"$derived"
chmod 700 "$derived"
if [[ "${Q38_A278_DERIVED_SOURCE_ONLY:-0}" == 1 ]]; then cat "$derived"; exit 0; fi
...
grep -Fxq 'rpc_dir=/tmp/q38-ple2k-a278-rpc' "$derived"
...
if [[ "${Q38_A278_VALIDATE_ONLY:-0}" == 1 ]]; then
...
if [[ "${Q38_A278_VALIDATE_ONLY:-0}" != 1 ]]; then
  [[ "$(findmnt -no SOURCE,FSTYPE --target /mnt/usb-models)" == "/dev/sda2 fuseblk" ]] || {
    printf 'FAIL: A278 evidence mount is not /dev/sda2 fuseblk\n' >&2
    exit 1
  }
  [[ "$(findmnt -no SOURCE,FSTYPE --target /mnt/fast-ai)" == "/dev/nvme0n1p2 ext4" ]] || {
    printf 'FAIL: A278 model mount is not /dev/nvme0n1p2 ext4\n' >&2
    exit 1
  }
  expected_nvme_aer_cor=${Q38_A278_NVME_AER_BASELINE:-}
  expected_root_aer_cor=${Q38_A278_ROOT_AER_BASELINE:-}
  expected_nvme_sectors_read=${Q38_A278_NVME_SECTORS_READ_BASELINE:-}
  [[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
     "$expected_nvme_sectors_read" =~ ^[0-9]+$ ]] || {
    printf 'FAIL: A278 requires numeric host-control AER baselines\n' >&2
    exit 1
  }
  ...
  (( mem_available_kib >= 120000000 )) || { printf 'FAIL: A278 requires MemAvailable >= 120000000 KiB\n' >&2; exit 1; }
  (( swap_total_kib == 0 )) || { printf 'FAIL: A278 requires disk-backed swap disabled\n' >&2; exit 1; }
  [[ "$aspm_policy" == *'[performance]'* ]] || { printf 'FAIL: A278 requires PCIe ASPM performance policy\n' >&2; exit 1; }
  (( root_aer_cor == expected_root_aer_cor && nvme_aer_cor >= expected_nvme_aer_cor && \
    ...
    printf 'FAIL: A278 bounded local-NVMe guard failed\n' >&2; exit 1;
  }
  (( nvme_available_bytes >= 220000000000 )) || { printf 'FAIL: A278 requires >= 220000000000 free NVMe bytes\n' >&2; exit 1; }
fi
...
export TORCH_TRACE=${RUN_PARENT}/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt278/torch-trace
...
export MTP=0 MTP_EXACT=0 MAX_MODEL_LEN=4352 ATTEMPT=278 PORT=19947
export KV_CACHE_MEMORY_BYTES=134217728
export Q38_STEP_TIMING_LOG=10
export Q38_MEM_NOTE=1
export Q38_DIAG_SKIP=gdn_core

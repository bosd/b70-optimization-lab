...
wrapper="${script_dir}/launch-tp4-mtp2-4352-ple-only-a309-fullgraphdet-w13n32.sh"
expected_wrapper=92b0d8d741787d984be7978fa1a47731824e24b7887667f024d38a511ea408d5
client="${script_dir}/run-tp4-mtp2-4352-ple-only-a309-fullgraphdet-w13n32-client.sh"
expected_client=b9adda77f7a9a2c509874c9cdc78885343959d0451da49829714d358eb72d726
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp2-4352-ple-only-r1-attempt309
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp2-4352-ple-only-r1-attempt309-compile
rpc_dir=/tmp/q38-ple2k-a309-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp2-4352-ple-only-r1-attempt309-supervisor
port=19977
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A309 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A309 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A309_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A309_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A309_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A309 host-pressure or NVMe-link guard'
  fi
  if [[ -e "$stop_file" || -e "$failure_file" ]]; then
    ...
    if [[ -e "$stop_file" && ! -e "$failure_file" ]] && \
       ...
       jq -e '.status == "passed" and .recovery_canary == "passed" and
         ...
         .identity.tp == 4 and .identity.ep == 4 and .identity.mtp == 2 and
         .identity.graph == "FULL_DECODE_ONLY" and
.identity.compilation_mode == "NONE" and
.identity.cudagraph_capture_sizes == [1,2,3] and
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A309 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

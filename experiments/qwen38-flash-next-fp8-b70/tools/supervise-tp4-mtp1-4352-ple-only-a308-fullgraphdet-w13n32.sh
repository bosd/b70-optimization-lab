...
wrapper="${script_dir}/launch-tp4-mtp1-4352-ple-only-a308-fullgraphdet-w13n32.sh"
expected_wrapper=d69e6851d7024eab64758e367cf4fc879b90dfc0409b68c7b41aab55d956984c
client="${script_dir}/run-tp4-mtp1-4352-ple-only-a308-fullgraphdet-w13n32-client.sh"
expected_client=df9b573d3f6029853b329562c47673d1b6e6c9a8ac4fe9841ea592f44137a69d
state=/tmp/q38-mtp1-ple-only-a308
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt308
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt308
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt308-compile
rpc_dir=/tmp/q38-ple2k-a308-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt308-supervisor
port=19976
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A308 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A308 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A308_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A308_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A308_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A308 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A308 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

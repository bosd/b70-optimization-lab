#!/usr/bin/env python3
"""Create the A312 packet from frozen A311: the same fused-QSA MTP1 identity and the same
3.5 GiB-per-rank expert host placement, with the census extended to name which host-placed
experts the routing selects (per-rank JSON via Q38_PLACEMENT_HIT_CENSUS_OUT) on head
cc6c01da. A311 proved parked experts are selected (1.0-1.1% of routed blocks on three
ranks, 17.5% on the rank holding the hot experts); this run identifies them so the
placement can be rebuilt. Diagnostic: the census hook adds device work, so its rate is not
a speed measurement."""
from __future__ import annotations
import hashlib, os, re, subprocess
from pathlib import Path
ROOT = Path(__file__).resolve().parent
VALIDATE_ONLY = os.environ.get("Q38_A312_REWRITE_VALIDATE_ONLY") == "1"
SOURCES = {
...
NEW_HEAD = "4c269ee463f8cf0031b49346c2e348d16f7218b3"  # q38-placement-hit-census-v2: per-expert host-placed hit histogram + per-rank dump
CENSUS_OUT = "/tmp/q38-a312-census"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp1-4352-ple-only-a311", "tp4-mtp1-4352-ple-only-a312")
        seg = seg.replace("attempt311", "attempt312").replace("19979", "19982")
        seg = seg.replace("ATTEMPT=311", "ATTEMPT=312").replace("a311", "a312").replace("A311", "A312")
        return seg
    parts = []
    last = 0
...
def main():
    ...
    env = os.environ.copy()
    env["Q38_A312_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a312-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a312" in derived
    ...
    out_names = (
        "launch-tp4-mtp1-4352-ple-only-a312-fullgraphdet-w13n32.sh",
        "run-tp4-mtp1-4352-ple-only-a312-fullgraphdet-w13n32-client.sh",
        "supervise-tp4-mtp1-4352-ple-only-a312-fullgraphdet-w13n32.sh",
        "run-q38-a312-host-controlled.sh",
    )

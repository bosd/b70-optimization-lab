...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a275-fullgraphdet-w13n32.sh"
expected_wrapper=d46ddf9261ccd351e0290f886e542cc8b9577bad5d5ad2bf55c14d07f93cf574
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a275-fullgraphdet-w13n32-client.sh"
expected_client=b6dfb4b174831ecab326292bc41c3a5c3124c12bb2aa3e3b957dbb74a6f83f2f
state=/tmp/q38-mtp0-ple-only-a275
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt275
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt275
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt275-compile
rpc_dir=/tmp/q38-ple2k-a275-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt275-supervisor
port=19944
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A275 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A275 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A275_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A275_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A275_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A275 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A275 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp1-4352-ple-only-a120", "tp4-mtp1-4352-ple-only-a306")
        seg = seg.replace("attempt120", "attempt306").replace("19792", "19975")
        seg = seg.replace("ATTEMPT=120", "ATTEMPT=306").replace("a120", "a306").replace("A120", "A306")
        return seg
    parts=[]; last=0
...
def main():
    ...
    launcher = patch_a306_launcher(launcher)
    ...
    env = os.environ.copy(); env["Q38_A306_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a306-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a306" in derived
    ...
    client = patch_a306_client(client)
    ...
    supervisor = patch_a306_supervisor(supervisor)
    ...
    out_names = ("launch-tp4-mtp1-4352-ple-only-a306-fullgraphdet-w13n32.sh", "run-tp4-mtp1-4352-ple-only-a306-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp1-4352-ple-only-a306-fullgraphdet-w13n32.sh", "run-q38-a306-host-controlled.sh")

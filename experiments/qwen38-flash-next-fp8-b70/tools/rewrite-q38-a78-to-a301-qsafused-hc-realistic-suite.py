#!/usr/bin/env python3
"""Create the A301 packet from frozen A78 for the A213 identity: PLE + embeddings offloaded at 12.25 (12.22 GiB), hot experts resident, never-hit experts host-placed at load time (/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/data/20260906-q38-expert-host-placement-3p5gib-per-rank.json) on clean head 2a372e860e27; realistic suite (LocalMaxxing metric)."""
...
VALIDATE_ONLY = os.environ.get("Q38_A301_REWRITE_VALIDATE_ONLY") == "1"
...
NEW_HEAD = "2a372e860e273273357cb7437ac7de1694304f9f"  # q38-placement-clean-v5: promoted overlay + table kernel + load-time never-hit expert placement
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp0-4352-ple-only-a78", "tp4-mtp0-4352-ple-only-a301")
        seg = seg.replace("attempt78", "attempt301").replace("19750", "19970")
        seg = seg.replace("ATTEMPT=78", "ATTEMPT=301").replace("a78", "a301").replace("A78", "A301")
        return seg
    parts=[]; last=0
...
def main():
    ...
    env = os.environ.copy(); env["Q38_A301_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a301-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a301" in derived
    ...
    out_names = ("launch-tp4-mtp0-4352-ple-only-a301-fullgraphdet-w13n32.sh", "run-tp4-mtp0-4352-ple-only-a301-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp0-4352-ple-only-a301-fullgraphdet-w13n32.sh", "run-q38-a301-host-controlled.sh")

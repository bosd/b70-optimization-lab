...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a245-fullgraphdet-w13n32.sh"
expected_wrapper=7e8f6193e80b3af08002c263f4e78a05cac373503de2cb97066926c98ff91b1c
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a245-fullgraphdet-w13n32-client.sh"
expected_client=9a71fd46321ebee5e9c96b3bc7037e212071b636ae1ca1a908028bdbdc7b9906
state=/tmp/q38-mtp0-ple-only-a245
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt245
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt245
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt245-compile
rpc_dir=/tmp/q38-ple2k-a245-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt245-supervisor
port=19915
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A245 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A245 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A245_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A245_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A245_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A245 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A245 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

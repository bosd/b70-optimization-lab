...
derived=/tmp/q38-ple2k-a324-base.sh
expected_base=d5ccc4d52220f7ef46f19202436edf56e0c40f125b1b807c84125df18093b5c1
expected_derived=bec6cdbde4f189a98be8f920d07f3c6447307c109219282768a789560e6b5e02
...
[[ "$(sha256sum "$tuned_config_map" | cut -d' ' -f1)" == 4fcb5d13ef0c859d12a4fe6b5aac09b04fccf4db24e47a6f004d3f9878e4e38f ]] || { printf 'FAIL: A324 tuned M1 map drifted\n' >&2; exit 1; }
[[ "$(jq -r '."1".num_warps' "$tuned_config_map")" == 8 && "$(jq -r '."1".W1_CONFIG.BLOCK_SIZE_N' "$tuned_config_map")" == 64 && "$(jq -r '."1" | has("W2_CONFIG")' "$tuned_config_map")" == false ]] || { printf 'FAIL: A324 tuned M1 map entry is not the qualified W13-N64 shape\n' >&2; exit 1; }
...
$0 == "rpc_dir=\"/tmp/${campaign}-attempt${attempt}-rpc\"" {
  print "rpc_dir=/tmp/q38-ple2k-a324-rpc"
  next
}
$0 == "expected_vllm_head=\"1372c62d975c554f4b465c8299bc5f3295301ceb\"" {
  print "expected_vllm_head=\"6d8724577dabbee5fa0bbc70c4d927c6174c8d8a\""
  next
}
...
$0 == "export CCL_TOPO_P2P_ACCESS=1" {
  ...
  print "export VLLM_TUNED_CONFIG_FOLDER=/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/configs/moe-m1-w13-n64"
...
index($0, "diagnostics=none") > 0 {
  ...
  print "  printf '\''tuned_config_folder=moe-m1-w13-n64\\n'\''"
  print "  printf '\''mkldnn_deterministic=1\\n'\''"
  print "  printf '\''tuned_config_map_sha256=4fcb5d13ef0c859d12a4fe6b5aac09b04fccf4db24e47a6f004d3f9878e4e38f\\n'\''"
  next
}
...
' "$base" >"$derived"
chmod 700 "$derived"
if [[ "${Q38_A324_DERIVED_SOURCE_ONLY:-0}" == 1 ]]; then cat "$derived"; exit 0; fi
...
grep -Fxq 'expected_vllm_head="6d8724577dabbee5fa0bbc70c4d927c6174c8d8a"' "$derived"
grep -Fxq "  printf 'diagnostics=full-decode-graph-public-oneccl-torch-trace\n'" "$derived"
! grep -Fq "diagnostics=none" "$derived"
grep -Fxq 'rpc_dir=/tmp/q38-ple2k-a324-rpc' "$derived"
...
[[ "$(grep -Fxc 'export VLLM_TUNED_CONFIG_FOLDER=/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/configs/moe-m1-w13-n64' "$derived")" == 1 ]]
[[ "$(grep -Fxc "  printf 'tuned_config_folder=moe-m1-w13-n64\\n'" "$derived")" == 1 ]]
[[ "$(grep -Fxc 'export VLLM_XPU_MKLDNN_DETERMINISTIC=1' "$derived")" == 1 ]]
[[ "$(grep -Fxc "  printf 'mkldnn_deterministic=1\\n'" "$derived")" == 1 ]]
[[ "$(grep -Fxc "  printf 'tuned_config_map_sha256=4fcb5d13ef0c859d12a4fe6b5aac09b04fccf4db24e47a6f004d3f9878e4e38f\\n'" "$derived")" == 1 ]]
...
if [[ "${Q38_A324_VALIDATE_ONLY:-0}" == 1 ]]; then
...
if [[ "${Q38_A324_VALIDATE_ONLY:-0}" != 1 ]]; then
  [[ "$(findmnt -no SOURCE,FSTYPE --target /mnt/usb-models)" == "/dev/sda2 fuseblk" ]] || {
    printf 'FAIL: A324 evidence mount is not /dev/sda2 fuseblk\n' >&2
    exit 1
  }
  [[ "$(findmnt -no SOURCE,FSTYPE --target /mnt/fast-ai)" == "/dev/nvme0n1p2 ext4" ]] || {
    printf 'FAIL: A324 model mount is not /dev/nvme0n1p2 ext4\n' >&2
    exit 1
  }
  expected_nvme_aer_cor=${Q38_A324_NVME_AER_BASELINE:-}
  expected_root_aer_cor=${Q38_A324_ROOT_AER_BASELINE:-}
  expected_nvme_sectors_read=${Q38_A324_NVME_SECTORS_READ_BASELINE:-}
  [[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
     "$expected_nvme_sectors_read" =~ ^[0-9]+$ ]] || {
    printf 'FAIL: A324 requires numeric host-control AER baselines\n' >&2
    exit 1
  }
  ...
  (( mem_available_kib >= 120000000 )) || { printf 'FAIL: A324 requires MemAvailable >= 120000000 KiB\n' >&2; exit 1; }
  (( swap_total_kib == 0 )) || { printf 'FAIL: A324 requires disk-backed swap disabled\n' >&2; exit 1; }
  [[ "$aspm_policy" == *'[performance]'* ]] || { printf 'FAIL: A324 requires PCIe ASPM performance policy\n' >&2; exit 1; }
  (( root_aer_cor == expected_root_aer_cor && nvme_aer_cor >= expected_nvme_aer_cor && \
    ...
    printf 'FAIL: A324 bounded local-NVMe guard failed\n' >&2; exit 1;
  }
  (( nvme_available_bytes >= 220000000000 )) || { printf 'FAIL: A324 requires >= 220000000000 free NVMe bytes\n' >&2; exit 1; }
fi
...
export TORCH_TRACE=${RUN_PARENT}/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt324/torch-trace
...
export MTP=1 MTP_EXACT=0 MAX_MODEL_LEN=4352 ATTEMPT=324 PORT=19994

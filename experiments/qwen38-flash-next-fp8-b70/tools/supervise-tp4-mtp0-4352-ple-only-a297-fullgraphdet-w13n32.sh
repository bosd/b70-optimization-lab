...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a297-fullgraphdet-w13n32.sh"
expected_wrapper=0344da2086bb740d6a33c89dd4dae75537761d5cfdcafcf7aa689e9aedd4acbb
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a297-fullgraphdet-w13n32-client.sh"
expected_client=a2f53db89feecd75837021fbd1cf3434a4f288c7e1b7b41eeca59632b1bc5a97
state=/tmp/q38-mtp0-ple-only-a297
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt297
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt297
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt297-compile
rpc_dir=/tmp/q38-ple2k-a297-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt297-supervisor
port=19966
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A297 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A297 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A297_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A297_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A297_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A297 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A297 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

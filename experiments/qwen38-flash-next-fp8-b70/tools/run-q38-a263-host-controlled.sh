...
supervisor=/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/tools/supervise-tp4-mtp0-4352-ple-only-a263-fullgraphdet-w13n32.sh
expected_supervisor=254ee8f6feefcfb9d09c6e161dfbb82914738b8028eaf297d901347d951e5982
...
restore_host() {
  ...
  if [[ "$child" =~ ^[1-9][0-9]*$ ]] && kill -0 "$child" 2>/dev/null; then
    ...
    if kill -0 "$child" 2>/dev/null; then
      printf 'FAIL: A263 child remained live after bounded teardown\n' >&2
      restore_rc=1
    else
  ...
  if (( restore_rc != 0 )); then
    printf 'FAIL: A263 host-control restoration was incomplete\n' >&2
    rc=71
  fi
  trap - EXIT
  exit "$rc"
...
[[ $EUID == 0 ]] || { printf 'FAIL: A263 host control must run as root\n' >&2; exit 1; }
[[ $# == 0 ]] || { printf 'FAIL: A263 host control takes no arguments\n' >&2; exit 2; }
[[ "$(sha256sum "$supervisor" | cut -d' ' -f1)" == "$expected_supervisor" ]] || {
  printf 'FAIL: A263 supervisor hash changed\n' >&2
  exit 1
}
...
[[ "${#active_swaps[@]}" == 1 ]] || { printf 'FAIL: A263 requires exactly one active swap device\n' >&2; exit 1; }
...
[[ "$original_swap_path" == /swap.img && "$original_swap_used" == 0 && \
  ...
  printf 'FAIL: A263 requires an unused /swap.img with a recorded priority\n' >&2
  exit 1
}
mem_available_kib=$(awk '/^MemAvailable:/ {print $2}' /proc/meminfo)
(( mem_available_kib >= 120000000 )) || { printf 'FAIL: A263 host-control memory floor failed\n' >&2; exit 1; }
if [[ "${Q38_A263_HOST_VALIDATE_ONLY:-0}" == 1 ]]; then
  printf 'PASS: A263 host-control static identity; ASPM=%s swap=%s used=%s priority=%s\n' \
...
export Q38_A263_NVME_AER_BASELINE
export Q38_A263_ROOT_AER_BASELINE
export Q38_A263_NVME_SECTORS_READ_BASELINE
Q38_A263_NVME_AER_BASELINE=$(awk '$1 == "TOTAL_ERR_COR" {print $2}' \
  /sys/bus/pci/devices/0000:01:00.0/aer_dev_correctable)
Q38_A263_ROOT_AER_BASELINE=$(< /sys/bus/pci/devices/0000:00:03.1/aer_rootport_total_err_cor)
Q38_A263_NVME_SECTORS_READ_BASELINE=$(awk '$3 == "nvme0n1" {print $6}' /proc/diskstats)
[[ "$Q38_A263_NVME_AER_BASELINE" =~ ^[0-9]+$ && "$Q38_A263_ROOT_AER_BASELINE" =~ ^[0-9]+$ && \
   "$Q38_A263_NVME_SECTORS_READ_BASELINE" =~ ^[0-9]+$ ]] || {
  printf 'FAIL: A263 could not establish numeric AER baselines\n' >&2
  exit 1
}

...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a295-fullgraphdet-w13n32.sh"
expected_wrapper=fa30caac29407e8fff598a51ec99560ef48f64d511d9e7e7c16727761b21ad89
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a295-fullgraphdet-w13n32-client.sh"
expected_client=b752385b3e88092241e51722f5f1ea5ef5e193fb55a31ac9128e9d8a1cd3a672
state=/tmp/q38-mtp0-ple-only-a295
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt295
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt295
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt295-compile
rpc_dir=/tmp/q38-ple2k-a295-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt295-supervisor
port=19964
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A295 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A295 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A295_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A295_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A295_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A295 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A295 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

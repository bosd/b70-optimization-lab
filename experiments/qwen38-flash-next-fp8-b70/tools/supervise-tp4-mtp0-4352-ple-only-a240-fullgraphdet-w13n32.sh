...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a240-fullgraphdet-w13n32.sh"
expected_wrapper=1da57ef77242268ea3c7df3fcf10aced1411b4c5ce6b53de0fad95b1279793d2
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a240-fullgraphdet-w13n32-client.sh"
expected_client=0e5ddde4a8064c6d5b6253e56154933fcdd806610e234f66892e2e0f742bd474
state=/tmp/q38-mtp0-ple-only-a240
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt240
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt240
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt240-compile
rpc_dir=/tmp/q38-ple2k-a240-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt240-supervisor
port=19910
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A240 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A240 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A240_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A240_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A240_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A240 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A240 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

#!/usr/bin/env python3
"""Create the A296 packet from frozen A78: graph MTP0 exact-2K timing screen with PLE + embeddings offloaded at budget 12.25 (hot experts resident) and 3.5 GiB/rank of never-hit experts host-placed at load time on q38-placement-v5 c4f921f5."""
...
VALIDATE_ONLY = os.environ.get("Q38_A296_REWRITE_VALIDATE_ONLY") == "1"
...
NEW_HEAD = "831ea0ec5d076c39fbe498b0e176b66a720618f1"  # q38-placement-v5: table kernel + load-time never-hit expert placement
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp0-4352-ple-only-a78", "tp4-mtp0-4352-ple-only-a296")
        seg = seg.replace("attempt78", "attempt296").replace("19750", "19965")
        seg = seg.replace("ATTEMPT=78", "ATTEMPT=296").replace("a78", "a296").replace("A78", "A296")
        return seg
    parts=[]; last=0
...
def main():
    ...
    env = os.environ.copy(); env["Q38_A296_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a296-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a296" in derived
    ...
    out_names = ("launch-tp4-mtp0-4352-ple-only-a296-fullgraphdet-w13n32.sh", "run-tp4-mtp0-4352-ple-only-a296-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp0-4352-ple-only-a296-fullgraphdet-w13n32.sh", "run-q38-a296-host-controlled.sh")

...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a284-fullgraphdet-w13n32.sh"
expected_wrapper=bd096c202a8bd8c15eb549deea156ddc0b0f57cec583701f042da9ab021e611c
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a284-fullgraphdet-w13n32-client.sh"
expected_client=a54d5923af1efe39f6ef49f3631f794d043784f3b6df0620d3bcd974725f69dd
state=/tmp/q38-mtp0-ple-only-a284
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt284
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt284
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt284-compile
rpc_dir=/tmp/q38-ple2k-a284-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt284-supervisor
port=19953
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A284 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A284 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A284_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A284_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A284_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A284 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A284 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

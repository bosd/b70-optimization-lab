...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a301-fullgraphdet-w13n32.sh"
expected_wrapper=c6b95a4c751ea54c65abe375c8d5fc14ffedccd3a85bbb05afab4eca2b9d4bcc
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a301-fullgraphdet-w13n32-client.sh"
expected_client=a9c0e2ea181af95c14be509da038fc924e887ccfddb48e2f8b411802e3d0f404
state=/tmp/q38-mtp0-ple-only-a301
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt301
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt301
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt301-compile
rpc_dir=/tmp/q38-ple2k-a301-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt301-supervisor
port=19970
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A301 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A301 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A301_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A301_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A301_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A301 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A301 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

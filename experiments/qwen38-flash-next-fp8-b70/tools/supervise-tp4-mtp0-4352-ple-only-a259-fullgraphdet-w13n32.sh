...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a259-fullgraphdet-w13n32.sh"
expected_wrapper=7902315eb69ba2be1811334ff1e7eca79bd6026cbccb8b0a6aca75584b0b8dfe
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a259-fullgraphdet-w13n32-client.sh"
expected_client=67b051cdc03f165a8e2c22577bb89bc46b41b4c124ab7ac6d855435bdd4bc569
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt259
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt259-compile
rpc_dir=/tmp/q38-ple2k-a259-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt259-supervisor
port=19925
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A259 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A259 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A259_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A259_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A259_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A259 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A259 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

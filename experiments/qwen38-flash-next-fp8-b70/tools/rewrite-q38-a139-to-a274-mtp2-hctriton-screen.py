#!/usr/bin/env python3
"""Create the A274 packet from frozen A139: graph MTP2 screen at the placement identity (PLE + embeddings offloaded at 12.25, hot experts resident, 3.5 GiB/rank never-hit experts host-placed) on the MTP1 placement head 005dc578."""
from __future__ import annotations
import hashlib, os, re, subprocess
from pathlib import Path
ROOT = Path(__file__).resolve().parent
VALIDATE_ONLY = os.environ.get("Q38_A274_REWRITE_VALIDATE_ONLY") == "1"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp2-4352-ple-only-a139", "tp4-mtp2-4352-ple-only-a274")
        seg = seg.replace("attempt139", "attempt274").replace("19810", "19943")
        seg = seg.replace("ATTEMPT=139", "ATTEMPT=274").replace("a139", "a274").replace("A139", "A274")
        return seg
    parts=[]; last=0
...
NEW_HEAD = "622191221475b53cc6f7f4d847860939f4c300ab"  # q38-placement-mtp1-clean: placement commits on the lossless MTP1 head; MTP2 = two speculative tokens on the same code
def patch_a274_launcher(l):
...
def main():
    ...
    launcher = patch_a274_launcher(launcher)
    ...
    env = os.environ.copy(); env["Q38_A274_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a274-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a274" in derived
    launcher = launcher.replace("expected_derived=" + "0"*64, "expected_derived=" + digest(derived))
    client = successor(source("run-tp4-mtp2-4352-ple-only-a139-fullgraphdet-w13n32-client.sh"))
    client = patch_a274_client(client)
    supervisor = successor(source("supervise-tp4-mtp2-4352-ple-only-a139-fullgraphdet-w13n32.sh"))
    supervisor = patch_a274_supervisor(supervisor)
    ...
    out_names = ("launch-tp4-mtp2-4352-ple-only-a274-fullgraphdet-w13n32.sh", "run-tp4-mtp2-4352-ple-only-a274-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp2-4352-ple-only-a274-fullgraphdet-w13n32.sh", "run-q38-a274-host-controlled.sh")

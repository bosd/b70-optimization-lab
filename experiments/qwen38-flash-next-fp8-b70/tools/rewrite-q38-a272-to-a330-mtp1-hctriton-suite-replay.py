...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp1-4352-ple-only-a272", "tp4-mtp1-4352-ple-only-a330")
        seg = seg.replace("attempt272", "attempt330").replace("19942", "19943")
        seg = seg.replace("ATTEMPT=272", "ATTEMPT=330").replace("a272", "a330").replace("A272", "A330")
    ...
    assert "ATTEMPT=330" in out or "ATTEMPT=" not in out, "the attempt number did not move"
...
def main():
    ...
    env = os.environ.copy()
    env["Q38_A330_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a330-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a330" in derived
    ...
    out_names = (
        "launch-tp4-mtp1-4352-ple-only-a330-fullgraphdet-w13n32.sh",
        "run-tp4-mtp1-4352-ple-only-a330-fullgraphdet-w13n32-client.sh",
        "supervise-tp4-mtp1-4352-ple-only-a330-fullgraphdet-w13n32.sh",
        "run-q38-a330-host-controlled.sh",
    )

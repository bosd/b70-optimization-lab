...
wrapper="${script_dir}/launch-tp4-mtp1-4352-ple-only-a271-fullgraphdet-w13n32.sh"
expected_wrapper=a5d65f5096a432e429a5b090a7a61c95f56066b73812d4b177efe4a4efd44f52
client="${script_dir}/run-tp4-mtp1-4352-ple-only-a271-fullgraphdet-w13n32-client.sh"
expected_client=df1ce2a35c5615d4cd154a00715c3d6cc8f5e51125cb21f4a5468546fcd72bc9
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt271
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt271-compile
rpc_dir=/tmp/q38-ple2k-a271-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt271-supervisor
port=19941
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A271 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A271 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A271_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A271_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A271_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A271 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A271 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

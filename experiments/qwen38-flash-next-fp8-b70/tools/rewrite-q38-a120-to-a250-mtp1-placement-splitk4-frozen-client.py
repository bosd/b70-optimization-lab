#!/usr/bin/env python3
"""Create the A250 packet from frozen A120 (MTP1) for the A213 identity: PLE + embeddings offloaded at 12.25 (12.22 GiB), hot experts resident, never-hit experts host-placed at load time (/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/data/20260906-q38-expert-host-placement-3p5gib-per-rank.json) on clean head e9e65888981a; frozen-client certification battery."""
from __future__ import annotations
import hashlib, os, re, subprocess
from pathlib import Path
ROOT = Path(__file__).resolve().parent
VALIDATE_ONLY = os.environ.get("Q38_A250_REWRITE_VALIDATE_ONLY") == "1"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp1-4352-ple-only-a120", "tp4-mtp1-4352-ple-only-a250")
        seg = seg.replace("attempt120", "attempt250").replace("19792", "19895")
        seg = seg.replace("ATTEMPT=120", "ATTEMPT=250").replace("a120", "a250").replace("A120", "A250")
        return seg
    parts=[]; last=0
...
NEW_HEAD = "e9e65888981a880b8298143f571f276c8c52e4a9"  # q38-placement-mtp1-clean: lossless MTP1 head + table kernel + load-time never-hit expert placement
def patch_a250_launcher(l):
...
def patch_a250_client(c):
    ...
    if '"placement": "ple_only_uva", "ple_host_bytes_per_rank": 12800061440,' in c:
        c = replace_once(c, '"placement": "ple_only_uva", "ple_host_bytes_per_rank": 12800061440,', '"placement": "ple_embed_budget12p25_uva_cold_expert_host_placement_splitk4", "ple_host_bytes_per_rank": 12800061440, "host_offload_bytes_per_rank": 13117911040, "host_offload_params": "ple_embedding.ngram_embedding.weight,embed_tokens.weight", "expert_host_placement": "/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/data/20260906-q38-expert-host-placement-3p5gib-per-rank.json",')
...
def main():
    ...
    launcher = patch_a250_launcher(launcher)
    ...
    env = os.environ.copy(); env["Q38_A250_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a250-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a250" in derived
    launcher = launcher.replace("expected_derived=" + "0"*64, "expected_derived=" + digest(derived))
    client = successor(source("run-tp4-mtp1-4352-ple-only-a120-fullgraphdet-w13n32-client.sh"))
    client = patch_a250_client(client)
    ...
    supervisor = patch_a250_supervisor(supervisor)
    ...
    out_names = ("launch-tp4-mtp1-4352-ple-only-a250-fullgraphdet-w13n32.sh", "run-tp4-mtp1-4352-ple-only-a250-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp1-4352-ple-only-a250-fullgraphdet-w13n32.sh", "run-q38-a250-host-controlled.sh")

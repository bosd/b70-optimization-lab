...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a285-mkldnndet-w13n32.sh"
expected_wrapper=ac4b7e8303bf40153be209d5b1044d90b4cd91bb8732b69ebeb3949ce73a845f
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a285-mkldnndet-w13n32-client.sh"
expected_client=c7f69f7673c224ef3645c6ea4267f508647333a5d2e8bcd290f10523d461e4c6
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-mkldnndet-mtp0-4352-ple-only-r1-attempt285
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-mkldnndet-mtp0-4352-ple-only-r1-attempt285-compile
rpc_dir=/tmp/q38-ple2k-a285-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-mkldnndet-mtp0-4352-ple-only-r1-attempt285-supervisor
port=19954
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A285 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A285 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A285_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A285_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A285_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A285 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A285 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

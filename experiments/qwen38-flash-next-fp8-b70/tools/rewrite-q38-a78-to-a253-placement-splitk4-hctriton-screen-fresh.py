...
VALIDATE_ONLY = os.environ.get("Q38_A253_REWRITE_VALIDATE_ONLY") == "1"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp0-4352-ple-only-a78", "tp4-mtp0-4352-ple-only-a253")
        seg = seg.replace("attempt78", "attempt253").replace("19750", "19919")
        seg = seg.replace("ATTEMPT=78", "ATTEMPT=253").replace("a78", "a253").replace("A78", "A253")
        return seg
    parts=[]; last=0
...
def main():
    ...
    env = os.environ.copy(); env["Q38_A253_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a253-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a253" in derived
    ...
    out_names = ("launch-tp4-mtp0-4352-ple-only-a253-fullgraphdet-w13n32.sh", "run-tp4-mtp0-4352-ple-only-a253-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp0-4352-ple-only-a253-fullgraphdet-w13n32.sh", "run-q38-a253-host-controlled.sh")

#!/usr/bin/env python3
"""Create the A286 step and per-layer timing packet from frozen A83 (eager MTP0 twin of A122 with the per-step timing log, no trace).
...
d132de8c). Attempt 163 / port 19955.
"""
...
VALIDATE_ONLY = os.environ.get("Q38_A286_REWRITE_VALIDATE_ONLY") == "1"
...
NEW_HEAD = "7751df34a3ca4865b13f27cee889f160cf62c21d"  # q38-placement-v5 diagnostics head: placement + all tuned-map fixes + timing hooks
MTP_RULE = '$0 == "[[ \\"${mtp}\\" == \\"0\\" ]] || {" {\n  print "[[ \\"${mtp}\\" == \\"1\\" ]] || {"\n  next\n}\n'
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp1-4352-ple-only-a83", "tp4-mtp0-4352-ple-only-a286")
        ...
        seg = seg.replace("attempt83", "attempt286").replace("19755", "19955")
        seg = seg.replace("ATTEMPT=83", "ATTEMPT=286").replace("a83", "a286").replace("A83", "A286")
        return seg
    parts=[]; last=0
...
def main():
    ...
    assert launcher.count("moe-m1-w13-n32") >= 1 and launcher.count("a8f1f8982e3e1af80ff31b9e0a00afaacf1af1b3c401585109b4d60d3c8267be") >= 1, (launcher.count("moe-m1-w13-n32"), launcher.count("a8f1f8982e3e1af80ff31b9e0a00afaacf1af1b3c401585109b4d60d3c8267be"))
    launcher = launcher.replace("moe-m1-w13-n32", "moe-m1-w13-n32-w16").replace("a8f1f8982e3e1af80ff31b9e0a00afaacf1af1b3c401585109b4d60d3c8267be", "39a1cce223f6911ca96d0769c1cabf6d15f5b82eb399b6748048d077917f450d")  # M=1 entry at 16 warps
    if '\'."1".num_warps\' "$tuned_config_map")" == 8 &&' in launcher:
        launcher = launcher.replace('\'."1".num_warps\' "$tuned_config_map")" == 8 &&', '\'."1".num_warps\' "$tuned_config_map")" == 16 &&')
    launcher = _fm.patch_launcher(launcher)
    if MTP0:
        launcher = replace_once(launcher, MTP_RULE, "")
        launcher = replace_once(launcher, MTP_GREPS, "")
        launcher = replace_once(launcher, "export MTP=1 MTP_EXACT=0 MAX_MODEL_LEN=4352 ATTEMPT=286 PORT=19955\n", "export MTP=0 MTP_EXACT=0 MAX_MODEL_LEN=4352 ATTEMPT=286 PORT=19955\n")
        ...
        launcher = replace_once(launcher, "export MTP=1 MTP_EXACT=0 MAX_MODEL_LEN=4352 ATTEMPT=286 PORT=19955\n", "export MTP=1 MTP_EXACT=1 MAX_MODEL_LEN=4352 ATTEMPT=286 PORT=19955\n")
    ...
    env = os.environ.copy(); env["Q38_A286_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a286-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a286" in derived and "  --enforce-eager\n" in derived
    ...
    names = ("launch-tp4-mtp0-4352-ple-only-a286-mkldnndet-w13n32.sh", "run-tp4-mtp0-4352-ple-only-a286-mkldnndet-w13n32-client.sh", "supervise-tp4-mtp0-4352-ple-only-a286-mkldnndet-w13n32.sh", "run-q38-a286-host-controlled.sh")

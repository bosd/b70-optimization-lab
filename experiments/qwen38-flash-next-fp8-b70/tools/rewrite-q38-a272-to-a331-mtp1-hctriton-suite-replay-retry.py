#!/usr/bin/env python3
"""Create the A331 retry of the A272 replay, the Triton-HC lossless-MTP1 suite record (37.045844),\nwith nothing changed but attempt and port.\n\nA328 and A329 established that the *current* MTP1 record replays, and that the lineage's\nsuite spread is about 0.86% peak-to-peak. This extends the audit one record back, to the\npredecessor on overlay 62219122, so "our published records still reproduce" becomes a\nsystematic claim about the lane rather than a spot check of its newest line."""
from __future__ import annotations
import hashlib, os, re, subprocess
from pathlib import Path
ROOT = Path(__file__).resolve().parent
VALIDATE_ONLY = os.environ.get("Q38_A331_REWRITE_VALIDATE_ONLY") == "1"
SOURCES = {
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp1-4352-ple-only-a272", "tp4-mtp1-4352-ple-only-a331")
        seg = seg.replace("attempt272", "attempt331").replace("19942", "19944")
        seg = seg.replace("ATTEMPT=272", "ATTEMPT=331").replace("a272", "a331").replace("A272", "A331")
    ...
    assert "ATTEMPT=331" in out or "ATTEMPT=" not in out, "the attempt number did not move"
...
def main():
    ...
    env = os.environ.copy()
    env["Q38_A331_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a331-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a331" in derived
    ...
    out_names = (
        "launch-tp4-mtp1-4352-ple-only-a331-fullgraphdet-w13n32.sh",
        "run-tp4-mtp1-4352-ple-only-a331-fullgraphdet-w13n32-client.sh",
        "supervise-tp4-mtp1-4352-ple-only-a331-fullgraphdet-w13n32.sh",
        "run-q38-a331-host-controlled.sh",
    )

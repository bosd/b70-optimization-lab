...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a260-fullgraphdet-w13n32.sh"
expected_wrapper=3ec12cb9c697f44605e2ff49a14cc35e2b03f9cc650415b040d4cc6703f4acbd
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a260-fullgraphdet-w13n32-client.sh"
expected_client=d8782b4c9be847324088fe4b5e626d6731c07156101cd9156a7c42f241faf455
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt260
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt260-compile
rpc_dir=/tmp/q38-ple2k-a260-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt260-supervisor
port=19926
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A260 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A260 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A260_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A260_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A260_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A260 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A260 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

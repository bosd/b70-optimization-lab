#!/usr/bin/env python3
"""Create the A332 third suite on the Triton-HC overlay 62219122.\n\nA331 replayed that record at 37.426051 against its published 37.045844, and A328/A329\nreplayed the fused-QSA record either side of its 37.825654. Each record reproduces, but\nthe +0.780 gap published between the two lineages came out at +0.358 on replay, against a\nmeasured single-packet spread of 0.324 -- so one suite per side cannot separate it.\n\nfused-QSA already has three suites (A306, A328, A329). This is the third for Triton-HC,\nmaking the comparison three against three, to be read as distributions rather than as a\ndifference of single medians."""
from __future__ import annotations
import hashlib, os, re, subprocess
from pathlib import Path
ROOT = Path(__file__).resolve().parent
VALIDATE_ONLY = os.environ.get("Q38_A332_REWRITE_VALIDATE_ONLY") == "1"
SOURCES = {
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp1-4352-ple-only-a272", "tp4-mtp1-4352-ple-only-a332")
        seg = seg.replace("attempt272", "attempt332").replace("19942", "19945")
        seg = seg.replace("ATTEMPT=272", "ATTEMPT=332").replace("a272", "a332").replace("A272", "A332")
    ...
    assert "ATTEMPT=332" in out or "ATTEMPT=" not in out, "the attempt number did not move"
...
def main():
    ...
    env = os.environ.copy()
    env["Q38_A332_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a332-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a332" in derived
    ...
    out_names = (
        "launch-tp4-mtp1-4352-ple-only-a332-fullgraphdet-w13n32.sh",
        "run-tp4-mtp1-4352-ple-only-a332-fullgraphdet-w13n32-client.sh",
        "supervise-tp4-mtp1-4352-ple-only-a332-fullgraphdet-w13n32.sh",
        "run-q38-a332-host-controlled.sh",
    )

...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a298-fullgraphdet-w13n32.sh"
expected_wrapper=84e940b2cbe827213ae1622ebae52dbf10f67c663c7613a4b527605334f5d5ac
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a298-fullgraphdet-w13n32-client.sh"
expected_client=cb562be6afff8e6e7b5de941d843c41f6bfce2c630a9122769556fae595bc66a
state=/tmp/q38-mtp0-ple-only-a298
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt298
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt298
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt298-compile
rpc_dir=/tmp/q38-ple2k-a298-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt298-supervisor
port=19967
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A298 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A298 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A298_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A298_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A298_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A298 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A298 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

...
VALIDATE_ONLY = os.environ.get("Q38_A300_REWRITE_VALIDATE_ONLY") == "1"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp0-4352-ple-only-a78", "tp4-mtp0-4352-ple-only-a300")
        seg = seg.replace("attempt78", "attempt300").replace("19750", "19969")
        seg = seg.replace("ATTEMPT=78", "ATTEMPT=300").replace("a78", "a300").replace("A78", "A300")
        return seg
    parts=[]; last=0
...
def main():
    ...
    Path("/tmp/q38-ple2k-a300-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a300" in derived
    ...
    out_names = ("launch-tp4-mtp0-4352-ple-only-a300-fullgraphdet-w13n32.sh", "run-tp4-mtp0-4352-ple-only-a300-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp0-4352-ple-only-a300-fullgraphdet-w13n32.sh", "run-q38-a300-host-controlled.sh")

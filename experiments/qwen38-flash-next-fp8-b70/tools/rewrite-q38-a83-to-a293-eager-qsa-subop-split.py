#!/usr/bin/env python3
"""Create the A293 step and per-layer timing packet from frozen A83 (eager MTP0 twin of A122 with the per-step timing log, no trace).
...
d132de8c). Attempt 163 / port 19962.
"""
...
VALIDATE_ONLY = os.environ.get("Q38_A293_REWRITE_VALIDATE_ONLY") == "1"
...
NEW_HEAD = "93013c522720f8fde144059ded048dc4aaf87e43"  # q38-placement-v5 diagnostics head: placement + all tuned-map fixes + timing hooks
MTP_RULE = '$0 == "[[ \\"${mtp}\\" == \\"0\\" ]] || {" {\n  print "[[ \\"${mtp}\\" == \\"1\\" ]] || {"\n  next\n}\n'
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp1-4352-ple-only-a83", "tp4-mtp0-4352-ple-only-a293")
        ...
        seg = seg.replace("attempt83", "attempt293").replace("19755", "19962")
        seg = seg.replace("ATTEMPT=83", "ATTEMPT=293").replace("a83", "a293").replace("A83", "A293")
        return seg
    parts=[]; last=0
...
def main():
    ...
    if MTP0:
        ...
        launcher = replace_once(launcher, "export MTP=1 MTP_EXACT=0 MAX_MODEL_LEN=4352 ATTEMPT=293 PORT=19962\n", "export MTP=0 MTP_EXACT=0 MAX_MODEL_LEN=4352 ATTEMPT=293 PORT=19962\n")
        ...
        launcher = replace_once(launcher, "export MTP=1 MTP_EXACT=0 MAX_MODEL_LEN=4352 ATTEMPT=293 PORT=19962\n", "export MTP=1 MTP_EXACT=1 MAX_MODEL_LEN=4352 ATTEMPT=293 PORT=19962\n")
    ...
    env = os.environ.copy(); env["Q38_A293_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a293-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a293" in derived and "  --enforce-eager\n" in derived
    ...
    names = ("launch-tp4-mtp0-4352-ple-only-a293-mkldnndet-w13n32.sh", "run-tp4-mtp0-4352-ple-only-a293-mkldnndet-w13n32-client.sh", "supervise-tp4-mtp0-4352-ple-only-a293-mkldnndet-w13n32.sh", "run-q38-a293-host-controlled.sh")

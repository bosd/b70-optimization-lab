#!/usr/bin/env python3
"""Create the A326 fresh-server suite repeat of A325.\n\nA325 put the realistic-suite median at 34.510128 class-balanced against the published\n33.797067, with all twelve rows above A301's highest row. The lane does not move a record\non one suite run, so this repeats it from cold on a reset host: same map, same certified\nhead, same suite driver."""
from __future__ import annotations
import hashlib, os, re, subprocess
from pathlib import Path
ROOT = Path(__file__).resolve().parent
VALIDATE_ONLY = os.environ.get("Q38_A326_REWRITE_VALIDATE_ONLY") == "1"
SOURCES = {
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp0-4352-ple-only-a301", "tp4-mtp0-4352-ple-only-a326")
        seg = seg.replace("attempt301", "attempt326").replace("19970", "19996")
        seg = seg.replace("ATTEMPT=301", "ATTEMPT=326").replace("a301", "a326").replace("A301", "A326")
...
def main():
    ...
    env = os.environ.copy()
    env["Q38_A326_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a326-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a326" in derived
    ...
    out_names = (
        "launch-tp4-mtp0-4352-ple-only-a326-fullgraphdet-w13n64.sh",
        "run-tp4-mtp0-4352-ple-only-a326-fullgraphdet-w13n64-client.sh",
        "supervise-tp4-mtp0-4352-ple-only-a326-fullgraphdet-w13n64.sh",
        "run-q38-a326-host-controlled.sh",
    )

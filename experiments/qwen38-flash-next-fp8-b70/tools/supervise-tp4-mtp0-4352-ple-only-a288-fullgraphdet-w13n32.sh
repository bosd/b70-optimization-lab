...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a288-fullgraphdet-w13n32.sh"
expected_wrapper=810854493b2224299801d01fff602f6f1626f7526fa8e8739a9188cd1928fa2b
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a288-fullgraphdet-w13n32-client.sh"
expected_client=1ec5273cba1d0904c2a69b42cbf53a58925f153b67a310cdbb19cfdc998fad79
state=/tmp/q38-mtp0-ple-only-a288
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt288
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt288
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt288-compile
rpc_dir=/tmp/q38-ple2k-a288-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt288-supervisor
port=19957
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A288 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A288 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A288_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A288_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A288_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A288 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A288 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

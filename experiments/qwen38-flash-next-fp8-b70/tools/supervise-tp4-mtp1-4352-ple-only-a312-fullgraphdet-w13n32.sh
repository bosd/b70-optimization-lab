...
wrapper="${script_dir}/launch-tp4-mtp1-4352-ple-only-a312-fullgraphdet-w13n32.sh"
expected_wrapper=734acb9eb3acd207f48cb9a79ae4900fc04b3750ca2dde97dc13afca6938030d
client="${script_dir}/run-tp4-mtp1-4352-ple-only-a312-fullgraphdet-w13n32-client.sh"
expected_client=7abc16783fab1e0d13df45ffc86c9ad724970e16192308aa0e9b3b2284022d72
state=/tmp/q38-mtp1-ple-only-a312
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt312
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt312
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt312-compile
rpc_dir=/tmp/q38-ple2k-a312-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt312-supervisor
port=19982
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A312 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A312 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A312_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A312_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A312_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A312 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A312 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

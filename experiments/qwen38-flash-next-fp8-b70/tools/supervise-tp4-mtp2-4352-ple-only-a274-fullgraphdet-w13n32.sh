...
wrapper="${script_dir}/launch-tp4-mtp2-4352-ple-only-a274-fullgraphdet-w13n32.sh"
expected_wrapper=1743eaf3e356ac14a812bebc97712035c9e68a56f5a581a24cf1479cff083403
client="${script_dir}/run-tp4-mtp2-4352-ple-only-a274-fullgraphdet-w13n32-client.sh"
expected_client=8448887a03d0aeb70c184abed120b2d5961c4bac07e99d3528ce3ebb5157a07e
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp2-4352-ple-only-r1-attempt274
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp2-4352-ple-only-r1-attempt274-compile
rpc_dir=/tmp/q38-ple2k-a274-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp2-4352-ple-only-r1-attempt274-supervisor
port=19943
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A274 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A274 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A274_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A274_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A274_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A274 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A274 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

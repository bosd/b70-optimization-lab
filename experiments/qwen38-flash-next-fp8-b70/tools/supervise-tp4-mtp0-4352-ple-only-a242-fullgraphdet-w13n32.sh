...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a242-fullgraphdet-w13n32.sh"
expected_wrapper=983a4b451d190b37a21b9f6770eb147660747c68e422f3ffb9b8fd90aae9e6b3
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a242-fullgraphdet-w13n32-client.sh"
expected_client=332ac4ab0fe79f1d58ae924cedb811c78a1e0de701ab0f363e0b5fa9194c2431
state=/tmp/q38-mtp0-ple-only-a242
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt242
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt242
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt242-compile
rpc_dir=/tmp/q38-ple2k-a242-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt242-supervisor
port=19912
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A242 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A242 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A242_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A242_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A242_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A242 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A242 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

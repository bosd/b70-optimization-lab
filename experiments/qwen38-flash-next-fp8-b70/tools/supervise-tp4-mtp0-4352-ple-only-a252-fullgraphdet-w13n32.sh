...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a252-fullgraphdet-w13n32.sh"
expected_wrapper=2a9496df60c100ceccc6d060e78a59cf63c47ffc84d8a12831a530be6fafddd0
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a252-fullgraphdet-w13n32-client.sh"
expected_client=d0f56c1d3a39b7342db910e35d31ae6f12d258b2cf48025ef8565207aa199b92
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt252
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt252-compile
rpc_dir=/tmp/q38-ple2k-a252-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt252-supervisor
port=19918
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A252 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A252 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A252_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A252_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A252_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A252 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A252 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

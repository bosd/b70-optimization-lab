...
wrapper="${script_dir}/launch-tp4-mtp1-4352-ple-only-a311-fullgraphdet-w13n32.sh"
expected_wrapper=08bf8ff673036d25028e5d9d1d30a5d0fe919d27f4988b864cb4b0e197422d73
client="${script_dir}/run-tp4-mtp1-4352-ple-only-a311-fullgraphdet-w13n32-client.sh"
expected_client=4c442ccbd036a0f20cf5464192fd0a3f89fdb73466d2e58f95e3272a611eca49
state=/tmp/q38-mtp1-ple-only-a311
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt311
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt311
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt311-compile
rpc_dir=/tmp/q38-ple2k-a311-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt311-supervisor
port=19979
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A311 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A311 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A311_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A311_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A311_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A311 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A311 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

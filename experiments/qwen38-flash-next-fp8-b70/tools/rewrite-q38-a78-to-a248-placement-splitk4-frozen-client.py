#!/usr/bin/env python3
"""Create the A248 packet from frozen A78 for the A213 identity: PLE + embeddings offloaded at 12.25 (12.22 GiB), hot experts resident, never-hit experts host-placed at load time (/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/data/20260906-q38-expert-host-placement-3p5gib-per-rank.json) on clean head 6a79c56d8a98; frozen-client certification battery."""
...
VALIDATE_ONLY = os.environ.get("Q38_A248_REWRITE_VALIDATE_ONLY") == "1"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp0-4352-ple-only-a78", "tp4-mtp0-4352-ple-only-a248")
        seg = seg.replace("attempt78", "attempt248").replace("19750", "19893")
        seg = seg.replace("ATTEMPT=78", "ATTEMPT=248").replace("a78", "a248").replace("A78", "A248")
        return seg
    parts=[]; last=0
...
def main():
    ...
    env = os.environ.copy(); env["Q38_A248_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a248-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a248" in derived
    ...
    out_names = ("launch-tp4-mtp0-4352-ple-only-a248-fullgraphdet-w13n32.sh", "run-tp4-mtp0-4352-ple-only-a248-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp0-4352-ple-only-a248-fullgraphdet-w13n32.sh", "run-q38-a248-host-controlled.sh")

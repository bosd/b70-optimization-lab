...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a327-fullgraphdet-w13n128.sh"
expected_wrapper=059cea612fc85553ce730da77f8878f17642faeee3eb50795506e63a06bb229e
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a327-fullgraphdet-w13n128-client.sh"
expected_client=e8f6b798eb66dc26deb47304a2ebc44326dc4babe835d77b8ee4bc06ef573d2d
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt327
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt327-compile
rpc_dir=/tmp/q38-ple2k-a327-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt327-supervisor
port=19997
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A327 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A327 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A327_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A327_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A327_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A327 host-pressure or NVMe-link guard'
  fi
...
.identity.max_model_len == 4352 and
         ...
         .identity.tuned_config_folder == "moe-m1-w13-n128" and
         .identity.tuned_config_map_sha256 == "8b67485c1ad13ea3b1b0ebdb06a6a2d0e18bd4b9058f44a2378c4a47d0f41315" and
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A327 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

...
#   q38-run-attempt-chain.sh <attempt> <port> <mtp0|mtp1|mtp2> <overlay-branch> <log-dir> [driver-command...]
# Without a driver command the exact-2K timing driver is used; with one (e.g. the packet's frozen
# client, or q38-suite-driver.sh with its arguments) that command is exec'd as the driver.
a=${1:?attempt}; port=${2:?port}; mtp=${3:?mtp}; br=${4:?branch}; L=${5:?log dir}; shift 5
...
mkdir -p "$L"; if [ $# -gt 0 ]; then printf '#!/usr/bin/env bash\nexec %q' "$1" > "$L/a$a-driver.sh"; shift; for x in "$@"; do printf ' %q' "$x" >> "$L/a$a-driver.sh"; done; printf '\n' >> "$L/a$a-driver.sh"; else printf '#!/usr/bin/env bash\nexec %s/q38-timing-driver.sh %s %s %s\n' "$E/tools" "$a" "$port" "$mtp" > "$L/a$a-driver.sh"; fi; chmod +x "$L/a$a-driver.sh"

...
wrapper="${script_dir}/launch-tp4-mtp1-4352-ple-only-a250-fullgraphdet-w13n32.sh"
expected_wrapper=750cee901af382a5bc867c84e2e72d9008d4568543e3af4a00fae1ec8ff457b6
client="${script_dir}/run-tp4-mtp1-4352-ple-only-a250-fullgraphdet-w13n32-client.sh"
expected_client=50b254ce9e269afa7b71a0c00e8492930618eb37cca7106efeb32a31bbe8d03a
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt250
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt250-compile
rpc_dir=/tmp/q38-ple2k-a250-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt250-supervisor
port=19895
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A250 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A250 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A250_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A250_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A250_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A250 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A250 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

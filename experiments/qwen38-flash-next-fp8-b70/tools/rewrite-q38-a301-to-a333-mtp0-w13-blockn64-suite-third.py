...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp0-4352-ple-only-a301", "tp4-mtp0-4352-ple-only-a333")
        seg = seg.replace("attempt301", "attempt333").replace("19970", "19946")
        seg = seg.replace("ATTEMPT=301", "ATTEMPT=333").replace("a301", "a333").replace("A301", "A333")
    ...
    assert "ATTEMPT=333" in out or "ATTEMPT=" not in out, "the attempt number did not move"
...
def main():
    ...
    env = os.environ.copy()
    env["Q38_A333_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a333-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a333" in derived
    ...
    out_names = (
        "launch-tp4-mtp0-4352-ple-only-a333-fullgraphdet-w13n64.sh",
        "run-tp4-mtp0-4352-ple-only-a333-fullgraphdet-w13n64-client.sh",
        "supervise-tp4-mtp0-4352-ple-only-a333-fullgraphdet-w13n64.sh",
        "run-q38-a333-host-controlled.sh",
    )

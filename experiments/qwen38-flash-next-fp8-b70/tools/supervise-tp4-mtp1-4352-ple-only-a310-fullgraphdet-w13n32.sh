...
wrapper="${script_dir}/launch-tp4-mtp1-4352-ple-only-a310-fullgraphdet-w13n32.sh"
expected_wrapper=9c35f7286dfcc4e7a4d36b3d6470f8032cf01a89afbf4beaba7d30884ff81dbb
client="${script_dir}/run-tp4-mtp1-4352-ple-only-a310-fullgraphdet-w13n32-client.sh"
expected_client=3102afc438bff876e206faccdb323266219fb3de706d5f270a62b3d67a026c9e
state=/tmp/q38-mtp1-ple-only-a310
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt310
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt310
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt310-compile
rpc_dir=/tmp/q38-ple2k-a310-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt310-supervisor
port=19978
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A310 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A310 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A310_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A310_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A310_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A310 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A310 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

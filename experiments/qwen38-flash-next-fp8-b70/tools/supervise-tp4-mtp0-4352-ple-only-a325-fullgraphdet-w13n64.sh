...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a325-fullgraphdet-w13n64.sh"
expected_wrapper=e06b17ed223035ef7dcd5b69ba0d09c502acd14dc11f65a066dcc2f1537c987b
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a325-fullgraphdet-w13n64-client.sh"
expected_client=d3d3727e8613f6678baaee73d609177805289a0e988e7fff435584f90c5f65c0
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt325
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt325-compile
rpc_dir=/tmp/q38-ple2k-a325-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt325-supervisor
port=19995
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A325 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A325 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A325_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A325_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A325_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A325 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A325 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

#!/usr/bin/env python3
"""Create the A291 packet from frozen A78: graph MTP0 exact-2K timing screen with PLE + embeddings offloaded at budget 12.25 (hot experts resident) and 3.5 GiB/rank of never-hit experts host-placed at load time on q38-placement-v5 c4f921f5."""
...
VALIDATE_ONLY = os.environ.get("Q38_A291_REWRITE_VALIDATE_ONLY") == "1"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp0-4352-ple-only-a78", "tp4-mtp0-4352-ple-only-a291")
        seg = seg.replace("attempt78", "attempt291").replace("19750", "19960")
        seg = seg.replace("ATTEMPT=78", "ATTEMPT=291").replace("a78", "a291").replace("A78", "A291")
        return seg
    parts=[]; last=0
...
def main():
    ...
    launcher = launcher.replace("moe-m1-w13-n32", "moe-m1-w13-n16").replace("a8f1f8982e3e1af80ff31b9e0a00afaacf1af1b3c401585109b4d60d3c8267be", "89c130d70a34432124b9cee1621080ea75f4b303c20bce784d13af82e4219894")  # W13 phase tile at BLOCK_N 16 (exact: the K order per output element is unchanged)
    launcher = replace_once(launcher, '\'."1".W1_CONFIG.BLOCK_SIZE_N\' "$tuned_config_map")" == 32', '\'."1".W1_CONFIG.BLOCK_SIZE_N\' "$tuned_config_map")" == 16')  # the shape guard pins the W13 phase tile
    ...
    env = os.environ.copy(); env["Q38_A291_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a291-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a291" in derived
    ...
    out_names = ("launch-tp4-mtp0-4352-ple-only-a291-fullgraphdet-w13n32.sh", "run-tp4-mtp0-4352-ple-only-a291-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp0-4352-ple-only-a291-fullgraphdet-w13n32.sh", "run-q38-a291-host-controlled.sh")

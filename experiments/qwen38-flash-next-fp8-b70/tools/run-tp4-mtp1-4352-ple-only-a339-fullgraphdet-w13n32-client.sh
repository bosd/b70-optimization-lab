...
supervisor="${repo}/experiments/qwen38-flash-next-fp8-b70/tools/supervise-tp4-mtp1-4352-ple-only-a339-fullgraphdet-w13n32.sh"
state=/tmp/q38-mtp1-ple-only-a339
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt339
base_url=http://127.0.0.1:19952
...
torchinductor_cache=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt339-compile/torchinductor
...
[[ "$supervisor_command" == *"supervise-tp4-mtp1-4352-ple-only-a339-fullgraphdet-w13n32.sh"* ]] || {
...
grep -zFxq 'VLLM_TUNED_CONFIG_FOLDER=/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/configs/moe-m1-w13-n32' "/proc/${server_pid}/environ" || {
  printf 'FAIL: live server lacks the exact A339 tuned M1 map folder\n' >&2
  exit 1
}
...
[[ "$(sha256sum '/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/configs/moe-m1-w13-n32/E=128,N=640,device_name=Intel(R)_Arc(TM)_Pro_B70_Graphics,dtype=fp8_w8a8,block_shape=[128,128].json' | cut -d' ' -f1)" == a8f1f8982e3e1af80ff31b9e0a00afaacf1af1b3c401585109b4d60d3c8267be ]] || {
  printf 'FAIL: A339 tuned M1 map drifted before client work\n' >&2
  exit 1
}
...
[[ "$server_command" == *"vllm serve /mnt/usb-models/llm-models/Qwen3.8-Flash-Next-FP8"* && \
   "$server_command" == *"--port 19952"* && "$server_command" == *"--max-model-len 4352"* ]] || {
...
[[ "$server_command" != *"--enforce-eager"* && "$server_command" == *"--cudagraph-metrics"* && \
  ...
  printf 'FAIL: frozen A339 graph command identity mismatch\n' >&2
  exit 1
}
for receipt in \
...
PY
journal_start=$(cat "/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt339-supervisor/journal-start-epoch.txt")
...
request = urllib.request.Request(
    ...
    headers={"Content-Type": "application/json", "X-Request-Id": "q38-ple-only-a339-recovery-canary"},
    method="POST")
...
timeout --signal=TERM --kill-after=10s 1200s "$python" "$quality" \
  ...
  --seed 20260609 --repeat-runs 16 --request-id-prefix q38-ple-only-a339 \

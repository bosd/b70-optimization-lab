#!/usr/bin/env python3
"""Create the A323 suite run from frozen A301 -- the packet behind the published MTP0\nfigure of 33.797067 tok/s -- with the W13 phase delta neutralised to 64.\n\nA321 and A322 measure the change on exact-depth rows; the published record is a\nrealistic-suite number (tok_s_1_100_intervals_after_ttft, median of twelve rows), so the\nrecord cannot be restated from those rows alone. A323 runs the same suite on the same\nhead with the same one-line map change, so its median is directly comparable to A301's.\n\nNo source change: 64 is already an allowed tile, so the head stays the certified\n2a372e86 and only the map moves."""
from __future__ import annotations
import hashlib, os, re, subprocess
from pathlib import Path
ROOT = Path(__file__).resolve().parent
VALIDATE_ONLY = os.environ.get("Q38_A323_REWRITE_VALIDATE_ONLY") == "1"
SOURCES = {
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp0-4352-ple-only-a301", "tp4-mtp0-4352-ple-only-a323")
        seg = seg.replace("attempt301", "attempt323").replace("19970", "19993")
        seg = seg.replace("ATTEMPT=301", "ATTEMPT=323").replace("a301", "a323").replace("A301", "A323")
...
def main():
    ...
    env = os.environ.copy()
    env["Q38_A323_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a323-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a323" in derived
    ...
    out_names = (
        "launch-tp4-mtp0-4352-ple-only-a323-fullgraphdet-w13n64.sh",
        "run-tp4-mtp0-4352-ple-only-a323-fullgraphdet-w13n64-client.sh",
        "supervise-tp4-mtp0-4352-ple-only-a323-fullgraphdet-w13n64.sh",
        "run-q38-a323-host-controlled.sh",
    )

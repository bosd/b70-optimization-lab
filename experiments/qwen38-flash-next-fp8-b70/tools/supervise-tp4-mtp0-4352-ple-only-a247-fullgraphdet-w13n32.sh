...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a247-fullgraphdet-w13n32.sh"
expected_wrapper=289f665f43d1dbc258910fb2fdb41806d0ac9290edbb10770b6da75ea4b432f5
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a247-fullgraphdet-w13n32-client.sh"
expected_client=4e492d2980458ceb4b3ef76de9c917e98ca9de915fc5a99b38510dc0fcf5773d
state=/tmp/q38-mtp0-ple-only-a247
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt247
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt247
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt247-compile
rpc_dir=/tmp/q38-ple2k-a247-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt247-supervisor
port=19917
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A247 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A247 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A247_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A247_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A247_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A247 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A247 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

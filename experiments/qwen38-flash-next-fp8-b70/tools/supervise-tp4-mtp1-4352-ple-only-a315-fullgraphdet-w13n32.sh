...
wrapper="${script_dir}/launch-tp4-mtp1-4352-ple-only-a315-fullgraphdet-w13n32.sh"
expected_wrapper=a1bceee189e16086006df22118989986e09a3a141fb3d10fe38419b8539aaf5b
client="${script_dir}/run-tp4-mtp1-4352-ple-only-a315-fullgraphdet-w13n32-client.sh"
expected_client=2b387d15e0234031c08f118d9768888c4450a09b96275182cc1467abf404f2ee
state=/tmp/q38-mtp1-ple-only-a315
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt315
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt315
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt315-compile
rpc_dir=/tmp/q38-ple2k-a315-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt315-supervisor
port=19985
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A315 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A315 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A315_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A315_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A315_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A315 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A315 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

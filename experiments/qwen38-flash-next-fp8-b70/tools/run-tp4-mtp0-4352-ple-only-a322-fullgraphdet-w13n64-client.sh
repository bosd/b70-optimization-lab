...
supervisor="${repo}/experiments/qwen38-flash-next-fp8-b70/tools/supervise-tp4-mtp0-4352-ple-only-a322-fullgraphdet-w13n64.sh"
state=/tmp/q38-mtp0-ple-only-a322
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt322
...
torchinductor_cache=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt322-compile/torchinductor
...
[[ "$supervisor_command" == *"supervise-tp4-mtp0-4352-ple-only-a322-fullgraphdet-w13n64.sh"* ]] || {
...
grep -zFxq 'VLLM_TUNED_CONFIG_FOLDER=/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/configs/moe-m1-w13-n64' "/proc/${server_pid}/environ" || {
  printf 'FAIL: live server lacks the exact A322 tuned M1 map folder\n' >&2
  exit 1
}
...
[[ "$(sha256sum '/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/configs/moe-m1-w13-n64/E=128,N=640,device_name=Intel(R)_Arc(TM)_Pro_B70_Graphics,dtype=fp8_w8a8,block_shape=[128,128].json' | cut -d' ' -f1)" == 4fcb5d13ef0c859d12a4fe6b5aac09b04fccf4db24e47a6f004d3f9878e4e38f ]] || {
  printf 'FAIL: A322 tuned M1 map drifted before client work\n' >&2
  exit 1
}
[[ "$(sha256sum "${repo}/experiments/qwen38-flash-next-fp8-b70/tools/verify-moe-m1-w13-n64-selection.py" | cut -d' ' -f1)" == f7b3e9340e26026ac7998256384ddf7c40ad43a06c1717dc52fba02e284045c9 ]] || {
  printf 'FAIL: W13-N64 selection verifier drifted\n' >&2
  exit 1
}
...
env PYTHONPATH=/mnt/usb-models/qwen38-build/runtime-core-moe-negidguard-b70:/home/steve/src/vllm-current-main \
  ...
  VLLM_TUNED_CONFIG_FOLDER=/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/configs/moe-m1-w13-n64 \
  "$python" "${repo}/experiments/qwen38-flash-next-fp8-b70/tools/verify-moe-m1-w13-n64-selection.py" \
  ...
  --candidate-config-file '/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/configs/moe-m1-w13-n64/E=128,N=640,device_name=Intel(R)_Arc(TM)_Pro_B70_Graphics,dtype=fp8_w8a8,block_shape=[128,128].json' \
  ...
  --output "${run_dir}/moe-m1-w13-n64-selection-receipt.json" || {
  printf 'FAIL: official W13-N64 resolver receipt failed\n' >&2
  exit 1
}
jq -e '.status == "pass" and .selected_batch_key == 1 and
  .m1.w13.BLOCK_SIZE_N == 64 and .m1.w13.num_warps == 8 and
  .m1.w2.BLOCK_SIZE_N == 64 and .m1.w2.num_warps == 8 and
  .config.candidate_sha256 == "4fcb5d13ef0c859d12a4fe6b5aac09b04fccf4db24e47a6f004d3f9878e4e38f" and
  .preservation.all_integer_m_2_through_512_match_retained_map == true' \
  "${run_dir}/moe-m1-w13-n64-selection-receipt.json" >/dev/null || {
  printf 'FAIL: W13-N64 resolver receipt did not prove key 1 W13-N64 / W2-N64\n' >&2
  exit 1
}
...
[[ "$server_command" == *"vllm serve /mnt/usb-models/llm-models/Qwen3.8-Flash-Next-FP8"* && \
   "$server_command" == *"--port 19992"* && "$server_command" == *"--max-model-len 4352"* ]] || {
...
[[ "$server_command" != *"--enforce-eager"* && "$server_command" == *"--cudagraph-metrics"* && \
  ...
  printf 'FAIL: frozen A322 graph command identity mismatch\n' >&2
  exit 1
}
for receipt in \
  ...
  'tuned_config_folder=moe-m1-w13-n64' 'tuned_config_map_sha256=4fcb5d13ef0c859d12a4fe6b5aac09b04fccf4db24e47a6f004d3f9878e4e38f' \
...
PY
journal_start=$(cat "/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt322-supervisor/journal-start-epoch.txt")
...
request = urllib.request.Request(
    ...
    headers={"Content-Type": "application/json", "X-Request-Id": "q38-ple-only-a322-recovery-canary"},
    method="POST")
...
timeout --signal=TERM --kill-after=10s 1200s "$python" "$quality" \
  ...
  --seed 20260609 --repeat-runs 16 --request-id-prefix q38-ple-only-a322 \
...
summary = {
    ...
    "identity": {
        ...
        "ccl_sycl_allreduce_ll": "twoshots",
        "tuned_config_folder": "moe-m1-w13-n64",
        "tuned_config_map_sha256": "4fcb5d13ef0c859d12a4fe6b5aac09b04fccf4db24e47a6f004d3f9878e4e38f",

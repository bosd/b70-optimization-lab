#!/usr/bin/env python3
"""Create the A287 step and per-layer timing packet from frozen A83 (eager MTP0 twin of A122 with the per-step timing log, no trace).
...
d132de8c). Attempt 163 / port 19956.
"""
...
VALIDATE_ONLY = os.environ.get("Q38_A287_REWRITE_VALIDATE_ONLY") == "1"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp1-4352-ple-only-a83", "tp4-mtp0-4352-ple-only-a287")
        ...
        seg = seg.replace("attempt83", "attempt287").replace("19755", "19956")
        seg = seg.replace("ATTEMPT=83", "ATTEMPT=287").replace("a83", "a287").replace("A83", "A287")
        return seg
    parts=[]; last=0
...
def main():
    ...
    launcher = launcher.replace("moe-m1-w13-n32", "moe-m1-w13-n32-bn32").replace("a8f1f8982e3e1af80ff31b9e0a00afaacf1af1b3c401585109b4d60d3c8267be", "efb3b0fe65d25412eb6a13babf19306139d729818861ebe3f02b567b80681aca")  # M=1 entry at 16 warps
    launcher = _fm.patch_launcher(launcher)
    if MTP0:
        launcher = replace_once(launcher, MTP_RULE, "")
        launcher = replace_once(launcher, MTP_GREPS, "")
        launcher = replace_once(launcher, "export MTP=1 MTP_EXACT=0 MAX_MODEL_LEN=4352 ATTEMPT=287 PORT=19956\n", "export MTP=0 MTP_EXACT=0 MAX_MODEL_LEN=4352 ATTEMPT=287 PORT=19956\n")
        ...
        launcher = replace_once(launcher, "export MTP=1 MTP_EXACT=0 MAX_MODEL_LEN=4352 ATTEMPT=287 PORT=19956\n", "export MTP=1 MTP_EXACT=1 MAX_MODEL_LEN=4352 ATTEMPT=287 PORT=19956\n")
    ...
    env = os.environ.copy(); env["Q38_A287_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a287-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a287" in derived and "  --enforce-eager\n" in derived
    ...
    names = ("launch-tp4-mtp0-4352-ple-only-a287-mkldnndet-w13n32.sh", "run-tp4-mtp0-4352-ple-only-a287-mkldnndet-w13n32-client.sh", "supervise-tp4-mtp0-4352-ple-only-a287-mkldnndet-w13n32.sh", "run-q38-a287-host-controlled.sh")

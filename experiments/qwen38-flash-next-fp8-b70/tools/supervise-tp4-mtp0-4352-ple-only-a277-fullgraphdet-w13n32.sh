...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a277-fullgraphdet-w13n32.sh"
expected_wrapper=1b03f4b59eb3e30377667f4838dbecc495650d0f6df01b10c59c5e9814669f3f
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a277-fullgraphdet-w13n32-client.sh"
expected_client=30d3d3df9fb770e9510a79e992b9540583425899a36a21d2cafc5a97764ebbb6
state=/tmp/q38-mtp0-ple-only-a277
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt277
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt277
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt277-compile
rpc_dir=/tmp/q38-ple2k-a277-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt277-supervisor
port=19946
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A277 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A277 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A277_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A277_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A277_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A277 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A277 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

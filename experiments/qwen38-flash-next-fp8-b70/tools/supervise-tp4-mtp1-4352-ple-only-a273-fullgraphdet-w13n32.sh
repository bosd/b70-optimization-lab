...
wrapper="${script_dir}/launch-tp4-mtp1-4352-ple-only-a273-fullgraphdet-w13n32.sh"
expected_wrapper=d0f7a30e8fde69b208bc5bb484825837ac4486cf780d767f2ff2a13227d0717b
client="${script_dir}/run-tp4-mtp1-4352-ple-only-a273-fullgraphdet-w13n32-client.sh"
expected_client=35aa9f7287156324fc3eafe53d82448bc4b3b27df0f251e1f9cec3e36ab8511c
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt273
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt273-compile
rpc_dir=/tmp/q38-ple2k-a273-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt273-supervisor
port=19900
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A273 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A273 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A273_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A273_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A273_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A273 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A273 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

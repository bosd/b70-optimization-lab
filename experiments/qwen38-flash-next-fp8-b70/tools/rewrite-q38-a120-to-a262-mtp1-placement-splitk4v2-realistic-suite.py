#!/usr/bin/env python3
"""Create the A262 packet from frozen A120 (MTP1) for the A213 identity: PLE + embeddings offloaded at 12.25 (12.22 GiB), hot experts resident, never-hit experts host-placed at load time (/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/data/20260906-q38-expert-host-placement-3p5gib-per-rank.json) on clean head 893e1ccc7a1e; realistic suite (LocalMaxxing metric)."""
from __future__ import annotations
import hashlib, os, re, subprocess
from pathlib import Path
ROOT = Path(__file__).resolve().parent
VALIDATE_ONLY = os.environ.get("Q38_A262_REWRITE_VALIDATE_ONLY") == "1"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp1-4352-ple-only-a120", "tp4-mtp1-4352-ple-only-a262")
        seg = seg.replace("attempt120", "attempt262").replace("19792", "19928")
        seg = seg.replace("ATTEMPT=120", "ATTEMPT=262").replace("a120", "a262").replace("A120", "A262")
        return seg
    parts=[]; last=0
...
def main():
    ...
    launcher = patch_a262_launcher(launcher)
    ...
    env = os.environ.copy(); env["Q38_A262_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a262-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a262" in derived
    ...
    client = patch_a262_client(client)
    ...
    supervisor = patch_a262_supervisor(supervisor)
    ...
    out_names = ("launch-tp4-mtp1-4352-ple-only-a262-fullgraphdet-w13n32.sh", "run-tp4-mtp1-4352-ple-only-a262-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp1-4352-ple-only-a262-fullgraphdet-w13n32.sh", "run-q38-a262-host-controlled.sh")

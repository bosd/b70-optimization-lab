...
wrapper="${script_dir}/launch-tp4-mtp1-4352-ple-only-a305-fullgraphdet-w13n32.sh"
expected_wrapper=40abf013e0bed5c8f240bddb4e49df09cef53459fa901ecfddf25846a1d670c3
client="${script_dir}/run-tp4-mtp1-4352-ple-only-a305-fullgraphdet-w13n32-client.sh"
expected_client=28c5b11cbc75ae39092282e7c6535208391ea68b521f2c02ebe4e0fd72f54ed2
state=/tmp/q38-mtp1-ple-only-a305
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt305
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt305
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt305-compile
rpc_dir=/tmp/q38-ple2k-a305-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt305-supervisor
port=19974
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A305 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A305 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A305_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A305_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A305_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A305 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A305 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

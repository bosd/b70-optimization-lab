#!/usr/bin/env python3
"""Create the A310 packet from frozen A120 (MTP1) for the A213 identity: PLE + embeddings offloaded at 12.25 (12.22 GiB), hot experts resident, never-hit experts host-placed at load time (/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/data/20260906-q38-expert-host-placement-3p5gib-per-rank.json) on clean head 6d8724577dab; frozen-client certification battery."""
from __future__ import annotations
import hashlib, os, re, subprocess
from pathlib import Path
ROOT = Path(__file__).resolve().parent
VALIDATE_ONLY = os.environ.get("Q38_A310_REWRITE_VALIDATE_ONLY") == "1"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp1-4352-ple-only-a120", "tp4-mtp1-4352-ple-only-a310")
        seg = seg.replace("attempt120", "attempt310").replace("19792", "19978")
        seg = seg.replace("ATTEMPT=120", "ATTEMPT=310").replace("a120", "a310").replace("A120", "A310")
        return seg
    parts=[]; last=0
...
def main():
    ...
    launcher = patch_a310_launcher(launcher)
    ...
    env = os.environ.copy(); env["Q38_A310_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a310-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a310" in derived
    launcher = launcher.replace("expected_derived=" + "0"*64, "expected_derived=" + digest(derived))
    client = successor(source("run-tp4-mtp1-4352-ple-only-a120-fullgraphdet-w13n32-client.sh"))
    client = patch_a310_client(client)
    ...
    supervisor = patch_a310_supervisor(supervisor)
    ...
    out_names = ("launch-tp4-mtp1-4352-ple-only-a310-fullgraphdet-w13n32.sh", "run-tp4-mtp1-4352-ple-only-a310-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp1-4352-ple-only-a310-fullgraphdet-w13n32.sh", "run-q38-a310-host-controlled.sh")

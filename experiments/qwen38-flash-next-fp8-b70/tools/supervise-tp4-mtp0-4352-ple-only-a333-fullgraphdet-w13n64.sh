...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a333-fullgraphdet-w13n64.sh"
expected_wrapper=de22261ed80be20965ee36a1294dd1ffdbfceb83d45a1d233f92314ab1b9caa6
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a333-fullgraphdet-w13n64-client.sh"
expected_client=1b7d2468986e09b37a10f3f5f6f16eb5c018cd1a7eee10fa011a63093362ba37
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt333
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt333-compile
rpc_dir=/tmp/q38-ple2k-a333-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt333-supervisor
port=19946
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A333 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A333 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A333_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A333_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A333_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A333 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A333 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

#!/usr/bin/env python3
"""Create the A264 packet from frozen A78 for the A213 identity: PLE + embeddings offloaded at 12.25 (12.22 GiB), hot experts resident, never-hit experts host-placed at load time (/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/data/20260906-q38-expert-host-placement-3p5gib-per-rank.json) on clean head 2aa369a6c069; frozen-client certification battery."""
...
VALIDATE_ONLY = os.environ.get("Q38_A264_REWRITE_VALIDATE_ONLY") == "1"
...
NEW_HEAD = "2aa369a6c069e64579c62905e0cfee8b456d8cc3"  # q38-placement-clean-v5: promoted overlay + table kernel + load-time never-hit expert placement
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp0-4352-ple-only-a78", "tp4-mtp0-4352-ple-only-a264")
        seg = seg.replace("attempt78", "attempt264").replace("19750", "19930")
        seg = seg.replace("ATTEMPT=78", "ATTEMPT=264").replace("a78", "a264").replace("A78", "A264")
        return seg
    parts=[]; last=0
...
def main():
    ...
    env = os.environ.copy(); env["Q38_A264_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a264-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a264" in derived
    ...
    out_names = ("launch-tp4-mtp0-4352-ple-only-a264-fullgraphdet-w13n32.sh", "run-tp4-mtp0-4352-ple-only-a264-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp0-4352-ple-only-a264-fullgraphdet-w13n32.sh", "run-q38-a264-host-controlled.sh")

#!/usr/bin/env python3
"""Create the A244 packet from frozen A78: graph MTP0 exact-2K timing screen with PLE + embeddings offloaded at budget 12.25 (hot experts resident) and 3.5 GiB/rank of never-hit experts host-placed at load time on q38-placement-v5 c4f921f5."""
...
VALIDATE_ONLY = os.environ.get("Q38_A244_REWRITE_VALIDATE_ONLY") == "1"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp0-4352-ple-only-a78", "tp4-mtp0-4352-ple-only-a244")
        seg = seg.replace("attempt78", "attempt244").replace("19750", "19914")
        seg = seg.replace("ATTEMPT=78", "ATTEMPT=244").replace("a78", "a244").replace("A78", "A244")
        return seg
    parts=[]; last=0
...
def main():
    ...
    env = os.environ.copy(); env["Q38_A244_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a244-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a244" in derived
    ...
    out_names = ("launch-tp4-mtp0-4352-ple-only-a244-fullgraphdet-w13n32.sh", "run-tp4-mtp0-4352-ple-only-a244-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp0-4352-ple-only-a244-fullgraphdet-w13n32.sh", "run-q38-a244-host-controlled.sh")

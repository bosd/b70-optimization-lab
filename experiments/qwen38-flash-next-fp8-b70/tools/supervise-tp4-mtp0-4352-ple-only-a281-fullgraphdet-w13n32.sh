...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a281-fullgraphdet-w13n32.sh"
expected_wrapper=5b7fc6d006e370c550bf91409b768ab8b151c7082381c35c41f3d58b24d4ee5a
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a281-fullgraphdet-w13n32-client.sh"
expected_client=8831edd7f69eb68e334ce5fa05bb8a1733a478fc9ce02795530410e75e1fbca3
state=/tmp/q38-mtp0-ple-only-a281
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt281
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt281
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt281-compile
rpc_dir=/tmp/q38-ple2k-a281-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt281-supervisor
port=19950
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A281 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A281 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A281_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A281_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A281_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A281 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A281 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

#!/usr/bin/env python3
"""Create the A315 packet from frozen A311: the same fused-QSA MTP1 identity and the same
3.5 GiB-per-rank expert host placement, with a full per-expert routing census on head
cc653214 (per-rank JSON via Q38_PLACEMENT_HIT_CENSUS_OUT). A311 proved parked experts are
...
VALIDATE_ONLY = os.environ.get("Q38_A315_REWRITE_VALIDATE_ONLY") == "1"
SOURCES = {
...
NEW_HEAD = "cc653214c7fa94689f029c8a786cacd801ce19f8"  # q38-placement-hit-census-v2: full per-expert routing histogram + per-rank dump
CENSUS_OUT = "/tmp/q38-a315-census"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp1-4352-ple-only-a311", "tp4-mtp1-4352-ple-only-a315")
        seg = seg.replace("attempt311", "attempt315").replace("19979", "19985")
        seg = seg.replace("ATTEMPT=311", "ATTEMPT=315").replace("a311", "a315").replace("A311", "A315")
        return seg
    parts = []
    last = 0
...
def main():
    ...
    env = os.environ.copy()
    env["Q38_A315_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a315-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a315" in derived
    ...
    out_names = (
        "launch-tp4-mtp1-4352-ple-only-a315-fullgraphdet-w13n32.sh",
        "run-tp4-mtp1-4352-ple-only-a315-fullgraphdet-w13n32-client.sh",
        "supervise-tp4-mtp1-4352-ple-only-a315-fullgraphdet-w13n32.sh",
        "run-q38-a315-host-controlled.sh",
    )

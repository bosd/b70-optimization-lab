#!/usr/bin/env python3
"""Create the A271 packet from frozen A120 (MTP1) for the A213 identity: PLE + embeddings offloaded at 12.25 (12.22 GiB), hot experts resident, never-hit experts host-placed at load time (/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/data/20260906-q38-expert-host-placement-3p5gib-per-rank.json) on clean head 622191221475; frozen-client certification battery."""
from __future__ import annotations
import hashlib, os, re, subprocess
from pathlib import Path
ROOT = Path(__file__).resolve().parent
VALIDATE_ONLY = os.environ.get("Q38_A271_REWRITE_VALIDATE_ONLY") == "1"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp1-4352-ple-only-a120", "tp4-mtp1-4352-ple-only-a271")
        seg = seg.replace("attempt120", "attempt271").replace("19792", "19941")
        seg = seg.replace("ATTEMPT=120", "ATTEMPT=271").replace("a120", "a271").replace("A120", "A271")
        return seg
    parts=[]; last=0
...
NEW_HEAD = "622191221475b53cc6f7f4d847860939f4c300ab"  # q38-placement-mtp1-clean: lossless MTP1 head + table kernel + load-time never-hit expert placement
def patch_a271_launcher(l):
...
def patch_a271_client(c):
    ...
    if '"placement": "ple_only_uva", "ple_host_bytes_per_rank": 12800061440,' in c:
        c = replace_once(c, '"placement": "ple_only_uva", "ple_host_bytes_per_rank": 12800061440,', '"placement": "ple_embed_budget12p25_uva_cold_expert_host_placement_hctriton", "ple_host_bytes_per_rank": 12800061440, "host_offload_bytes_per_rank": 13117911040, "host_offload_params": "ple_embedding.ngram_embedding.weight,embed_tokens.weight", "expert_host_placement": "/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/data/20260906-q38-expert-host-placement-3p5gib-per-rank.json",')
...
def main():
    ...
    launcher = patch_a271_launcher(launcher)
    ...
    env = os.environ.copy(); env["Q38_A271_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a271-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a271" in derived
    launcher = launcher.replace("expected_derived=" + "0"*64, "expected_derived=" + digest(derived))
    client = successor(source("run-tp4-mtp1-4352-ple-only-a120-fullgraphdet-w13n32-client.sh"))
    client = patch_a271_client(client)
    client = client.replace(OLD_HEAD, NEW_HEAD)
    assert client.count("afffd2110812762164862b6388f054bb56696ee57b07eadce411a702c40bc714") == 1 and client.count("c6193cc6c9a1553f56d7ce78faea9c8bfa628a67fcea229b1c99279a149f6639") == 1
    client = client.replace("afffd2110812762164862b6388f054bb56696ee57b07eadce411a702c40bc714", "86b5b6c71903a79fe915a3fc43765af845080314a58539812db7376b40292b8d").replace("c6193cc6c9a1553f56d7ce78faea9c8bfa628a67fcea229b1c99279a149f6639", "b89822ce3b8a2719e0824d0e49222d535c8bd9fd86bc58feebd3cddaa9477b7c")  # Triton-HC new-authority exact-depth hashes (A266/A267 pair)
    client = client.replace("0bd36f13056d79924e7598bf8d844db3a5b8b35639737c0ef0b5af68cad14753", "20546ff1b349ce5d969d29855ed9a95b38c5be2d22084b6cb4326bac4a7e8849").replace("4f4942289f3853f0dec60b9fcd14c644ca300abaaa9d9fa2ea56135f4d9f9c52", "20546ff1b349ce5d969d29855ed9a95b38c5be2d22084b6cb4326bac4a7e8849")
    assert client.count("20546ff1b349ce5d969d29855ed9a95b38c5be2d22084b6cb4326bac4a7e8849") >= 1
    supervisor = successor(source("supervise-tp4-mtp1-4352-ple-only-a120-fullgraphdet-w13n32.sh"))
    supervisor = patch_a271_supervisor(supervisor)
    ...
    out_names = ("launch-tp4-mtp1-4352-ple-only-a271-fullgraphdet-w13n32.sh", "run-tp4-mtp1-4352-ple-only-a271-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp1-4352-ple-only-a271-fullgraphdet-w13n32.sh", "run-q38-a271-host-controlled.sh")

...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a336-fullgraphdet-w13n64.sh"
expected_wrapper=cb48ec0507f78d618ee60d2dbb61a29ce027041fefd5b2cd292cf3458eba9240
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a336-fullgraphdet-w13n64-client.sh"
expected_client=e681c5eaf4dbbfc4de37b1c848581ae9a01d8d1f900bd6eefdbdc9377844139a
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt336
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt336-compile
rpc_dir=/tmp/q38-ple2k-a336-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt336-supervisor
port=19949
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A336 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A336 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A336_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A336_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A336_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A336 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A336 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

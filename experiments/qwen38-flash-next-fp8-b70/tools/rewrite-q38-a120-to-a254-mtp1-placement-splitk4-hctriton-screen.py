#!/usr/bin/env python3
"""Create the A254 packet from frozen A120 (MTP1) for the A213 identity: PLE + embeddings offloaded at 12.25 (12.22 GiB), hot experts resident, never-hit experts host-placed at load time (/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/data/20260906-q38-expert-host-placement-3p5gib-per-rank.json) on clean head f57d40d75986; frozen-client certification battery."""
from __future__ import annotations
import hashlib, os, re, subprocess
from pathlib import Path
ROOT = Path(__file__).resolve().parent
VALIDATE_ONLY = os.environ.get("Q38_A254_REWRITE_VALIDATE_ONLY") == "1"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp1-4352-ple-only-a120", "tp4-mtp1-4352-ple-only-a254")
        seg = seg.replace("attempt120", "attempt254").replace("19792", "19920")
        seg = seg.replace("ATTEMPT=120", "ATTEMPT=254").replace("a120", "a254").replace("A120", "A254")
        return seg
    parts=[]; last=0
...
NEW_HEAD = "f57d40d759861ed0943316756efd9a7f69aaa7a4"  # q38-placement-mtp1-clean: lossless MTP1 head + table kernel + load-time never-hit expert placement
def patch_a254_launcher(l):
...
def patch_a254_client(c):
    ...
    if '"placement": "ple_only_uva", "ple_host_bytes_per_rank": 12800061440,' in c:
        c = replace_once(c, '"placement": "ple_only_uva", "ple_host_bytes_per_rank": 12800061440,', '"placement": "ple_embed_budget12p25_uva_cold_expert_host_placement_splitk4_hctriton", "ple_host_bytes_per_rank": 12800061440, "host_offload_bytes_per_rank": 13117911040, "host_offload_params": "ple_embedding.ngram_embedding.weight,embed_tokens.weight", "expert_host_placement": "/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/data/20260906-q38-expert-host-placement-3p5gib-per-rank.json",')
...
def main():
    ...
    env = os.environ.copy(); env["Q38_A254_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a254-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a254" in derived
    launcher = launcher.replace("expected_derived=" + "0"*64, "expected_derived=" + digest(derived))
    client = successor(source("run-tp4-mtp1-4352-ple-only-a120-fullgraphdet-w13n32-client.sh"))
    client = patch_a254_client(client)
    client = client.replace(OLD_HEAD, NEW_HEAD)
    client = client.replace("0bd36f13056d79924e7598bf8d844db3a5b8b35639737c0ef0b5af68cad14753", "dd58fde9d6ea054819698dd77e3f05ce8225a395b1842ebe03cc04f43bfe7dad").replace("4f4942289f3853f0dec60b9fcd14c644ca300abaaa9d9fa2ea56135f4d9f9c52", "dd58fde9d6ea054819698dd77e3f05ce8225a395b1842ebe03cc04f43bfe7dad")
    assert client.count("dd58fde9d6ea054819698dd77e3f05ce8225a395b1842ebe03cc04f43bfe7dad") >= 1
    supervisor = successor(source("supervise-tp4-mtp1-4352-ple-only-a120-fullgraphdet-w13n32.sh"))
    supervisor = patch_a254_supervisor(supervisor)
    ...
    out_names = ("launch-tp4-mtp1-4352-ple-only-a254-fullgraphdet-w13n32.sh", "run-tp4-mtp1-4352-ple-only-a254-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp1-4352-ple-only-a254-fullgraphdet-w13n32.sh", "run-q38-a254-host-controlled.sh")

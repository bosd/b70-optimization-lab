...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a322-fullgraphdet-w13n64.sh"
expected_wrapper=a5e9a843b72692ac39a375169712d05ae5bfff45153bdb77aa823e2f34801098
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a322-fullgraphdet-w13n64-client.sh"
expected_client=b90ed4faa032c966535917e5934cd2e8d008b37fcebc7747231c8137f4c3d685
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt322
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt322-compile
rpc_dir=/tmp/q38-ple2k-a322-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt322-supervisor
port=19992
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A322 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A322 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A322_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A322_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A322_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A322 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A322 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

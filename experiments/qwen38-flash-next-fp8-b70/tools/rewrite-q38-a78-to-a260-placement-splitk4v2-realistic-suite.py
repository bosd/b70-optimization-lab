...
VALIDATE_ONLY = os.environ.get("Q38_A260_REWRITE_VALIDATE_ONLY") == "1"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp0-4352-ple-only-a78", "tp4-mtp0-4352-ple-only-a260")
        seg = seg.replace("attempt78", "attempt260").replace("19750", "19926")
        seg = seg.replace("ATTEMPT=78", "ATTEMPT=260").replace("a78", "a260").replace("A78", "A260")
        return seg
    parts=[]; last=0
...
def main():
    ...
    env = os.environ.copy(); env["Q38_A260_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a260-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a260" in derived
    ...
    out_names = ("launch-tp4-mtp0-4352-ple-only-a260-fullgraphdet-w13n32.sh", "run-tp4-mtp0-4352-ple-only-a260-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp0-4352-ple-only-a260-fullgraphdet-w13n32.sh", "run-q38-a260-host-controlled.sh")

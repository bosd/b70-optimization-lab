...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a283-fullgraphdet-w13n32.sh"
expected_wrapper=cb3459d7223042be1dd0401e5d8e94c4ab48a45534397b1320aaed13e9f8b11e
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a283-fullgraphdet-w13n32-client.sh"
expected_client=e99ac687c4f85dd9aae60ffd7a834dc9503f281bc0eee44cd959e6508581280b
state=/tmp/q38-mtp0-ple-only-a283
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt283
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt283
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt283-compile
rpc_dir=/tmp/q38-ple2k-a283-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt283-supervisor
port=19952
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A283 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A283 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A283_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A283_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A283_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A283 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A283 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

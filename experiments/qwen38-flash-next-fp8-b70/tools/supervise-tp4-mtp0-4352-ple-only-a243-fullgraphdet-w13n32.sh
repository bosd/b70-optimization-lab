...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a243-fullgraphdet-w13n32.sh"
expected_wrapper=a8594544b13902af57ec86ccb092b7dcf20c83fd1740a3eb07dd78732615a911
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a243-fullgraphdet-w13n32-client.sh"
expected_client=0e1c4db751367a0a9b4bdd7d222ff6bb761574300189ececae7997eec5a4ce56
state=/tmp/q38-mtp0-ple-only-a243
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt243
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt243
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt243-compile
rpc_dir=/tmp/q38-ple2k-a243-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt243-supervisor
port=19913
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A243 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A243 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A243_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A243_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A243_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A243 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A243 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

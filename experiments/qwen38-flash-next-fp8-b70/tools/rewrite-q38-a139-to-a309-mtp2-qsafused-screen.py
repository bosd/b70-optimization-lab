#!/usr/bin/env python3
"""Create the A309 packet from frozen A139: graph MTP2 screen at the placement identity (PLE + embeddings offloaded at 12.25, hot experts resident, 3.5 GiB/rank never-hit experts host-placed) on the MTP1 placement head 005dc578."""
from __future__ import annotations
import hashlib, os, re, subprocess
from pathlib import Path
ROOT = Path(__file__).resolve().parent
VALIDATE_ONLY = os.environ.get("Q38_A309_REWRITE_VALIDATE_ONLY") == "1"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp2-4352-ple-only-a139", "tp4-mtp2-4352-ple-only-a309")
        seg = seg.replace("attempt139", "attempt309").replace("19810", "19977")
        seg = seg.replace("ATTEMPT=139", "ATTEMPT=309").replace("a139", "a309").replace("A139", "A309")
        return seg
    parts=[]; last=0
...
def patch_a177(l):
    ...
    l = l.replace('_selective_ple_only_uva', '_selective_ple_embed_budget12p25_uva_hctriton')
...
NEW_HEAD = "6d8724577dabbee5fa0bbc70c4d927c6174c8d8a"  # q38-placement-mtp1-clean: placement commits on the lossless MTP1 head; MTP2 = two speculative tokens on the same code
def patch_a309_launcher(l):
...
def patch_a309_client(c):
    ...
    c = patch_client(c)
    c = c.replace("0bd36f13056d79924e7598bf8d844db3a5b8b35639737c0ef0b5af68cad14753", "20546ff1b349ce5d969d29855ed9a95b38c5be2d22084b6cb4326bac4a7e8849").replace("4f4942289f3853f0dec60b9fcd14c644ca300abaaa9d9fa2ea56135f4d9f9c52", "20546ff1b349ce5d969d29855ed9a95b38c5be2d22084b6cb4326bac4a7e8849")
    return c
...
def main():
    ...
    out_names = ("launch-tp4-mtp2-4352-ple-only-a309-fullgraphdet-w13n32.sh", "run-tp4-mtp2-4352-ple-only-a309-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp2-4352-ple-only-a309-fullgraphdet-w13n32.sh", "run-q38-a309-host-controlled.sh")

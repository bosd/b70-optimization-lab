...
wrapper="${script_dir}/launch-tp4-mtp1-4352-ple-only-a313-fullgraphdet-w13n32.sh"
expected_wrapper=935eb83ad270cb9ae11c1194d47c1e9ef9998a0a3caf5c294c119178558b5fcb
client="${script_dir}/run-tp4-mtp1-4352-ple-only-a313-fullgraphdet-w13n32-client.sh"
expected_client=e7a9f4f6c78c6ae706baa078f6dc6af04d3dacb3a43560725385eaf3b102ae44
state=/tmp/q38-mtp1-ple-only-a313
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt313
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt313
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt313-compile
rpc_dir=/tmp/q38-ple2k-a313-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt313-supervisor
port=19983
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A313 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A313 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A313_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A313_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A313_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A313 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A313 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

#!/usr/bin/env python3
"""Create the A316 packet from frozen A311: the same fused-QSA MTP1 identity and the same
3.5 GiB-per-rank expert host placement, with a full per-expert routing census on head
248cd315 (per-rank JSON via Q38_PLACEMENT_HIT_CENSUS_OUT). A311 proved parked experts are
...
VALIDATE_ONLY = os.environ.get("Q38_A316_REWRITE_VALIDATE_ONLY") == "1"
SOURCES = {
...
NEW_HEAD = "248cd31596d0b6fc3df9de93a7d0181986c35da9"  # q38-placement-hit-census-v2: full per-expert routing histogram + per-rank dump
CENSUS_OUT = "/tmp/q38-a316-census"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp1-4352-ple-only-a311", "tp4-mtp1-4352-ple-only-a316")
        seg = seg.replace("attempt311", "attempt316").replace("19979", "19986")
        seg = seg.replace("ATTEMPT=311", "ATTEMPT=316").replace("a311", "a316").replace("A311", "A316")
        return seg
    parts = []
    last = 0
...
def main():
    ...
    env = os.environ.copy()
    env["Q38_A316_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a316-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a316" in derived
    ...
    out_names = (
        "launch-tp4-mtp1-4352-ple-only-a316-fullgraphdet-w13n32.sh",
        "run-tp4-mtp1-4352-ple-only-a316-fullgraphdet-w13n32-client.sh",
        "supervise-tp4-mtp1-4352-ple-only-a316-fullgraphdet-w13n32.sh",
        "run-q38-a316-host-controlled.sh",
    )

...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a293-mkldnndet-w13n32.sh"
expected_wrapper=751a23971064b85edb849b3cdde6d114c034d1857f8a80f7821f63115bb77a7e
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a293-mkldnndet-w13n32-client.sh"
expected_client=2b54e5a2c5a903f3ea06956a6aaf99918acd65da4d7109cc0854ccb72d0e7abb
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-mkldnndet-mtp0-4352-ple-only-r1-attempt293
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-mkldnndet-mtp0-4352-ple-only-r1-attempt293-compile
rpc_dir=/tmp/q38-ple2k-a293-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-mkldnndet-mtp0-4352-ple-only-r1-attempt293-supervisor
port=19962
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A293 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A293 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A293_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A293_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A293_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A293 host-pressure or NVMe-link guard'
  fi
  if [[ -e "$stop_file" || -e "$failure_file" ]]; then
    ...
    if [[ -e "$stop_file" && ! -e "$failure_file" ]] && \
       ...
       jq -e '.status == "passed" and .phase == "after" and
         ...
         "${run_dir}/mkldnndet-runtime-after.json" >/dev/null 2>&1 && \
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A293 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

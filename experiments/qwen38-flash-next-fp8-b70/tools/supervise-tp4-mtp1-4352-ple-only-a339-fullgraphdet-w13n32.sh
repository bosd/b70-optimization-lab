...
wrapper="${script_dir}/launch-tp4-mtp1-4352-ple-only-a339-fullgraphdet-w13n32.sh"
expected_wrapper=11e39475f40f8244788e60eec61188d3a00ca7653db8187da95d999786dac730
client="${script_dir}/run-tp4-mtp1-4352-ple-only-a339-fullgraphdet-w13n32-client.sh"
expected_client=cda68ae44ab2692ce9ad68fa35226eb29733f57e1519919ec4c189f2c1cf03cd
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt339
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt339-compile
rpc_dir=/tmp/q38-ple2k-a339-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt339-supervisor
port=19952
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A339 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A339 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A339_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A339_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A339_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A339 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A339 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a300-fullgraphdet-w13n32.sh"
expected_wrapper=4648d2a075dc5d62e0822f019c495cd602e55c51a801a4ad558b29e2cffe64c0
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a300-fullgraphdet-w13n32-client.sh"
expected_client=f0997c18db653210185342cfec49b09167721a34f7150262e3dbd6d892261d98
state=/tmp/q38-mtp0-ple-only-a300
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt300
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt300
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt300-compile
rpc_dir=/tmp/q38-ple2k-a300-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt300-supervisor
port=19969
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A300 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A300 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A300_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A300_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A300_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A300 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A300 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

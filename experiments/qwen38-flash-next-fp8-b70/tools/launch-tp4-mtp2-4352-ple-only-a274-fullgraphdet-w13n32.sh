...
derived=/tmp/q38-ple2k-a274-base.sh
expected_base=d5ccc4d52220f7ef46f19202436edf56e0c40f125b1b807c84125df18093b5c1
expected_derived=a165b7afc61d3340a8ee0faa1c14f6ad2d0a3e6991f0d90c6a8446d21af7c6fd
campaign=qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp2-4352-ple-only-r1
...
[[ "$(sha256sum "$tuned_config_map" | cut -d' ' -f1)" == a8f1f8982e3e1af80ff31b9e0a00afaacf1af1b3c401585109b4d60d3c8267be ]] || { printf 'FAIL: A274 tuned M1 map drifted\n' >&2; exit 1; }
[[ "$(jq -r '."1".num_warps' "$tuned_config_map")" == 8 && "$(jq -r '."1".W1_CONFIG.BLOCK_SIZE_N' "$tuned_config_map")" == 32 && "$(jq -r '."1" | has("W2_CONFIG")' "$tuned_config_map")" == false ]] || { printf 'FAIL: A274 tuned M1 map entry is not the qualified W13-N32 shape\n' >&2; exit 1; }
...
$0 == "rpc_dir=\"/tmp/${campaign}-attempt${attempt}-rpc\"" {
  print "rpc_dir=/tmp/q38-ple2k-a274-rpc"
  next
}
$0 == "expected_vllm_head=\"1372c62d975c554f4b465c8299bc5f3295301ceb\"" {
  print "expected_vllm_head=\"622191221475b53cc6f7f4d847860939f4c300ab\""
  next
}
...
$0 == "print(f'\''engine_config=tp4_ep4_triton_eager_mtp{mtp}_selective_ple_and_embed_uva'\'')" {
  print "print(f'\''engine_config=tp4_ep4_triton_fullgraphdet_mtp{mtp}_selective_ple_embed_budget12p25_uva_hctriton'\'')"
  next
}
...
$0 == "    generation_config='\''vllm'\'', load_format='\''safetensors'\'', async_scheduling=False," {
  ...
  print "        '\''cudagraph_capture_sizes'\'': [1, 2, 3],"
  print "        '\''max_cudagraph_capture_size'\'': 3, '\''compile_sizes'\'': [],"
...
$0 == "assert config.cache_config.kv_cache_memory_bytes == kv_cache_memory_bytes" {
  ...
  print "assert config.compilation_config.cudagraph_capture_sizes == [1, 2, 3]"
  print "assert config.compilation_config.max_cudagraph_capture_size == 3"
...
$0 == "  --enforce-eager" {
  print "  --compilation-config '\''{\"mode\":0,\"cudagraph_mode\":\"FULL_DECODE_ONLY\",\"cudagraph_capture_sizes\":[1,2,3],\"max_cudagraph_capture_size\":3,\"compile_sizes\":[],\"cudagraph_num_of_warmups\":1}'\''"
...
index($0, "diagnostics=none") > 0 {
  ...
  print "  printf '\''compilation_config={\"mode\":0,\"cudagraph_mode\":\"FULL_DECODE_ONLY\",\"cudagraph_capture_sizes\":[1,2,3],\"max_cudagraph_capture_size\":3,\"compile_sizes\":[],\"cudagraph_num_of_warmups\":1}\\n'\''"
...
' "$base" >"$derived"
chmod 700 "$derived"
if [[ "${Q38_A274_DERIVED_SOURCE_ONLY:-0}" == 1 ]]; then cat "$derived"; exit 0; fi
...
grep -Fxq "print(f'engine_config=tp4_ep4_triton_fullgraphdet_mtp{mtp}_selective_ple_embed_budget12p25_uva_hctriton')" "$derived"
...
grep -Fxq 'expected_vllm_head="622191221475b53cc6f7f4d847860939f4c300ab"' "$derived"
grep -Fxq "  printf 'diagnostics=full-decode-graph-public-oneccl-torch-trace\n'" "$derived"
! grep -Fq "diagnostics=none" "$derived"
grep -Fxq 'rpc_dir=/tmp/q38-ple2k-a274-rpc' "$derived"
grep -Fxq '[[ "${max_model_len}" == "4352" ]] || {' "$derived"
grep -Fxq '[[ "${mtp}" == "2" ]] || {' "$derived"
...
if [[ "${Q38_A274_VALIDATE_ONLY:-0}" == 1 ]]; then
...
if [[ "${Q38_A274_VALIDATE_ONLY:-0}" != 1 ]]; then
  [[ "$(findmnt -no SOURCE,FSTYPE --target /mnt/usb-models)" == "/dev/sda2 fuseblk" ]] || {
    printf 'FAIL: A274 evidence mount is not /dev/sda2 fuseblk\n' >&2
    exit 1
  }
  [[ "$(findmnt -no SOURCE,FSTYPE --target /mnt/fast-ai)" == "/dev/nvme0n1p2 ext4" ]] || {
    printf 'FAIL: A274 model mount is not /dev/nvme0n1p2 ext4\n' >&2
    exit 1
  }
  expected_nvme_aer_cor=${Q38_A274_NVME_AER_BASELINE:-}
  expected_root_aer_cor=${Q38_A274_ROOT_AER_BASELINE:-}
  expected_nvme_sectors_read=${Q38_A274_NVME_SECTORS_READ_BASELINE:-}
  [[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
     "$expected_nvme_sectors_read" =~ ^[0-9]+$ ]] || {
    printf 'FAIL: A274 requires numeric host-control AER baselines\n' >&2
    exit 1
  }
  ...
  (( mem_available_kib >= 120000000 )) || { printf 'FAIL: A274 requires MemAvailable >= 120000000 KiB\n' >&2; exit 1; }
  (( swap_total_kib == 0 )) || { printf 'FAIL: A274 requires disk-backed swap disabled\n' >&2; exit 1; }
  [[ "$aspm_policy" == *'[performance]'* ]] || { printf 'FAIL: A274 requires PCIe ASPM performance policy\n' >&2; exit 1; }
  (( root_aer_cor == expected_root_aer_cor && nvme_aer_cor >= expected_nvme_aer_cor && \
    ...
    printf 'FAIL: A274 bounded local-NVMe guard failed\n' >&2; exit 1;
  }
  (( nvme_available_bytes >= 220000000000 )) || { printf 'FAIL: A274 requires >= 220000000000 free NVMe bytes\n' >&2; exit 1; }
fi
...
export TORCH_TRACE=${RUN_PARENT}/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp2-4352-ple-only-r1-attempt274/torch-trace
...
export MTP=2 MTP_EXACT=0 MAX_MODEL_LEN=4352 ATTEMPT=274 PORT=19943

...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a296-fullgraphdet-w13n32.sh"
expected_wrapper=2fcc4372929a4a883d22e381cffe30ba70e9e6003ffe47ce6f01742a363fad99
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a296-fullgraphdet-w13n32-client.sh"
expected_client=cfad747dd2dd7cbf4567c4a88dbac01914860ff2c20c8570dc1d288018582aa6
state=/tmp/q38-mtp0-ple-only-a296
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt296
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt296
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt296-compile
rpc_dir=/tmp/q38-ple2k-a296-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt296-supervisor
port=19965
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A296 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A296 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A296_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A296_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A296_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A296 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A296 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

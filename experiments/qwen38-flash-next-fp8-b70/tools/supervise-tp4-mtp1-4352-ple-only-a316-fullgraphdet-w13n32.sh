...
wrapper="${script_dir}/launch-tp4-mtp1-4352-ple-only-a316-fullgraphdet-w13n32.sh"
expected_wrapper=1cf77c4a4336afc5e050ebfd901843a3c47bfac4b14d013951b3b9bff286f25e
client="${script_dir}/run-tp4-mtp1-4352-ple-only-a316-fullgraphdet-w13n32-client.sh"
expected_client=825fb8c4de42ed607f8ecc3999939d6dc605309be68fce1f3a0800c4c73a730c
state=/tmp/q38-mtp1-ple-only-a316
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt316
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt316
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt316-compile
rpc_dir=/tmp/q38-ple2k-a316-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt316-supervisor
port=19986
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A316 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A316 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A316_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A316_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A316_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A316 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A316 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

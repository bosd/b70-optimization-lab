...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a334-fullgraphdet-w13n32.sh"
expected_wrapper=a9444a356e905ac4e4f35d90d5984d61a81f1d4501ae9ed021a0bd40319b612c
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a334-fullgraphdet-w13n32-client.sh"
expected_client=ca9c6ec8b93c78db716554bec94b1d751b54dfc89ad02c58f384f81ef060c183
state=/tmp/q38-mtp0-ple-only-a334
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt334
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt334
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt334-compile
rpc_dir=/tmp/q38-ple2k-a334-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt334-supervisor
port=19947
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A334 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A334 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A334_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A334_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A334_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A334 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A334 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

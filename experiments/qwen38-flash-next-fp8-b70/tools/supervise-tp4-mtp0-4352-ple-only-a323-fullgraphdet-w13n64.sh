...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a323-fullgraphdet-w13n64.sh"
expected_wrapper=7c4cf495b315475c04681354303437c8c13c4e251bc4727befa54927a4fa6bc5
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a323-fullgraphdet-w13n64-client.sh"
expected_client=16eb2f15a974ca5c655bb6e8f3491d460ff9c09c9692a2a0b0042666aa3ba332
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt323
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt323-compile
rpc_dir=/tmp/q38-ple2k-a323-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt323-supervisor
port=19993
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A323 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A323 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A323_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A323_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A323_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A323 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A323 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

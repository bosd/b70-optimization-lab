...
wrapper="${script_dir}/launch-tp4-mtp1-4352-ple-only-a268-fullgraphdet-w13n32.sh"
expected_wrapper=cb180b699f45ecdc35049b11f973599f4ee7a78a52ee4dd9a68c37a222177970
client="${script_dir}/run-tp4-mtp1-4352-ple-only-a268-fullgraphdet-w13n32-client.sh"
expected_client=bce4eb96d178801925ed160443ae10b16f6cad8ef55f0a0d520df2f0d6e09e3e
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt268
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt268-compile
rpc_dir=/tmp/q38-ple2k-a268-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt268-supervisor
port=19938
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A268 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A268 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A268_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A268_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A268_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A268 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A268 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

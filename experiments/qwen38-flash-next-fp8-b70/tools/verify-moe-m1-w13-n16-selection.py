#!/usr/bin/env python3
"""Verify the default-off M1 W13-N16 tuned-map integration.

This helper performs no inference.  It binds the candidate map to the retained
M1 map, verifies that their sole semantic difference is the nested W13 N16
...
def validate_maps(
    ...
    if candidate[1].get("W1_CONFIG") != W13_DELTA:
        raise IntegrationContractError("key 1 lacks the exact W13 N16 delta")
    ...
    if flattened_m1 != base[1]:
        raise IntegrationContractError("key 1 changes fields beyond W13 N16")
...
def build_receipt(
    ...
    return {
        "schema_version": 1,
        "status": "pass",
        "classification": "qwen38_m1_w13_n16_static_integration_receipt",

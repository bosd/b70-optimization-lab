...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a237-fullgraphdet-w13n32.sh"
expected_wrapper=26b0538d7134be6a975f68e9f199d367093fe96137c27c051b0f64abafd879ff
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a237-fullgraphdet-w13n32-client.sh"
expected_client=0d6dc4767992bf8b2b458a934623fade755d97db3ba3135db1d6a3cfc5dfc2ce
state=/tmp/q38-mtp0-ple-only-a237
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt237
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt237
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt237-compile
rpc_dir=/tmp/q38-ple2k-a237-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt237-supervisor
port=19907
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A237 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A237 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A237_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A237_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A237_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A237 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A237 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

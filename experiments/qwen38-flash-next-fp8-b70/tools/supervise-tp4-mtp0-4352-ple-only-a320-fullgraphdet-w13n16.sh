...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a320-fullgraphdet-w13n16.sh"
expected_wrapper=644e1dbc4f678baa4b3871301d0b6b43c36148039560f1daaf9941d68b39dca7
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a320-fullgraphdet-w13n16-client.sh"
expected_client=483306891a0aa1ea8ff196d0f1a9fd20efd5de8d3865a6174636f597d4c541cb
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt320
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt320-compile
rpc_dir=/tmp/q38-ple2k-a320-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt320-supervisor
port=19990
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A320 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A320 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A320_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A320_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A320_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A320 host-pressure or NVMe-link guard'
  fi
...
.identity.max_model_len == 4352 and
         ...
         .identity.tuned_config_folder == "moe-m1-w13-n16" and
         .identity.tuned_config_map_sha256 == "89c130d70a34432124b9cee1621080ea75f4b303c20bce784d13af82e4219894" and
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A320 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a276-fullgraphdet-w13n32.sh"
expected_wrapper=41e72340a09acc068d3172707a6c23f3e50c0f7f07d8df45a61a2f57a2745ec2
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a276-fullgraphdet-w13n32-client.sh"
expected_client=0623bcd91ee258eb704f7f615d70c5ff59317080ec42a74c9bf296c754bbe978
state=/tmp/q38-mtp0-ple-only-a276
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt276
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt276
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt276-compile
rpc_dir=/tmp/q38-ple2k-a276-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp0-4352-ple-only-r1-attempt276-supervisor
port=19945
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A276 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A276 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A276_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A276_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A276_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A276 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A276 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

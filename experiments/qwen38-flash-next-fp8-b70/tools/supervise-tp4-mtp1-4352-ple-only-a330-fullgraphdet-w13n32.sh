...
wrapper="${script_dir}/launch-tp4-mtp1-4352-ple-only-a330-fullgraphdet-w13n32.sh"
expected_wrapper=c87b9c43523ba94c90e43d9db823b80b96358913cb326e27f2e76303d3b3c92a
client="${script_dir}/run-tp4-mtp1-4352-ple-only-a330-fullgraphdet-w13n32-client.sh"
expected_client=03a62fe233463cc4e03c0e8e30a571f9a36609ac496cf875d1c54538f01ba011
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt330
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt330-compile
rpc_dir=/tmp/q38-ple2k-a330-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt330-supervisor
port=19943
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A330 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A330 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A330_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A330_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A330_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A330 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A330 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

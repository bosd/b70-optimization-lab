...
wrapper="${script_dir}/launch-tp4-mtp1-4352-ple-only-a229-fullgraphdet-w13n32.sh"
expected_wrapper=d8b62fd8821a5533c9f3e4ad341632f1f786ac31b262152a85044b9c269ca8d4
client="${script_dir}/run-tp4-mtp1-4352-ple-only-a229-fullgraphdet-w13n32-client.sh"
expected_client=c26eedc2a89ace35b8eee0bf83f1de236d855abf67b4d417a0f74f3fdce4a70e
state=/tmp/q38-mtp1-ple-only-a229
stop_file="${state}.stop"
failure_file="${state}.failed"
run_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt229
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt229
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt229-compile
rpc_dir=/tmp/q38-ple2k-a229-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-fullgraphdet-mtp1-4352-ple-only-r1-attempt229-supervisor
port=19900
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A229 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A229 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A229_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A229_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A229_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A229 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A229 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

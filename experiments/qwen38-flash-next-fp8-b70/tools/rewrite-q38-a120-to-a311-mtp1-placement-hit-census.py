#!/usr/bin/env python3
"""Create the A311 packet from frozen A120 (MTP1) for the A213 identity: PLE + embeddings offloaded at 12.25 (12.22 GiB), hot experts resident, never-hit experts host-placed at load time (/home/steve/llm-optimizations/experiments/qwen38-flash-next-fp8-b70/data/20260906-q38-expert-host-placement-3p5gib-per-rank.json) on clean head 795a9dd94c16; realistic suite (LocalMaxxing metric)."""
from __future__ import annotations
import hashlib, os, re, subprocess
from pathlib import Path
ROOT = Path(__file__).resolve().parent
VALIDATE_ONLY = os.environ.get("Q38_A311_REWRITE_VALIDATE_ONLY") == "1"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp1-4352-ple-only-a120", "tp4-mtp1-4352-ple-only-a311")
        seg = seg.replace("attempt120", "attempt311").replace("19792", "19979")
        seg = seg.replace("ATTEMPT=120", "ATTEMPT=311").replace("a120", "a311").replace("A120", "A311")
        return seg
    parts=[]; last=0
...
NEW_HEAD = "795a9dd94c16bcfbedc2117bbcb33c988f2e6c90"  # q38-placement-mtp1-clean: lossless MTP1 head + table kernel + load-time never-hit expert placement
def patch_a311_launcher(l):
...
def main():
    ...
    assert "q38-ple2k-a311" in derived
    ...
    client = patch_a311_client(client)
    ...
    supervisor = patch_a311_supervisor(supervisor)
    ...
    out_names = ("launch-tp4-mtp1-4352-ple-only-a311-fullgraphdet-w13n32.sh", "run-tp4-mtp1-4352-ple-only-a311-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp1-4352-ple-only-a311-fullgraphdet-w13n32.sh", "run-q38-a311-host-controlled.sh")

#!/usr/bin/env python3
"""Create the A285 step and per-layer timing packet from frozen A83 (eager MTP0 twin of A122 with the per-step timing log, no trace).
...
d132de8c). Attempt 163 / port 19954.
"""
...
VALIDATE_ONLY = os.environ.get("Q38_A285_REWRITE_VALIDATE_ONLY") == "1"
...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp1-4352-ple-only-a83", "tp4-mtp0-4352-ple-only-a285")
        ...
        seg = seg.replace("attempt83", "attempt285").replace("19755", "19954")
        seg = seg.replace("ATTEMPT=83", "ATTEMPT=285").replace("a83", "a285").replace("A83", "A285")
        return seg
    parts=[]; last=0
...
def main():
    ...
    if MTP0:
        ...
        launcher = replace_once(launcher, "export MTP=1 MTP_EXACT=0 MAX_MODEL_LEN=4352 ATTEMPT=285 PORT=19954\n", "export MTP=0 MTP_EXACT=0 MAX_MODEL_LEN=4352 ATTEMPT=285 PORT=19954\n")
        ...
        launcher = replace_once(launcher, "export MTP=1 MTP_EXACT=0 MAX_MODEL_LEN=4352 ATTEMPT=285 PORT=19954\n", "export MTP=1 MTP_EXACT=1 MAX_MODEL_LEN=4352 ATTEMPT=285 PORT=19954\n")
    ...
    env = os.environ.copy(); env["Q38_A285_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a285-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a285" in derived and "  --enforce-eager\n" in derived
    ...
    names = ("launch-tp4-mtp0-4352-ple-only-a285-mkldnndet-w13n32.sh", "run-tp4-mtp0-4352-ple-only-a285-mkldnndet-w13n32-client.sh", "supervise-tp4-mtp0-4352-ple-only-a285-mkldnndet-w13n32.sh", "run-q38-a285-host-controlled.sh")

...
wrapper="${script_dir}/launch-tp4-mtp0-4352-ple-only-a286-mkldnndet-w13n32.sh"
expected_wrapper=5475c18344a28d1e28fc8953ec400a259069a3ab5d8f96e47ff189ddc3fa8dea
client="${script_dir}/run-tp4-mtp0-4352-ple-only-a286-mkldnndet-w13n32-client.sh"
expected_client=5c732d9b7b01e8b90fbac6a5b76cc9546d1a9d9db83de1808bc1e30feab09e13
...
cache_dir=/mnt/usb-models/llm-runtime/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-mkldnndet-mtp0-4352-ple-only-r1-attempt286
compile_dir=/tmp/qwen38-flash-next-fp8-tp4-ep4-mkldnndet-mtp0-4352-ple-only-r1-attempt286-compile
rpc_dir=/tmp/q38-ple2k-a286-rpc
evidence_dir=/mnt/usb-models/bench-results/qwen38-flash-next-fp8-b70/qwen38-flash-next-fp8-tp4-ep4-mkldnndet-mtp0-4352-ple-only-r1-attempt286-supervisor
port=19955
...
[[ "$expected_nvme_aer_cor" =~ ^[0-9]+$ && "$expected_root_aer_cor" =~ ^[0-9]+$ && \
  ...
  printf 'FAIL: A286 supervisor requires numeric host-control AER baselines\n' >&2
  exit 1
}
...
if ! sample_pressure; then
  ...
  wait "$journal_follow_pid" 2>/dev/null || true
  journal_follow_pid=""
  printf 'FAIL: initial A286 host-pressure gate failed\n' >&2
  exit 1
fi
...
timeout --signal=TERM --kill-after=30s 10000s env -i \
  ...
  Q38_A286_NVME_AER_BASELINE="$expected_nvme_aer_cor" \
  Q38_A286_ROOT_AER_BASELINE="$expected_root_aer_cor" \
  Q38_A286_NVME_SECTORS_READ_BASELINE="$expected_nvme_sectors_read" \
...
while kill -0 "$child" 2>/dev/null; do
  ...
  if ! sample_pressure; then
    write_atomic "$failure_file" 'FAIL A286 host-pressure or NVMe-link guard'
  fi
...
if sample_pressure; then
  ...
  write_atomic "$failure_file" 'FAIL A286 final host-pressure or NVMe-link guard'
fi
rc=$child_rc

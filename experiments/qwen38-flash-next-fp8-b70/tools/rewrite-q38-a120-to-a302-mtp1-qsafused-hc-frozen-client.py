...
def successor(text):
    def rename(seg):
        seg = seg.replace("tp4-mtp1-4352-ple-only-a120", "tp4-mtp1-4352-ple-only-a302")
        seg = seg.replace("attempt120", "attempt302").replace("19792", "19971")
        seg = seg.replace("ATTEMPT=120", "ATTEMPT=302").replace("a120", "a302").replace("A120", "A302")
        return seg
    parts=[]; last=0
...
def main():
    ...
    launcher = patch_a302_launcher(launcher)
    ...
    env = os.environ.copy(); env["Q38_A302_DERIVED_SOURCE_ONLY"] = "1"
    derived = subprocess.run(["bash"], input=launcher, text=True, capture_output=True, check=True, env=env).stdout
    Path("/tmp/q38-ple2k-a302-base.sh").unlink(missing_ok=True)
    assert "q38-ple2k-a302" in derived
    launcher = launcher.replace("expected_derived=" + "0"*64, "expected_derived=" + digest(derived))
    client = successor(source("run-tp4-mtp1-4352-ple-only-a120-fullgraphdet-w13n32-client.sh"))
    client = patch_a302_client(client)
    ...
    supervisor = patch_a302_supervisor(supervisor)
    ...
    out_names = ("launch-tp4-mtp1-4352-ple-only-a302-fullgraphdet-w13n32.sh", "run-tp4-mtp1-4352-ple-only-a302-fullgraphdet-w13n32-client.sh", "supervise-tp4-mtp1-4352-ple-only-a302-fullgraphdet-w13n32.sh", "run-q38-a302-host-controlled.sh")

...
- **R221 built and validated (11:20):** image `neural-download/vllm-openai-xpu:qwen38-int4-w4a16-fixed-k-r221`
  id 699e2699, `_xpu_C` sha256 271db0d4. Default two-tier fixed-K: all 14 shapes invariant, bitwise equal to
  the natural decode entry, decode timings unchanged (`data/qwen38-int4-w4a16-strategy-screen-r220/r221-default.json`).
  `/mnt/fast-ai/bench-results/r221-image.env` written; the @reboot cron now launches the R222 matrix
...
- **2026-09-06 14:15, LocalMaxxing:** INT4 headline approved on submission, `cmtq3mx7e03gen701xxpwu67z`, **112.344 tok/s** (center of the R257 pair), ledger `results/localmaxxing-submissions.md`.
- **2026-09-06 night (R261-R278), INT4 lane optimization pass:** single user is at the floor of this stack (V2 model runner with the draft-only INT4 head ported, R266/R269: 112.70/112.96 lossless = V1; async scheduling already on; CPU binding no effect; graph sizes no effect). Multi-user: single-pass ladders under-report c2 (one-time ~1 s Dynamo recompile); warm two-pass ladder (R265b) c2 147.6, c4 252.9, c8 405.4, **c16 580.4 (16/16 exact)**, c32 361 (31/32), c64 445 (58/64); MTP0 exact c1-c64 at ~1000 tok/s. c16->c32 collapse: W4A16 GEMM dequant-bound above ~32 rows (55 ms at M=160), GDN speculative kernel streams fp32 SSM state per sequence (27 ms at c32), grouped GDN branch adds ~40 ms host and blocks graph capture above 16 sequences (R276 overlay fixes the capture); captured verify graphs at c32/c64 did not move the ladder (R275b), so the c32 cap was the R213b W4A16 pad silently on above 128 rows in every runner-launched server (`VLLM_XPU_W4A16_DETERMINISM_PAD` not forwarded by the launcher; R278k profile: 529 vs 180 us per GEMM); launchers fixed; corrected ladder R281 c16 584 (16/16), c32 633 (30/32), c64 604 (62/64). R282 (R276 image, graphs to 320): c2 191 / c4 295 / c16 579 / c32 635; R283 strict pair 112.90/113.00 12/12 -> R276 adopted as the served image (GHCR @sha256:521eb277…, launcher default, capture sizes to 320). Correction: the R257 headline pair ran the R228 image (R256 identical code path). 04:47 xe fault on 03:00.0 during the R277 load and 05:34 fault on e3:00.0 during the R278 load: boot retired; `@reboot` autolaunch runs R277b then R278 (`boot-20260906-qwen38-int4-r277b-r278-autolaunch.sh`).
- **2026-09-06 evening (R281-R290), many users settled:** with the W4A16 pad forwarded off and a 128-sequence admission budget, the depth-4 aggregate plateaus at ~590-640 tok/s from 32 users (the same near-tie prompts flip at the same token positions in every rung of 64+, the >32-row GEMM tier), depth 1 is the fastest exact setting at 8-32 users (711/854 tok/s, R287/R288), and no speculation is fastest and exact from 64 users (991 at c64, 1085 at c128, R284). One card (R285/R289): MTP0 exact through c128, 514 at c32; depth 1 465 at c16; depth 4 caps ~245. A 4096-token prefill budget (R290) is withheld: the same prompts flip without speculation and aggregate does not improve. Recipe table, package profiles, home-page cells and `learn/multi-user.html` updated. The R220+R221 kernel chain replayed bit-for-bit from a clean clone (`_xpu_C` 271db0d4, both image ids identical; stale builder hash pin and a `grep -q` pipefail bug fixed in the scripts).
- **2026-09-06 evening, Gemma 4 26B A4B Q8 lane:** the record toolchain is packaged as a pinned oneAPI 2026.0 container (`repro/gemma4-26b-a4b-q8-b70-125tps-20260701/docker/`, image `e22b500d…`, `llama-server` ae45b2f0…, entrypoint pins the 2026.0 runtime and preserves arguments; both B70s enumerate through the harness shim), plus a labelled 2026.1.1 host compatibility build (26fc4868…). The pinned model was never on this host; the 27.6 GB download is WAN-throttled (~0.7 MB/s all afternoon and evening; aria2 16 connections) to `/home/steve/llm-models/gemma4-26b-a4b-it-q8-gguf`, and `/home/steve/llm-models/gemma4-autoreplay-queued.sh` will quantize the draft and run the strict record gate with both binaries unattended when it lands. The model landed at 23:58 once the WAN recovered (9 MiB/s); on 2026-09-07 both binaries reconstructed the reference draft (1f6706e4…) and passed the strict gate: host 2026.1 build 115.29 tok/s median (class-balanced 115.79), container 111.20 (112.05) and 116.35 (115.53) on a second run, canary 512/512, cache zero (`data/gemma4-q8-gpu0-125repro-{compat2026.1,container2026.0}-20260907T04*/summary.json`). Valid support runs, not new records. **2026-09-07 afternoon, the kernel explains the identity ceiling (two models, one cause).** The lab's row-invariant W4A16 INT4 kernel (fixed-K two-tier, built for the Qwen3.8 INT4 lane) is what makes outputs reproducible, not the model or the workload. Both Qwen3.5 sizes show it, in different ways, on identical stacks: the 9B FP8 route is repeat-exact but flips near-tie tokens from 16 concurrent users, while its W4A16 route is byte-exact at every rung through 64 users (1268.4 tok/s, 64/64, both passes); the 4B FP8 route is not even repeat-exact between fresh servers (11/12, 9/12, 11/12 over three pairs; three tie-prone prompts, two variants each), while its W4A16 route passes every gate 12/12 at 177.41/177.17 tok/s. Packages: `qwen35-9b-w4a16-b70` (113.63/112.90, LocalMaxxing `cmtrhoyl1000cps01o43bhl72`) and `qwen35-4b-w4a16-b70` (LocalMaxxing `cmtrj2tp3000hps01n3fadg9d`), families `qwen-9b` and `qwen-4b`. The 4B FP8 route is documented and rejected, not packaged. The W4A16 determinism pad is inert without speculation and costs 13% at 64 users with it, buying no identity, so it stays off; the residual speculative flips above 16 users are not row-tier effects.

**2026-09-07 midday, Qwen3.5 quick lanes:** (a) **Qwen3.5-9B W4A16** (RedHatAI a398088c, compressed-tensors INT4) is the better route for the same model: depth 3 + draft INT4 head 113.627/112.904 tok/s vs FP8's 98.251/98.027, all gates 12/12, LocalMaxxing approved `cmtrhoyl1000cps01o43bhl72` at 113.265. Its compressed-tensors path selects the lab's row-invariant wNa16 INT4 kernel with no relabel, and **without speculation it is byte-exact at every ladder rung through 64 users in both passes (1268.4 tok/s at c64, 64/64)** where the FP8 route flips from c16. The identity ceiling is a kernel property, not a model or workload property. Recipe `repro/qwen35-9b-w4a16-b70/`, package, family packet and home rows published. (b) **Qwen3.5-4B FP8** (397b7ba4) fails the base gate: two fresh MTP0 servers differ on `code-review` at token 284 (a clean near-tie: "table in the DB" vs "`failed_events` table"), 11/12, so the harness aborted before speculation; two more MTP0 pairs (q2/q3) are running to tell a stable split from a rare flip. A pad probe (W4A16 determinism pad on, ladders) is queued after them.

**2026-09-07 morning, Qwen3.5-9B full lane (user: lossless, matrix, package/recipe, A+, docker, publish incl. LocalMaxxing):** recipe `repro/qwen35-9b-fp8-b70/` (public launcher on the FP8 strict chain + R276 contract identities), package `packages/qwen35-9b-fp8-b70/` (candidate), catalog entry, harness `experiments/qwen35-9b-b70/scripts/run-20260907-qwen35-9b-campaign-v2.sh`. Campaign c1 (TP1, depth 3, graphs): strict pair 76.917/76.879 tok/s class-balanced, G1/G2/G3 12/12; MTP0 50.18/50.15; ladders depth 3 exact through c8 (463), MTP0 c64 1254 (58/64). The 105 tok/s figure from the quick lane is chat mode with visible thinking (workload, not env: 14-variant bisect). ML Bottleneck target 56.56 (1 card) → A+ at 1.36x. Image also mirrored to `ghcr.io/steveseguin/vllm-openai-xpu-b70` (that package is still **private**, so use the public `ghcr.io/steveseguin/vllm-openai-xpu-qwen38-int4@sha256:521eb277…`, verified anonymously pullable by tag and digest on 2026-09-07). c2 (depth 3 + draft-only INT4 lm_head): 98.251/98.027 tok/s, G1/G2/G3 12/12, the new headline (1.74x the ML Bottleneck target); c3 depth 4 withheld (repeat-exact, 8/12 vs the oracle, no faster). LocalMaxxing approved `cmtqyanxc00bjpa01nojy87wp` at 98.139 (one card) and `cmtqzhlvn00c0pa0109xxfb2f` at 147.801 (two cards, c5: 147.712/147.890, G1-G3 12/12; graph off c4 96.78/96.75 lossless). Matrix complete 06:05: depth 4 withheld on both card counts (8/12, 9/12), depths 5/6 withheld and slower; 2K-32K ladders 18/18 exact on one card (86.8 at 32K) and two (140.6 at 32K). GPUs free. Matrix chain c2 (depth 4) c3 (graph off) c4/c5 (TP2) c6/c7 (32K) queued behind the harness bisect. The quick Qwen3.5-9B lane then ran overnight on one B70 with the R276 image and the FP8 launchers (`experiments/qwen35-9b-b70/`): MTP depth 3 with graph capture gives 105.3 tok/s median (tokens 1-100) at 12/12 byte-identity with no-speculation (50.6); depths 4-6 reach ~110 but diverge on one near-tie prompt. One pass per row, no package yet.
...
A77, an independently started repeat, reproduced A76 at every depth: the
...
`0a03a84c`) turned out not to apply: A90 showed the base attention
forward is never entered on this model, whose full-attention layers run
their own query-sparse attention (`Qwen4ExpQSAAttention`); A87-A90 are
four more servers of the A85 identity. The remaining two-row suspects are
the QSA indexer/top-k and kernel, the Triton block-FP8 MoE at M=2, and the

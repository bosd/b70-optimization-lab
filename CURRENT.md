...
- **2026-09-06/07, Qwen3.5 and Gemma work (condensed; detail in the lane notes and result packets).** Four lanes were
  published in this window and their evidence lives in `experiments/qwen35-{4b,9b}-b70/`, `repro/qwen35-*`,
  `repro/gemma4-26b-a4b-q8-b70-125tps-20260701/` and `results/localmaxxing-submissions.md`.
  - **The identity ceiling is a kernel property, not a model or workload one.** The lab's row-invariant W4A16 INT4
    kernel keeps outputs reproducible where FP8 does not, and it fails differently on each model: Qwen3.5-9B FP8 is
    repeat-exact but flips near-tie tokens from 16 concurrent users, while Qwen3.5-4B FP8 is not even repeat-exact
    between fresh servers (11/12, 9/12, 11/12 over three pairs; three tie-prone prompts with two variants each). Both
    models pass every gate on W4A16, and the 9B is byte-exact at every ladder rung through 64 users (1268.4 tok/s,
    64/64, both passes). The determinism pad does not help: inert below its 128-row threshold, 13% slower at 64 users
    above it, no identity gain, so it stays off. Reader-facing writeup: `learn/multi-user.html#identity-kernel`.
  - **Published packages:** `qwen35-9b-w4a16-b70` (113.63/112.90 tok/s, LocalMaxxing `cmtrhoyl1000cps01o43bhl72`),
    `qwen35-4b-w4a16-b70` (177.41/177.17, `cmtrj2tp3000hps01n3fadg9d`), `qwen35-9b-fp8-b70` (98.25/98.03 one card and
    147.71/147.89 on two, `cmtqyanxc00bjpa01nojy87wp` and `cmtqzhlvn00c0pa0109xxfb2f`; superseded by the INT4 route and
    cross-linked as such). The 4B FP8 route is documented and rejected, not packaged. Families `qwen-4b`, `qwen-9b`.
  - **Qwen3.8 27B INT4 many-user guidance:** depth 4 for one to four users, depth 1 for eight to thirty-two (854 tok/s
    at 32, exact), no speculation beyond (992 at 64, 1085 at 128). One card: MTP0 exact through c128, 514 at c32. A
    4096-token prefill budget is withheld. The R220+R221 kernel chain replays bit-for-bit from a clean clone.
  - **Gemma 4 26B A4B Q8:** the record toolchain is packaged as a pinned oneAPI 2026.0 container (image `e22b500d…`,
    `llama-server` `ae45b2f0…`); both it and a labelled oneAPI 2026.1 host build reproduce the record gate on one B70
    (115.29, 111.20 and 116.35 tok/s against a 113.6-125.0 spread), so the container carries no overhead.
  - **Infrastructure:** GHCR images verified anonymously pullable by tag and digest (the `-b70` mirror name is private,
    every recipe cites the public `-qwen38-int4` one); all 18 model pins verify against their publishers
    (`tools/verify-model-manifests-remote.py`); repository-relative documentation links are checked in CI
    (`tools/check-doc-links.py`); the MiniMax vLLM compile caches moved to the external SSD, taking the fast disk from
    14 GB to 325 GB free.
